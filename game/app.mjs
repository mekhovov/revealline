...
} from "./content-design/default-entry.mjs";
import {
...
import {
  librarySuccessor,
  retainedLibraryMission,
} from "./mission-library/continuous-next.mjs";
import {
...
import {
  BoardPainter,
  boardPaintSizeForRun,
  boardPaintSizeForLevel,
} from "./ui/render.mjs";
...
try {
  globalThis.RevealLineBoot?.progress?.(
    "Loading missions and flight equipment…",
  );
  ...
  let installedEntries = [baseEntry],
    ...
    masteryCatalog = createMasteryCatalog([
      { campaign: baseEntry.campaign, sourcePackId: null },
    ]);
    ...
    const registrations = createMasteryCatalog(
      entries.map((entry) => ({
        ...
        ...(entry.sourcePackFormat
          ? {
              sourcePackFormat: entry.sourcePackFormat,
              masteries: entry.masteries,
            }
          : {}),
      })),
    );
  ...
  if (!buildVersion || buildVersion === "__REVEALLINE_VERSION__")
    buildVersion = "dev";
  let isRelease = false;
  try {
    ...
    if (
      ...
      /^[0-9a-f]{40,64}$/.test(info.sourceRevision)
    )
  ...
  if (isRelease)
    document
      .querySelectorAll("[data-source-only]")
  ...
  if (courseRequest) {
    ...
    const prepared = await prepareScenario(JSON.parse(raw), {
      classRecipes: classRegistry,
    });
  ...
  const authoredRoute =
    !practiceSession && (await loadAuthoredJourneyRoute(journeyRequest));
  ...
  const journeyPreferences = authoredJourney
    ? createJourneyPreferences({ window })
    : null;
  ...
  const journeyEnabled =
    (params.get("journey") === "1" || authoredJourney) && !practiceSession;
  const journeySaveNotice = journeyEnabled
    ? attachJourneySaveNotice({ document })
    : null;
  ...
  const controllerPreviewRequested =
    practiceSession && params.get("controller-preview") === "1";
  if (
    ...
    throw new Error(
      "Return to Controller practice and choose Load practice to continue.",
    );
  const controllerPreview = attachControllerPreview({
  ...
  const packCommits = createPackCommitCoordinator({
    ...
    onReconciled: () => {
      ...
      contentStatus(
        "Pack storage updated; your newer play selection was kept.",
      );
    },
    onError: (error) => {
  ...
  globalThis.RevealLineBoot?.progress?.(
    "Reading your saved flight and installed chapters…",
  );
  ...
  async function inspectChapters({ signal } = {}) {
    ...
    if (values.some((value) => value !== null))
      throw new Error(
        "Safe chapter recovery requires Web Locks. Stored data is preserved.",
      );
  ...
  function contentFromChapters(snapshot) {
    return {
      ...prepareContentCatalog(snapshot.packs),
  ...
  async function writeCheckedPacks(value) {
    ...
    const review = await externalChapters.prepareMutation(
      snapshot,
      value ?? emptyPackLibrary(),
    );
  ...
  async function writeLegacyBackupPacks(value) {
    ...
    if (
      (await readAssetStore(`${libraryKey}.external-chapter-index.v1`)) !==
        null ||
      (await readAssetStore(`${libraryKey}.external-chapter-journal.v1`)) !==
        null
    )
      throw new Error(
  ...
  const backupAdapters = () => ({
    ...
    commitProfile: (next, options) => {
      ...
      const current = loadLibrary(localStorage, libraryKey, {
        campaigns: [campaign],
      });
  ...
  if (persistenceReady) {
    try {
      ...
      if (
        chapters.status === "recovery-required" &&
        chapters.reason !== "backup-recovery"
      )
        throw new Error(
  ...
  async function readSavedState() {
    ...
    return externalChapters
      ? externalChapters.withCurrent(chapters, read)
      : read();
  }
  try {
  ...
  if (loaded.warning || packWarning) {
    $("save-warning").textContent = [loaded.warning, packWarning]
      .filter(Boolean)
      .join(" ");
  ...
  const masteryAwards = createMasteryAwards({
    ...
    onStatus: (status) => {
      if (["earned", "session"].includes(status.status))
        libraryPanel.refreshMasteries();
  ...
  theme =
    themesFile.themes.find(
      (t) =>
        t.id === (rememberedSelection?.themeId || library.preferences.themeId),
    ) || theme;
  ...
  function renderMusicPreview() {
    ...
    $("music-preview").textContent = ["blocked", "error"].includes(
      musicPreviewState.status,
    )
  ...
  const renderMasterPreferences = ({ muted, volume }) => {
    ...
    $("settings-master-mute").textContent = muted
      ? "Unmute sound"
      : "Mute sound";
  ...
  getPictureManager();
  const pictureStore = practice
    ? null
    : createStillMediaStore({ managedStore: pictureManager });
  const storyStore = practice
    ? null
    : createStoryMediaStore({ managedStore: pictureManager });
  ...
  const preparationFeedback = createOperationStatus(
  ...
  function beginPreparation(
    ...
    const operation = {
      status: preparationFeedback.begin({ message, stage }),
      cancel,
      result,
    };
    ...
    return {
      update(status) {
        if (preparationOperation !== operation || status.status !== "preparing")
          return;
  ...
  $("flight-preparation-cancel").onclick = () => {
    ...
    if (!operation) return;
    const restoreFocus =
      document.activeElement === $("flight-preparation-cancel");
    if (operation.result) {
  ...
  victoryStoryButton.onclick = () => {
    ...
    cancelResultAttempt();
    const notify = flightInformation.captureWarning("host.story", {
      allowTerminal: true,
    });
    try {
      ...
      if (
        !pin ||
        !backdrop ||
        canonicalJSON(pin.picturePin) !== canonicalJSON(backdrop.pin)
      )
      ...
      void storyDialog
        .open({
          ...
          drawPoster(canvas) {
            ...
            new BoardPainter(presets).drawGallery(
  ...
  function pictureIdentity(metadata) {
    return createPictureIdentityCatalog({
  ...
  function newFlightPictures({
    ...
    if (candidateHost?.owns(entry)) {
      const manifest = entry.manifests.find(
        (item) => item.level.id === nextRun.levelId,
      );
    ...
    const authoredBackground =
      entry.levelVisuals?.find((item) => item.levelId === nextRun.level.id)
    ...
    return createFlightPictures({
      ...
      prepareSelection: !legacy
        ? (options) =>
            releasePictures.prepareSelection({
              ...options,
              authoredBackground:
                entry.levelVisuals?.find(
                  (row) => row.levelId === nextRun.level.id,
                )?.visualOverrides?.background ??
                entry.visualOverrides?.background ??
                null,
            })
      ...
      selectPins:
        !legacy &&
        chapterSnapshot?.index?.chapters.some(
          (d) => d.id === entry.sourcePackId,
        )
          ? async ({ media, selection, explicitLegacy, signal }) => {
              ...
              const fallback =
                explicitLegacy ||
                current.choices.some((choice) => choice.kind === "legacy");
  ...
  function pictureFailure(error, notify) {
    ...
    pictureRecovery = needsWriter
      ? { owner: flightPictures, run, themeId: theme.id }
      : null;
    ...
    if (pictureResume === null && !preparationOperation && !pictureThemePending)
      preparationFeedback
        .begin({ message })
        .finish({ message, state: "error" });
    for (const button of pictureRecoveryButtons) button.hidden = !needsWriter;
    legacyPictureButton.hidden =
      !!candidateHost || needsWriter || started || practice;
    if (!started)
      $("start-button").textContent = needsWriter
        ? "Check picture →"
        : "Retry picture →";
  }
  function warmPicture() {
    ...
    void prewarm.promise
      .then(() => {
        if (picturePrewarm === prewarm && owner === flightPictures)
          refreshHUD();
      })
      .catch((error) => {
        if (picturePrewarm !== prewarm || owner !== flightPictures) return;
  ...
  function soundtrackContext() {
    ...
    const level =
      scenario?.level ||
      (activeEntry.baseCampaign || campaign).levels[levelIndex];
    if (
      $("shell-home").open ||
  ...
  async function activateAudio({ explicit = false } = {}) {
    ...
    if (explicit || !soundtrackPlayer.snapshot().track)
  ...
  function soundtrackStatus(message, preparation = null) {
    if (preparation) {
      if (!soundtrackOperation)
        soundtrackOperation = soundtrackFeedback.begin(preparation);
      else soundtrackOperation.update(preparation);
    } else {
      (soundtrackOperation ?? soundtrackFeedback.begin({ message })).finish({
        message,
      });
  ...
  async function initializeSoundtrack() {
    try {
      ...
      const source = createSoundtrackSource({
        ...
        installedOnly: () =>
          soundtrackLibrary?.listening?.installedOnly ?? false,
      });
      ...
      soundtrackPanel = attachSoundtrackPanel({
        ...
        onLibrary: (_library, snapshot) => {
          ...
          soundtrackAssets = new Map(
            snapshot.assets.map(({ sha256, blob }) => [sha256, blob]),
          );
        },
      ...
      try {
        const snapshot = await soundtrackStore.read({
          signal: soundtrackLoad.signal,
        });
        soundtrackLoading = false;
        if (
          !soundtrackDisposed &&
          snapshot.generation >= soundtrackGeneration
        ) {
          soundtrackGeneration = snapshot.generation;
          soundtrackAssets = new Map(
            snapshot.assets.map(({ sha256, blob }) => [sha256, blob]),
  ...
  const presentationFeedback = cosmeticFeedback(
  ...
  try {
    ...
    presentationReady = presentationHost
      ...
      .catch((error) => {
        ...
        compiledPresentationWarning =
          "Release artwork is unavailable; source artwork is shown.";
        painter.onAsset("");
        return null;
      });
  } catch {
  ...
  function applyFlightPresentation() {
    ...
    if (snapshot)
      (flightVisualLease ?? presentationHost).apply(document.documentElement);
  ...
  function prepareFreshAttemptVisuals(entry, level, themeId, options) {
    if (
      ...
      !freshSoloVisualSelection(entry, themeId)
    )
  ...
  const releasePictures = createReleasePictureDefaults({
    ...
    async commit(store, prepared, options) {
      ...
      return externalChapters
        ? externalChapters.withCurrent(snapshot, write, {
  ...
  $("match-class-appearance").checked =
  ...
  let controllerLabels = controllerBindingLabels(
  ...
  function adoptControllerBoostMode({ force = false } = {}) {
    if (
      force ||
      controller.boostState().mode !== library.preferences.controllerBoostMode
    ) {
  ...
  function controllerScope() {
    ...
    if (run?.status === "won")
      return $("show-result").hidden ? "won" : "picture";
  ...
  function controllerFocus() {
    ...
    if (dialog) {
      ...
      return [...dialog.querySelectorAll("button,select,summary")].find(
        availableFocusTarget,
      );
    }
    const scope = controllerScope();
    if (scope === "celebration" || scope === "defeat-presentation")
    ...
    if (courseSession && scope === "won")
      return !$("first-flight-next").hidden
        ? $("first-flight-next")
        : $("retry-button");
    if (scope === "won" || scope.startsWith("overview:"))
      return $("next-button");
  ...
  function controllerMenuRegion() {
    ...
    if (!$("game-overlay").hidden)
      return courseSession
        ? $("game-overlay").closest(".arena-panel")
        : $("game-overlay");
    if (
      defeatActive ||
      celebrationActive ||
  ...
  function controllerMenuRoot() {
    ...
    controllerCompositeRegion =
      region !== document && !controllerDialog() && !courseBlocked()
        ? region
        : null;
  ...
  function controllerBack() {
    ...
    if (dialog) {
      ...
      if (dialog.id === "library-dialog" && libraryPanel.cancelAttemptExport())
        return;
    ...
    if (
      pictureResume !== null &&
      preparationOperation?.cancel === cancelPictureStart
    ) {
  ...
  function refreshInputPresentation() {
    ...
    if (document.body.dataset.arenaChrome !== chrome)
      document.body.dataset.arenaChrome = chrome;
    ...
    if (document.body.dataset.screenControls !== next)
      document.body.dataset.screenControls = next;
  }
  controllerNavigation = attachControllerNavigation({
    ...
    onNativeInput: (event) => {
      setInputModality(
        nextInputModality(document.body.dataset.inputMode, event),
      );
    ...
    getContext: () => {
      const capabilities = arcadeActionCapabilities(run.level),
        ...
        labels = bindingLabels(
          resolveKeyBindings(library.preferences.keyboardBindings),
        ),
      ...
      for (const enemy of run.enemies)
        roles.set(enemy.type, (roles.get(enemy.type) || 0) + 1);
      return {
  ...
  enemyGuide = attachEnemyGuide({
    ...
    loadImpactScenario: () =>
      getJSON("content/scenarios/line-impact-demo.json"),
    onPractice: () => {
      clearInput();
      guideMusicWasPlaying =
        soundtrackPlayer?.snapshot().desired ?? sound.enabled;
      suspendAudio();
    },
    onReturn: () => {
      clearInput();
  ...
  handlePageHide = (event) => {
    ...
    if (!event.persisted) {
      ...
      if (browsingJourneyPreferences !== journeyPreferences)
  ...
  function refreshKeyPrompts() {
    ...
    $("game-canvas").setAttribute(
      "aria-label",
      `Territory capture game. ${description}`,
    );
    for (const [id, action] of [
      ...
      $(id).querySelector("kbd").textContent = keyLabel(
        bindings.bindings[action][0],
      )
        .replace("Left ", "L ")
        .replace("Right ", "R ");
  ...
  function courseBlocked() {
    return (
      !!courseEntry ||
      (courseSession && ["switching", "leaving", "ended"].includes(coursePhase))
    );
  }
  function refreshCourse() {
    ...
    if (
      courseSession &&
      !["switching", "leaving", "ended"].includes(coursePhase)
    )
      coursePhase = !started
        ? "ready"
        : ["won", "lost"].includes(run?.status)
          ...
          : snapshot?.outcome === "missed"
            ...
            : snapshot?.outcome === "complete"
              ...
              : snapshot?.available === false
                ...
                : lesson.steps.find((item) => item.id === snapshot?.stepId)
    ...
    if ($("first-flight-task").textContent !== text)
      $("first-flight-task").textContent = text;
  }
  ...
  function stopCourseGuidance() {
    courseUnavailable = {
      ...
      available: false,
      error:
        "Guidance is unavailable for this attempt. You can still play, retry, skip or leave.",
    };
    courseObserver = null;
  }
  function cancelCourseEntry(
    message = "Course entry cancelled. Your flight remains paused.",
  ) {
  ...
  async function enterFirstFlight() {
    ...
    courseEntryMessage =
      "Preparing First Flight. Your current flight stays paused.";
    refreshCourse();
    ...
    const assertCurrent = () => {
      if (
        ...
        throw new DOMException(
          "Course entry was cancelled by a newer change.",
          "AbortError",
        );
    };
    try {
      if (started && ["running", "respawning"].includes(run.status)) {
        await retainFlightForFirstFlight({
          ...
          presentationLevel: campaign.levels.find(
            (level) => level.id === run.levelId,
          ),
          ...(flightVisualLease
            ? { visualThemePin: flightVisualLease.pin() }
            : {}),
          ...
          assertWritable: async () => {
            ...
            if (
              ...
              throw new Error(
                "Stored data needs recovery before this flight can be retained.",
              );
          },
          withStorageLock: (work) => {
            if (!navigator.locks?.request)
              throw new Error(
                "This browser cannot retain the flight safely before navigation.",
              );
  ...
  const librarySourceReturn = readMissionLibraryReturn(params, {
    mode: "solo",
  });
  ...
  const modeDestinations = Object.freeze({
    ...
    ...(librarySourceReturn
      ? { [librarySourceReturn.mode]: librarySourceDestination }
      : {}),
  ...
  function prepareModeHint(ticket) {
    ...
    if (librarySourceReturn?.mode === ticket.kind)
      return {
        token: null,
        href: new URL(modeDestination(ticket), location.href).href,
      };
  ...
  function clearModeHint(ticket) {
    if (ticket.token)
      ((ticket.kind === "library" ? ticket.libraryMode : ticket.kind) ===
      "versus"
        ? modeReturnV2
        : modeReturn
      ).clear(ticket.token);
  ...
  function modeDepartureCurrent(ticket) {
    if (
      ...
      (ticket.libraryTarget &&
        unifiedLibrary?.library.find(ticket.libraryTarget.id) !==
          ticket.libraryTarget) ||
      run !== ticket.run ||
      recorder !== ticket.recorder ||
      runId !== ticket.runId ||
  ...
  async function retainNavigationFlight(ticket, assertCurrent, onProgress) {
    ...
    if (!attemptReadbackMatches(raw, retained.session))
      throw new Error(
        "The checked saved flight changed before departure was ready.",
      );
    ticket.savedRaw = raw;
  ...
  async function requestModeDeparture(
    ...
    if (
      ...
      warning(
        `Finish the current operation before choosing ${modeLabel(kind)}.`,
      );
      return false;
    }
    const ticket = {
      ...
      libraryHref:
        kind === "library"
          ? missionLibraryHref({
              ...
              mode: libraryMode,
              journey:
                libraryTarget.collection === "Journey"
                  ? libraryTarget.editionId
                  : "legacy",
    ...
    if (origin === "solo-title" && !ticket.unfinished) {
      try {
        ...
        warning(
          `${modeLabel(kind)} could not open. Your flight remains here. ${error.message}`,
        );
      }
      return;
    }
    if (!ticket.unfinished) {
      ...
      try {
        ...
        if (modeDeparture !== ticket || ticket.controller.signal.aborted)
          return;
    ...
    try {
      ...
      warning(
        `The departure could not open. Your flight remains here. ${error.message}`,
      );
      return;
    }
    try {
    ...
    if (ticket.failure)
      $("mode-leave-status").textContent +=
        ` Saving was not verified: ${ticket.failure}`;
  }
  $("shell-team").onclick = (event) =>
    requestModeDeparture("team", event, $("shell-team"));
  $("shell-versus").onclick = (event) =>
    requestModeDeparture("versus", event, $("shell-versus"));
  ...
  function missionReplacementCurrent(ticket, { installed = false } = {}) {
    if (
      ...
      throw new Error(
        "The flight or available missions changed. Stay here and choose again.",
      );
  }
  const libraryLaunchKinds = new Set([
    "library-installed",
    "library-challenge",
    "library-picture",
  ]);
  ...
  function requestLibraryLaunch(descriptor, launch) {
    ...
    if (request.kind === "library-picture") {
      if (
        typeof descriptor.recordIdentity !== "string" ||
        descriptor.recordIdentity.length > 8192
      )
        return Promise.reject(
          new Error("This earned picture identity is unavailable."),
        );
      request.recordIdentity = descriptor.recordIdentity;
    }
    if (!launch.isCurrent() || !availableFocusTarget(launch.opener))
  ...
  function captureWorldPlay({ launch, signal }) {
    if (
      ...
      throw new Error(
        "Return to the normal game and resolve recovery before playing a chapter.",
      );
  ...
  async function requestWorldPlay(
    ...
    const entry =
      pack === null
        ? baseEntry
        : resolvePackCampaign(pack, campaignId ?? pack.campaigns[0].id);
    const request = {
      kind: "world-play",
    ...
    const selected = await requestMissionReplacement(
      request,
      launch.opener,
      launch,
    );
  ...
  function worldAttemptCurrent(ticket) {
    if (worldAttempt !== ticket || ticket.controller.signal.aborted)
      return false;
    try {
      if (assertWorldPlay(ticket.request.launch) !== ticket.intent)
        return false;
  ...
  function assertWorldArtworkOwner(entry) {
    ...
    const presentation = (candidate, themeId) => ({
      ...
      backgrounds: candidate.campaign.levels.map((level) => ({
        id: level.id,
        background:
          candidate.levelVisuals?.find((item) => item.levelId === level.id)
            ?.visualOverrides?.background ??
          candidate.visualOverrides?.background ??
          null,
      })),
      external:
        chapterSnapshot?.index?.chapters.find(
          (item) => item.id === candidate.sourcePackId,
        )?.id ?? null,
    });
    for (const other of installedEntries) {
      if (
        other.sourcePackId === entry.sourcePackId ||
        campaignKey(other.campaign) !== key
      )
        continue;
  ...
  async function prepareWorldAttempt(request, replacement = null) {
    ...
    if (!entry)
      throw new Error("This chapter does not support the selected difficulty.");
    const selection = difficultyNavigation.selection(
      entry,
      library.campaigns,
      progressFor(library, entry.campaign),
    );
    const destinationIndex = installedMissionExecutionIndex(
      target,
      entry,
    ...
    const signals = [request.signal, replacement?.controller.signal].filter(
      Boolean,
    );
  ...
  function resolveMissionRequest(request) {
    ...
    if (request.kind === "library-installed") {
      const entry = installedEntries.find(
        (item) =>
          item.sourcePackId && campaignKey(item.campaign) === request.id,
      );
      if (!entry)
        throw new Error(
          "That exact installed campaign is no longer available.",
        );
      return {
        same:
          (entry.baseCampaignKey || campaignKey(entry.campaign)) ===
          modeSelection().campaignKey,
    ...
    if (request.kind === "library-challenge") {
      ...
      const next = challengeCampaign(
        match[1],
        match[2],
        baseEntry.classRecipes,
      );
      return {
    ...
    if (request.kind === "library-picture") {
      ...
      if (!item || canonicalJSON(item) !== request.recordIdentity)
        throw new Error(
          "That earned picture changed. Reopen it before replaying.",
        );
      const picture =
        createGalleryDifficultyResolver(executionEntries()).picture(item);
      if (!picture)
        throw new Error(
          "Reinstall this picture’s exact campaign before replaying.",
        );
      const options = {
    ...
    if (request.kind === "lesson") {
      ...
      return {
        same: courseRequest.lessonId === lesson.id,
        title: lesson.title,
      };
    }
    if (request.kind === "class") {
      ...
      if (courseSession || !recipe)
        throw new Error("That starting class is unavailable.");
      return {
        same: classId === request.id,
        title: `Starting class: ${recipe.label}`,
      };
    }
    ...
    if (request.kind === "level" || request.kind === "card") {
      const index = campaign.levels.findIndex(
        (level) => level.id === request.id,
      );
    ...
    if (request.kind === "campaign") {
      const entry = catalog().find(
        (item) => campaignKey(item.campaign) === request.id,
      );
  ...
  async function applyMissionRequest(request, ticket = null) {
    const target = resolveMissionRequest(request);
    if (request.kind === "world-play")
      return prepareWorldAttempt(request, ticket);
    if (isLibraryRequest(request)) {
      ...
      contentStatus(
        `${campaign.levels[levelIndex].name} selected. Deploy when ready.`,
      );
      return true;
    }
    if (request.kind === "lesson") {
    ...
    if (request.kind === "campaign") {
      ...
      leavePractice();
      levelIndex = campaign.levels.findIndex(
        (level) => level.id === request.id,
      );
      prepare();
      rememberSelection();
  ...
  function missionReplacementMessage(ticket) {
    ...
    if (ticket.failure)
      $("mission-replace-status").textContent += ` ${ticket.failure}`;
  }
  async function requestMissionReplacement(request, opener, launch = null) {
    ...
    try {
      ...
      if (missionReplacement !== ticket || ticket.controller.signal.aborted)
        return false;
      ticket.savedRaw = null;
      ticket.failure = error.message;
    }
  ...
  $("mission-replace-dialog").addEventListener("close", () => {
    ...
    cancelMissionReplacement();
    if (
      ticket?.launch?.onCancelled?.({ dialog: $("mission-replace-dialog") }) ===
      true
    )
      return;
    if (
      (!ticket?.launch || ticket.launch.isCurrent()) &&
      availableFocusTarget(ticket?.opener)
    )
      ticket.opener.focus({ preventScroll: true });
  });
  $("mission-replace-confirm").onclick = async () => {
    ...
    try {
      ...
      if (!selected) {
        if (availableFocusTarget(ticket.opener))
          ticket.opener.focus({ preventScroll: true });
        return;
      }
      ...
      } else if (ticket.request.kind === "lesson")
        $("start-button").focus({ preventScroll: true });
      else if (availableFocusTarget(ticket.opener))
        ticket.opener.focus({ preventScroll: true });
    } catch (error) {
      if (missionReplacement !== ticket) return;
      ticket.pending = false;
  ...
  function selectCourseLesson(lessonId) {
    ...
    scenario = createLessonScenario(lessonId, {
      turnPolicy,
      theme: courseTheme,
    });
    classId = scenario.settings.classId;
  ...
  function nextCourseLesson(skip = false) {
    ...
    const index = FIRST_FLIGHT_LESSONS.findIndex(
      (item) => item.id === courseRequest.lessonId,
    );
  ...
  function catalog() {
    ...
    if (
      !entries.some(
        (e) => campaignKey(e.campaign) === campaignKey(selected.campaign),
      )
    )
    ...
    return entries.filter(
      (e, i, all) =>
        all.findIndex(
          (x) => campaignKey(x.campaign) === campaignKey(e.campaign),
        ) === i,
    );
  }
  ...
  function currentSelection(options = {}) {
    if (candidateHost?.owns(activeEntry)) {
      ...
      const index = campaign.levels.findIndex(
        (level) => level.id === options.levelId,
      );
      return {
    ...
    return difficultyNavigation.selection(
      activeEntry,
      library.campaigns,
      progress,
      options,
  ...
  function currentAppearanceMilestones() {
    return difficultyNavigation.milestones(
      activeEntry,
      library.campaigns,
      progress,
    );
  ...
  function executionForEntry(entry, mode) {
    ...
    if (!entry.sourcePackId)
      return executionCatalog.select(baseKey, mode) || entry;
    ...
    if (!authored)
      throw new Error(
        "This exact chapter is no longer installed. Choose an available chapter.",
      );
  ...
  function refreshDifficulty() {
    const nextPressure = gameplayTuning.snapshot(
      browsingJourneyPreferences.snapshot().difficulty,
    );
    $("menu-difficulty").value = nextPressure.difficulty;
    $("menu-difficulty").disabled =
      contentSwitchBusy || backupBusy || sessionBusy;
    $("menu-difficulty-note").textContent =
      ...
      (nextPressure.adminOverride
        ? "Admin playtest · no normal clears or awards. "
        : "") +
      (browsingJourneyPreferences.snapshot().error ||
        gameplayTuning.status().error ||
        "");
    ...
    const standard = executionCatalog.select(
        activeEntry.baseCampaignKey,
        "standard",
      ),
  ...
  function refreshCampaigns() {
    $("campaign-select").replaceChildren(
      ...catalog().map(
        (e) =>
          new Option(
            e.campaign.title || e.campaign.name || e.campaign.id,
  ...
  function contentStatus(
    message,
    error = false,
    { busy = false, stage = "preparing" } = {},
  ) {
    for (const feedback of contentFeedback) {
      const options = { message, stage };
      if (busy && feedback.target.dataset.state === "busy")
        feedback.lease.update(options);
      else {
        feedback.lease = feedback.presenter.begin(options);
        if (!busy)
          feedback.lease.finish({ message, state: error ? "error" : "ready" });
      }
  ...
  function invalidateContentSwitch({ announce = false } = {}) {
    ...
    if (announce && interrupted && !backupBusy)
      contentStatus(
        "Pending pack launch cancelled; your newer play choice is kept.",
      );
    else if (interrupted)
      for (const feedback of contentFeedback)
        if (feedback.target.dataset.state === "busy")
          feedback.presenter.clear();
    return interrupted;
  }
  function refreshContentSelectors() {
    const installedIds = new Set(packs.packs.map((pack) => pack.id));
    const options = [
      new Option(`Base game · ${baseCampaign.levels.length} levels`, ""),
    ];
    for (const summary of packCatalog.packs) {
      const levels = summary.campaigns.reduce(
    ...
    for (const pack of packs.packs) {
      ...
      const levels = pack.campaigns.reduce(
        (total, item) => total + item.levels.length,
        0,
      );
      options.push(
  ...
  function persistProfile({ mode = "merge" } = {}) {
    if (courseSession || courseEntry)
      return {
        ok: false,
        warning: "Training does not save player-library changes.",
      };
    let saved;
    try {
      saved =
      ...
      saved = {
        ok: false,
        warning: "Storage is unavailable. Export your player library.",
      };
    }
  ...
  function preferences(patch) {
    if (courseEntry)
      return {
        ok: false,
        warning:
          "Finish or cancel the course handoff before changing settings.",
      };
  ...
  async function launchJourneyMission(
    ...
    clearInput();
    if (candidateHost) {
      const entry = candidateHost.select(
        mission,
        journeyPreferences.snapshot().difficulty,
      );
      if (!entry) return false;
      const adopted = await prepareResultAttempt(
        kind,
        mission.levelIndex,
        entry,
      );
    ...
    try {
      owner.cancel = ({ restoreFocus = false } = {}) => {
        ...
        owner.feedback?.finish(
          "Preparation cancelled. Your current flight is kept.",
          "cancelled",
        );
      ...
      if (mission.packId)
        await ensureBundledPack(mission.packId, operation, {
      ...
      const authored = installedEntries.find(
        (entry) =>
          ...
          entry.campaign.id === mission.campaignId,
      );
      if (!authored)
        throw new Error("This mission is unavailable in the current edition.");
      ...
      const entry = executionForEntry(
        authored,
        library.preferences.campaignDifficulty,
      );
      const index = entry.campaign.levels.findIndex(
        (level) => level.id === mission.levelId,
      );
      if (index < 0)
        throw new Error("This mission is unavailable in the current edition.");
      contentSwitchBusy = false;
      ...
      if (
        ...
        warning(
          `Could not open the next mission. Your current flight is kept. ${error.message}`,
        );
      }
      return false;
    } finally {
  ...
  function selectEntry(
    ...
    entry = executionForEntry(
      entry,
      difficulty ??
        (practiceSession ? "standard" : library.preferences.campaignDifficulty),
    );
    activeEntry = entry;
    campaign = entry.campaign;
    ...
    if (!classRegistry.some((c) => c.id === classId))
      classId = classRegistry[0].id;
    theme =
      entry.themes.find((t) => t.id === (themeId || campaign.themeId)) ||
      entry.themes[0];
    bodyId = theme.player;
    $("theme-select").replaceChildren(
      ...entry.themes.map((t) => new Option(t.name, t.id)),
    );
    $("theme-select").value = theme.id;
    $("class-select").replaceChildren(
      ...classRegistry.map((c) => new Option(c.label, c.id)),
    );
    const track =
      entry.music?.find((m) => m.id === campaign.musicId) || entry.music?.[0];
    if (track) {
      assignMusic(track);
      $("music-select").value = track.genre;
    } else
      assignMusic(
        DEFAULT_TRACKS.find((t) => t.genre === library.preferences.musicGenre),
      );
    refreshCampaigns();
  ...
  async function replacePackLibrary(
    ...
    if (courseEntry)
      throw new Error("Cancel the course handoff before changing packs.");
    attemptFiles?.invalidate();
    ...
    try {
      ...
      if (
        ...
        before.packs.some(
          (old) =>
            JSON.stringify(next.packs.find((item) => item.id === old.id)) !==
            JSON.stringify(old),
        )
      )
        throw new Error(
          "Installing a world must preserve every existing pack.",
        );
      ...
      try {
        ...
        throw new Error(
          `Pack storage failed; previous installed packs are kept. ${error.message}`,
        );
      }
      ...
      if (
        ...
        const pack = packs.packs.find(
          (item) => item.id === activeEntry.sourcePackId,
        );
  ...
  async function ensureBundledPack(
    ...
    if (!summary)
      throw new Error("This pack is not bundled with the current build.");
    assertWriter();
    ...
    const prepared = await packLaunchGuard.run(
      ...
      async () => {
        try {
          contentStatus(
            `Checking ${summary.name} and preparing its artwork…`,
            false,
            {
              busy: true,
              stage: "verifying",
    ...
    packLaunchGuard.assert(operation, packs);
    contentStatus(`Saving ${summary.name} on this device…`, false, {
      busy: true,
      stage: "saving",
    });
  ...
  async function installSourceChapter(
    ...
    const notify = flightInformation.captureWarning("host.chapter", {
      allowTerminal: true,
    });
    ...
    try {
      ...
      const prepared = download
        ...
        : await prepareSourceExternalChapter(files, {
      ...
      if (
        ...
        snapshot.reason !== "external-recovery"
      )
        throw new Error(
  ...
  async function activatePack(
    ...
    let notify = flightInformation.captureWarning("host.chapter", {
      allowTerminal: true,
    });
    attemptFiles?.invalidate();
    cancelRestore();
    ...
    try {
      ...
      const entry = installedEntries.find(
        (item) =>
          item.sourcePackId === result.pack.id &&
          item.campaign.id === source.id,
      );
      if (!entry)
        throw new Error("The installed pack campaign could not be selected.");
      beforeSelect?.();
      if (levelId) {
        const targetIndex = entry.campaign.levels.findIndex(
        ...
        if (!missionAvailable(targetIndex, entry)) {
          if (!preserveCurrentRun) {
            ...
            notify = flightInformation.captureWarning("host.chapter", {
              allowTerminal: true,
            });
          }
          throw new Error(
  ...
  function selectLevel(levelId) {
    ...
    if (index < 0 || !missionAvailable(index)) {
      refreshContentSelectors();
      contentStatus(
        "That level is still locked. Complete the earlier missions first.",
        true,
      );
      return false;
    }
  ...
  function refreshSavedFlight() {
    ...
    const savedMode = difficultyLabel(
      executionEntries().find(
        (entry) => campaignKey(entry.campaign) === saved?.campaignKey,
      ),
    );
  ...
  function assertWriter() {
    if (courseSession)
      throw new Error("Training does not write campaign data.");
    if (!persistenceReady || !writer.writable)
      throw new Error(
        writer.reason || "Storage recovery must finish before saving.",
      );
    if (localStorage.getItem(`${libraryKey}.backup-lock`) !== null)
      throw new Error(
        "A backup is being restored. Saving resumes when it finishes.",
      );
  }
  ...
  function snapshotCurrentBackup(savedAt = new Date().toISOString()) {
    return externalBackup.snapshot(() => {
      if (contentSwitchBusy || sessionBusy || backupBusy)
        throw new Error(
          "Finish the pending content or save operation before exporting.",
        );
  ...
  function persistAttempt(notify = true) {
    if (titleFlightHold)
      throw new Error(
        "Explicitly Resume the verified flight before saving again.",
      );
    if (modeDepartureHold)
      throw new Error(
        "Return to the flight and explicitly Resume before saving again.",
      );
    if (courseEntryHold)
      throw new Error(
    ...
    try {
      ...
      saved = {
        ok: false,
        warning: "Storage is unavailable. Export this attempt.",
      };
    }
    if (saved.ok) {
      try {
  ...
  async function restoreAttempt(
    candidate,
    { beforeAdopt, onAdopted, onStatus, signal } = {},
  ) {
    if (courseSession || courseEntry)
    ...
    if (signal?.aborted)
      throw new DOMException("Title launch cancelled.", "AbortError");
    ...
    try {
      ...
      const restored = await restoreSession(candidate, {
        ...
        mediaIdentityCatalog: candidate?.presentationPins
          ? pictureIdentity()
          : undefined,
        masteryDefinition:
          masteryFor(
      ...
      seed = run.seed;
      bodyId = presets.characters[restored.session.bodyId]
        ? restored.session.bodyId
        : theme.player;
      started = true;
      paused = true;
  ...
  function cancelTitleFlight() {
    ...
    ticket.visuals?.release();
    ticket.visuals = null;
    if (ticket.prewarm?.observe === ticket.observe)
      ticket.prewarm.observe = null;
    ...
    try {
      ...
      const assertCurrent = () => {
        if (!titleFlightCurrent(ticket))
          throw new DOMException(
            "Title launch changed. Your flight stays paused.",
            "AbortError",
          );
      };
      assertCurrent();
      ...
      ticket.observe = (status) => {
        if (titleFlightCurrent(ticket) && status.status === "preparing")
          feedback.update(status);
      };
      ...
      if (!continuing && kind === "continue") {
        if (!ticket.savedRaw)
          throw new Error(
            "The saved flight is unavailable. Open Library & saves to review it.",
          );
        let candidate;
        try {
        ...
        if (!owner)
          throw new Error(
            "The mission picture is unavailable. Choose the mission again.",
          );
        ticket.prewarm =
        ...
        try {
          await Promise.race([
            ...
            new Promise((resolve, reject) => {
              abort = () =>
                reject(
                  new DOMException("Title launch cancelled.", "AbortError"),
                );
              signal.addEventListener("abort", abort, { once: true });
      ...
      if (
        !titleFlightCurrent(ticket, expected) ||
  ...
  function adoptPreferences() {
    ...
    classId = classRegistry.some((c) => c.id === p.classId)
      ? p.classId
      : classRegistry[0].id;
    ...
    applyDisplayPreferences(displayPreferences.snapshot());
    $("tap-steering").checked =
      p.tapSteering ?? matchMedia("(pointer: coarse)").matches;
    syncAssistControls();
    keySettings.refresh();
  ...
  const libraryPanel = attachLibraryPanel({
    ...
    backupSet: practice
      ...
      : {
          ...
          gameIdentity: () => {
            ...
            if (started && !paused && !["won", "lost"].includes(run.status))
              throw new Error(
                "Pause the flight before preparing a backup set.",
              );
          ...
          readMetadata: async (options) =>
            (await pictureManager.usage(options)).generations,
    ...
    profileTransfer: isRelease
      ? {
          ...
          ...(externalBackup
            ? { readExternalSnapshot: externalBackup.readExternalSnapshot }
            : {}),
        }
      : null,
    get: () => ({
    ...
    applyBackup: async (prepared) => {
      ...
      try {
        ...
        let safeOriginal =
          persistenceReady && storedStateAdopted && writer.writable;
        try {
          safeOriginal =
            safeOriginal &&
            localStorage.getItem(`${libraryKey}.backup-lock`) === null;
  ...
  $("level-select").onchange = () =>
    requestMissionReplacement(
      { kind: "level", id: $("level-select").value },
      $("level-select"),
    );
  ...
  $("menu-difficulty").onchange = () => {
    ...
    if (!candidateHost?.owns(activeEntry))
      preferences({
        campaignDifficulty: difficulty === "gentle" ? "gentle" : "standard",
      });
    cancelResultAttempt();
    cancelWorldAttempt();
    cancelTitleFlight();
  ...
  $("switch-class-select").onchange = () => {
    $("switch-description").textContent =
      run.classRecipes.find((c) => c.id === $("switch-class-select").value)
        ?.description || "";
  };
  ...
  profileRecovery = attachProfileRecoveryDialog({
    currentVersion: buildVersion,
    packaged: isRelease,
    resolveSourceVersion: async () =>
      (await getJSON("build-config.json")).version,
    onOpen: () => clearInput(),
    unavailable: () => {
  ...
  $("music-preview").onclick = async () => {
    const notify = flightInformation.captureWarning("host.music", {
      allowTerminal: true,
    });
  ...
  $("touch-side").onchange = () => {
    ...
    status.textContent =
      ...
      (saved.ok
        ? "Steering hand saved."
        : resolveTouchControls(library.preferences.touchControls).side ===
            requested
  ...
  function refreshScreenSteeringHand() {
    ...
    const strip = document.querySelector(".play-controls"),
      ...
      ordered =
        hand === "right" ? [actions, ...steering] : [...steering, actions];
    if (ordered.some((group, index) => strip.children[index] !== group)) {
      ...
      if (ordered.some((group) => group.contains(focused)))
  ...
  $("text-size").onchange = () =>
    changeDisplay({ textSize: $("text-size").value });
  ...
  $("text-face").onchange = () =>
    changeDisplay({ textFace: $("text-face").value });
  function applyDisplayPreferences(state) {
    ...
    document.body.dataset.effects = state.effectiveReducedEffects
      ? "reduced"
      : "full";
  ...
  $("settings-grid").onchange = () =>
    preferences({ showGrid: $("settings-grid").checked });
  function syncAssistControls() {
    ...
    $("touch-instruction").textContent =
      touch.mode === "swipe" ? "Swipe to turn" : "Drag to steer";
  }
  ...
  function visuals() {
    return (
      scenario?.visualOverrides || {
        ...
        ...activeEntry.levelVisuals?.find(
          (v) => v.levelId === campaign.levels[levelIndex]?.id,
        )?.visualOverrides,
      }
    );
  }
  ...
  function updateBodies() {
    ...
    if (!allowed.has(bodyId) && !practice)
      bodyId =
        allowed.has(theme.player) &&
        Object.hasOwn(presets.characters, theme.player)
          ? theme.player
          : "neutral-marker";
  ...
  const bodyLabels = (ids) =>
    ids.map((id) => presets.characters[id]?.label || id);
  function paintAppearanceRewards(entry) {
    ...
    for (const tier of difficultyNavigation.milestones(
      ...
      if (body?.src)
        thumbnail.src = new URL(
          `../authoring/motion-lab/${body.src}`,
          import.meta.url,
        ).href;
  ...
  function focusAppearance() {
    ...
    if (!practiceSession) {
      void openUnifiedMissions(document.activeElement, {
        focusSetup: "body-select",
      });
      return;
    }
  ...
  function leavePractice() {
    ...
    campaignOverview = false;
    theme =
      themesFile.themes.find((t) => t.id === theme.id) || themesFile.themes[0];
    if (!classRegistry.some((c) => c.id === classId))
      classId = classRegistry[0].id;
    ...
    $("theme-select").replaceChildren(
      ...themesFile.themes.map((t) => new Option(t.name, t.id)),
    );
    $("theme-select").value = theme.id;
    setTheme();
  }
  function paintMissions() {
    ...
    campaign.levels.forEach((level, index) => {
      ...
      const earnedPicture =
        ...
        !!difficultyNavigation.access(activeEntry, library.campaigns)?.levels[
          index
        ]?.completed;
      ...
      b.setAttribute(
        "aria-label",
        `${index + 1}. ${level.name}${b.disabled ? " — locked" : ""}`,
      );
      ...
      const reward = { 1: "bronze", 2: "silver", 3: "gold" }[
        progress.clears[level.id]?.medals
      ];
      ...
      b.onclick = (event) => {
        const pending = requestMissionReplacement(
          { kind: "card", id: level.id },
          b,
        );
  ...
  function updateLoadout() {
    const recipe =
      run?.classRecipe ||
      (scenario?.classRecipes || classRegistry).find((c) => c.id === classId);
    $("class-description").textContent = recipe.description;
    const actions = arcadeActionCapabilities(run?.level);
    $("action-button").firstChild.textContent =
      `${recipe.id === "scout" ? "Scan" : recipe.label} `;
  ...
  function focusMission() {
    ...
    $("arena-shell").scrollIntoView({
      block: "center",
      inline: "nearest",
      behavior: "instant",
    });
    (campaignOverview ? $("next-button") : $("start-button")).focus({
      preventScroll: true,
    });
  }
  ...
  function overlay(kind, { preserveFocus = false } = {}) {
    ...
    drawResultPicture($("result-picture"), {
      kind,
      run,
      theme,
      seed,
    ...
    if (kind === "campaign-complete") {
      ...
      $("overlay-eyebrow").textContent = allCleared
        ? "CAMPAIGN COMPLETE"
        : "CAMPAIGN END";
      ...
      $("overlay-footnote").textContent =
        "Your earned pictures and best results are kept.";
    }
    if (kind === "pause") {
  ...
  function resultAttemptCurrent(ticket) {
    if (resultAttempt !== ticket || ticket.controller.signal.aborted)
      return false;
    ...
    try {
      ...
      const row = mission
        ...
        : retainedLibraryMission(host.library, {
            ...
            ...(retainedLibraryOwner?.entry === activeEntry
      ...
      if (host.library.availability(next, "solo").state !== "ready") {
        const ready = await host.library.prepare(next, {
          mode: "solo",
          signal: controller.signal,
        });
        if (!current()) return;
      ...
      const launched = await host.library.launch(next, {
        mode: "solo",
        ...context,
      });
  ...
  async function prepareResultAttempt(
    ...
    if (
      resultAttempt ||
      resultAttemptEpoch !== previousEpoch + (previous ? 1 : 0)
    )
      return;
    if (
      practice ||
    ...
    try {
      ...
      if (candidateHost?.owns(entry)) {
        ...
        if (!resultAttemptCurrent(ticket)) {
          if (candidateHost.preparer.current(candidateAttempt))
            candidateHost.preparer.cancel();
          throw new DOMException("Preparation cancelled.", "AbortError");
        }
      ...
      await ticket.pictures.ensure(nextTheme.id, {
        ...
        onStatus(status) {
          if (resultAttemptCurrent(ticket))
            ticket.feedback.update({
              ...status,
              message: `${level.name}: ${status.message}`,
            });
        },
      });
      if (!resultAttemptCurrent(ticket))
  ...
  function prepare({
    ...
    if (
      courseEntry ||
      (courseSession && ["leaving", "ended"].includes(coursePhase))
    )
      return;
    if (preparedAttempt) {
      const current = () =>
      ...
      if (contentSwitchTicket)
        packLaunchGuard.assert(contentSwitchTicket, packs);
      else invalidateContentSwitch({ announce: true });
      courseEntryHold = false;
      modeDepartureHold = false;
      titleFlightHold = false;
    ...
    if (preparedAttempt) {
      ...
      themesFile.themes = activeEntry.themes;
      $("theme-select").replaceChildren(
        ...activeEntry.themes.map((t) => new Option(t.name, t.id)),
      );
      $("class-select").replaceChildren(
        ...classRegistry.map((c) => new Option(c.label, c.id)),
      );
      ...
      if (preparedAttempt.kind === "world-play") {
        ...
        const track =
          activeEntry.music?.find((item) => item.id === campaign.musicId) ||
          activeEntry.music?.[0];
        assignMusic(
          track ||
            DEFAULT_TRACKS.find(
              (item) => item.genre === library.preferences.musicGenre,
            ),
        );
    ...
    const observedCampaign = explicitPractice
      ...
      : scenario
        ? catalog()
            ...
            .find((entry) => {
              if (!masteryFor(campaignKey(entry), run.levelId, masteryCatalog))
                return false;
              const map = entry.levels.find(
                (level) => level.id === run.levelId,
              );
              return (
                map &&
                canonicalJSON(normalizedLevel(map)) ===
                  canonicalJSON(run.level) &&
                canonicalJSON(entry.classRecipes) ===
                  canonicalJSON(run.classRecipes)
              );
            })
        : campaign;
    masteryDefinition =
    ...
    if (masteryDefinition)
      try {
        masteryObserver = createMasteryObserver({
          ...
          initial: captureMasteryFacts(run, {
            runId,
            definition: masteryDefinition,
          }),
        });
    ...
    if (!scenario && !themeOverride) {
      theme =
        themesFile.themes.find(
          (t) => t.id === (authoredLevel.themeId || campaign.themeId),
        ) || themesFile.themes[0];
      if (
        library.preferences.matchClassAppearance ||
  ...
  function resume({
    ...
    if (!flightPictures?.ready(theme.id) || needsFreshVisuals) {
      ...
      const notify = flightInformation.captureWarning("host.picture");
      const feedback = beginPreparation(
        picturePreparingMessage,
        cancelPictureStart,
      );
      const prewarm =
        picturePrewarm?.owner === owner &&
        picturePrewarm.themeId === selectedTheme
          ? picturePrewarm
          : null;
      if (prewarm) {
      ...
      void prepared
        ...
        .finally(() => {
          stagedVisuals?.release();
          if (pictureVisualController === controller)
            pictureVisualController = null;
    ...
    refreshCourse();
    refreshHUD();
    if (courseSession && alignCourseBoard)
      revealFirstFlightBoard($("arena-shell"));
  }
  ...
  function advanceDefeatPresentation(dt) {
    ...
    defeatRemaining = Math.max(
      0,
      defeatRemaining - Math.max(0, Math.min(0.1, dt)),
    );
  ...
  function pause(force, { preserveWorld = false } = {}) {
    ...
    if (defeatActive) {
      ...
      clearInput();
      warning(
        "Defeat presentation paused. Choose Show defeat menu when ready.",
      );
      return;
    }
  ...
  function refreshHUD() {
    ...
    $("shell-edition").textContent = courseSession
      ...
      : practice
        ...
        : !arcadeActionCapabilities(run?.level).manualAbility
          ? "ARCADE EDITION"
          : "TACTICAL EDITION";
    document.body.dataset.pictureState = flightPictures?.ready(theme.id)
      ? "ready"
      : "pending";
        ...
        : campaignOverview || ["won", "lost"].includes(run.status)
          ...
          : !started
            ...
            : paused
              ...
              : "running";
    $("coverage").innerHTML =
      `${(run.coverage * 100).toFixed(1)}<small>%</small>`;
    $("coverage").setAttribute(
      "aria-valuenow",
      (run.coverage * 100).toFixed(1),
    );
    $("coverage").setAttribute(
      "aria-valuetext",
              ...
              : paused
                ...
                : run.player.cutting
                  ...
                  : [
                    ...
                    ? "Reclaimed ground"
                    : "Safe ground";
    $("status-dot").style.background = run.player.cutting
      ? "var(--danger)"
      : "var(--safe)";
    ...
    $("restart-button").disabled =
      defeatActive || courseBlocked() || campaignOverview;
    ...
    $("encounter-status").dataset.phase =
      encounter?.phase ||
      (classic?.enemies.some((enemy) => enemy.mode === "warning")
        ? "warning"
        : "open");
    refreshMastery();
    refreshCourse();
  ...
  function eventFeedback(events) {
    ...
    const openedGates = events.filter(
      (event) => event.type === "relay.opened",
    ).length;
      ...
      for (const [index, event] of events.entries()) {
        flightInformation.observeEvent(ticket, index, () => {
          ...
          if (event.type === "class.switched") {
            ...
            setTheme();
            warning(
              `Now flying ${run.classRecipe.label}. Charges and cooldowns are preserved.`,
            );
          }
          if (event.type === "class.rejected")
            warning(
              `Cannot switch craft: ${event.reason}. Return to safe hangar ground.`,
            );
          if (event.type === "cells.claimed") {
            if (teachingCapture)
              warning(
                [captureTeaching(), captureTerrain].filter(Boolean).join(" "),
                "secured",
              );
            else
              warning(
            ...
            warning(
              {
                ...
                "mission-timeout":
                  "The mission clock ran out. Try a faster route.",
                "cut-timeout":
                  "Your live line stayed open too long. Make a shorter cut.",
                "cable-limit":
                  "Your cable budget ran out. Close a shorter line.",
                "lethal-terrain":
                  "A lethal field caught your craft. Enclose it before crossing.",
              }[event.cause] ||
          ...
          if (event.type === "shield.absorbed")
            warning(
              "Shield absorbed the hit. Your unfinished line is cancelled; no life lost.",
            );
          if (event.type === "ability.rejected")
            warning(
              {
                empty:
                  "No charges left. Return to a supply pad and press Supply.",
                cooldown:
                  "Ability is recharging. Watch the cooldown beside your controls.",
            ...
            warning(
              {
                ...
                "stun-field":
                  "Stun field placed. Nearby moving field enemies are briefly held.",
                "slow-field":
                  "Slow field placed. Nearby field enemies move at quarter speed.",
                shield:
                  "Shield active. One enemy contact can cancel your line safely.",
            ...
            warning(
              {
                ...
                "enemy-slow":
                  "Slow pickup: enemies move at half speed for six seconds.",
                "enemy-freeze":
          ...
          if (event.type === "rover.warning")
            warning(
              "Claimed-ground rover waking in one second. Watch the marked actor.",
            );
          if (event.type === "rover.activated")
            warning(
              "Claimed-ground rover active. Your secured ground still has a moving threat.",
            );
          if (event.type === "erosion.warning")
            warning(
              "The marked captured cell is about to reopen. Watch the edge timer.",
            );
  ...
  function update(elapsed) {
    ...
    if (connectionHint.parentElement !== hintParent)
      hintParent.append(connectionHint);
    connectionHint.hidden = !(
      assigned ||
      ["waiting-neutral", "unsupported", "disconnected"].includes(status.code)
    );
    const connectionText =
      assigned && status.code !== "waiting-neutral"
        ? scope === "flight"
          ...
          : controllerMenuHint()
        : status.message;
    if (connectionHint.textContent !== connectionText)
      connectionHint.textContent = connectionText;
    ...
    if (status.code === "joined" && scope !== "flight")
      controllerNavigation.engage();
    if (disconnected) {
      clearInput();
      pause(true);
      warning(
      ...
      if (
        scope === "flight" &&
        (flight.stop || (flight.action && !capabilities.manualAbility))
      ) {
        pause(true);
        clearInput();
      } else if (
    ...
    if (
      ...
      if (elapsed > 0.25) {
        pause(true);
        warning(
          "Paused after a long frame interruption. Resume to continue safely.",
        );
        return;
      }
      accumulator += elapsed;
      while (
        accumulator + 1e-9 >= FIXED_DT &&
        !["won", "lost"].includes(run.status)
      ) {
        const command = demo
        ...
        if (masteryObserver)
          try {
            ...
            masteryAward = {
              status: "unavailable",
              message:
                "Live goal tracking is unavailable. Your picture progress is kept.",
            };
          }
        pendingAction = false;
        pendingPickup = false;
      ...
      if (!handled && ["won", "lost"].includes(run.status)) {
        ...
        if (
          ...
          if (!candidateHost?.owns(activeEntry)) {
            ...
            try {
              ...
              if (tuning) {
                const authored = campaign.levels.find(
                  (level) => level.id === run.levelId,
                );
                if (
                  tuning.adminOverride ||
                  canonicalJSON(applyGameplayTuning(authored, tuning)) !==
                    canonicalJSON(run.level)
                )
                  throw new Error(
              ...
              $("save-warning").textContent =
                `${completionWarning} ${error.message}`;
              show("save-warning", true);
            }
            progress = progressFor(library, campaign);
            appearanceRewardIds = [...availableBodies()].filter(
              (id) => !previousBodies.has(id),
            );
            persistProfile();
            updateBodies();
            paintMissions();
        ...
        if (run.status === "won") {
          ...
          show("show-result", false);
          warning(
            "Picture unlocked. A whole world, from one brave line.",
            null,
            "host.won",
          );
    ...
    if (
      !sound.previewActive &&
      $("music-preview").textContent.startsWith("Playing")
    )
      $("music-preview").textContent = "Preview music ♫";
    refreshHUD();
  }
  for (const t of themesFile.themes)
    $("theme-select").append(new Option(t.name, t.id));
  ...
  $("theme-preparation-cancel").onclick = () => {
    const restoreFocus =
      document.activeElement === $("theme-preparation-cancel");
    cancelPictureStart();
    $("theme-select").value = theme.id;
  ...
  $("theme-select").onchange = async () => {
    ...
    const next =
      ...
      (scenario?.theme?.id === $("theme-select").value
        ? scenario.theme
        : themesFile.themes[0]);
    const notify = flightInformation.captureWarning("host.picture", {
      allowTerminal: true,
    ...
    const feedback = themeFeedback.begin({
      ...
      isCurrent: () =>
        pictureThemePending?.ticket === ticket && !controller.signal.aborted,
    });
  ...
  $("class-select").onchange = () =>
    requestMissionReplacement(
      { kind: "class", id: $("class-select").value },
      $("class-select"),
    );
  $("turn-select").onchange = () =>
    requestMissionReplacement(
      { kind: "steering", id: $("turn-select").value },
      $("turn-select"),
    );
  $("start-button").onclick = () => resume();
  $("continue-saved").onclick = async () => {
    const notify = flightInformation.captureWarning("host.restore", {
      allowTerminal: true,
    });
    try {
  ...
  function requestRestart(opener) {
    ...
    try {
      ...
      restartRequest = null;
      warning(
        `Restart confirmation unavailable: ${error.message}. Your flight remains paused.`,
      );
  ...
  restartDialog.addEventListener("close", () => {
    ...
    if (
      availableFocusTarget(ticket.opener) &&
      (!top || top.contains(ticket.opener))
    )
      ticket.opener.focus({ preventScroll: true });
  });
  ...
  $("demo-button").onclick = () => {
    ...
    resume();
    warning(
      "Demonstration: this is a real simulated cut. It grants no rewards.",
    );
  };
  $("sound-button").onclick = () =>
    setMasterMuted(!audioMaster.snapshot().muted);
  $("settings-master-mute").onclick = () =>
    setMasterMuted(!audioMaster.snapshot().muted);
  $("shell-sound").onclick = () =>
    setMasterMuted(!audioMaster.snapshot().muted);
  for (const id of ["tap-steering", "settings-tap-steering"]) {
    $(id).onchange = () => {
      ...
      $("tap-steering").checked =
        library.preferences.tapSteering ??
    ...
    if (!collectionContexts.has(collectionContextKey)) {
      collectionContextKey =
        [...library.gallery]
          ...
          .find((item) => collectionContexts.has(item.campaignKey))
  ...
  document.querySelectorAll("[data-close]").forEach(
    (b) =>
      (b.onclick = () => {
        if (b.dataset.close === "help-dialog" && courseEntry)
          cancelCourseEntry();
  ...
  async function downloadCurrentReplay(replay) {
    const notify = flightInformation.captureWarning("host.replay", {
      allowTerminal: true,
    });
    ...
    try {
      ...
      if (operation.observed && $("replay-dialog").open)
        operation.status.finish({
          message: `Replay download: ${error.message}`,
          state: "error",
        });
    } finally {
  ...
  $("download-replay").onclick = () =>
    lastReplay && downloadCurrentReplay(lastReplay);
  function suspendInteraction() {
    ...
    if (courseEntry)
      cancelCourseEntry(
        "Course entry cancelled when focus changed. Your flight remains paused.",
      );
    clearInput();
    controllerPreview?.clear();
    suspendAudio();
  ...
  function restoreListening() {
    if (document.hidden || soundtrackDisposed || enemyGuide?.practiceActive)
      return;
    if (!soundtrackPlayer) {
  ...
  document.addEventListener("pointerdown", restoreAudioOnGesture, {
    capture: true,
    passive: true,
  });
  document.addEventListener("touchstart", restoreAudioOnGesture, {
    capture: true,
    passive: true,
  });
  ...
  class FieldScene extends Phaser.Scene {
    ...
    update(now, delta) {
      ...
      painter.draw(this.boardTexture.context, run, Math.min(dt, 0.1), {
        ...
        showGrid:
          scenario?.presentation?.showGrid || library.preferences.showGrid,
        backdrop: flightPictures?.current(),
  ...
  missionPicker = attachMissionPicker({
    archivedIds: preparePackCatalog(archiveCatalogSource).packs.map(
      ({ id }) => id,
    ),
  });
  async function prepareLibraryClassic(row, { signal }) {
    ...
    if (row.source === "optional") {
      ...
      if (!summary)
        throw new Error(
          "That optional chapter is unavailable in this release.",
        );
    ...
    if (contentSwitchBusy || backupBusy || sessionBusy)
      throw new Error(
        "Finish the current operation before downloading another chapter.",
      );
    const operation = packLaunchGuard.begin(packs);
    packCommits.markIntent();
    contentSwitchBusy = true;
  ...
  function departLibraryMission(context) {
    ...
    if (context.transferContinuation && !context.transferContinuation())
      return false;
    const target = unifiedLibrary.library.find(context.libraryMissionId);
    return requestModeDeparture(
      "library",
      { preventDefault() {} },
      $("shell-play"),
      {
  ...
  async function launchLibraryClassic(pack, selection, context) {
    ...
    if (context.continuation && journeyEnabled && context.mode === "solo") {
      ...
      const authored =
        pack === null
          ? baseEntry
          : resolvePackCampaign(pack, selection.campaignId);
      const entry = executionForEntry(
        authored,
        library.preferences.campaignDifficulty,
      );
      const level = entry.campaign.levels.find(
        (item) => item.id === selection.levelId,
      );
      ...
      try {
        await pictures.ensure(nextTheme.id, {
    ...
    if (journeyEnabled || context.mode !== "solo")
      return departLibraryMission(context);
    if (context.continuation && run?.status === "won") {
      ...
      const nextIndex = entry.campaign.levels.findIndex(
        (level) => level.id === selection.levelId,
      );
      if (nextIndex < 0)
        throw new Error("The exact next mission is unavailable.");
      if (context.transferContinuation && !context.transferContinuation())
        return false;
    ...
    const launch = {
      ...
      onSelected: () => {},
      onCancelled: () => {
        if (!launch.isCurrent() || document.hasFocus?.() === false)
          return false;
        unifiedChooser?.restore();
    ...
    const selected = await requestWorldPlay(pack, {
      ...
      onStatus: (status) => {
        if (launch.isCurrent())
          contentStatus(status.message, false, {
            busy: status.stage !== "ready",
          });
      },
    });
  ...
  async function getUnifiedMissionLibrary() {
    ...
    unifiedLibraryLoading = (async () => {
      ...
      const route =
        authoredRoute ||
        (await loadAuthoredJourneyRoute(DEFAULT_JOURNEY_ROUTES.solo));
      const originalThemes = (await getJSON("content-design/themes.json"))
        .themes;
      ...
      const { createCandidateVersusHost } = await import(
        "./content-design/versus-host.mjs"
      );
      ...
      const manifestFor = (
        ...
        host
          ...
          ?.manifests.find(
            (manifest) => manifest.missionId === mission.levelId,
          );
      const source = journeyLibrarySource({
        ...
        tags: (mission) =>
          authoredJourneyMissionTags(mission, manifestFor(mission, "standard")),
        card: (mission) =>
          host.card(mission, browsingJourneyPreferences.snapshot().difficulty),
        launch: (mission, context) => {
          ...
          return launchJourneyMission(mission, {
            kind: context.continuation ? "next" : "choose",
          });
        },
      });
      const versusSource = journeyLibrarySource({
        ...
        tags: (mission) =>
          authoredJourneyMissionTags(mission, versusPreview.manifest(mission)),
        card: (mission) =>
          versusPreview.card(
            mission,
            browsingJourneyPreferences.snapshot().difficulty,
      ...
      const result = await createInstalledMissionLibrary({
        ...
        availabilityExternal: (row, pack) => {
          if (!pack)
            return isRelease
              ...
              : {
                  state: "unavailable",
                  reason:
                    "Original-picture download is available in published builds.",
                };
          ...
          return present
            ...
            : {
                state: "unavailable",
                reason:
                  "Install this chapter with its original pictures in Worlds.",
              };
        },
        launchClassic: async (row, context) => {
          if (row.source === "external") {
            ...
            if (
              packs.packs.find((pack) => pack.id === row.packId) !==
              context.pack
            )
              throw new Error(
                "Installed originals changed. Choose Play again.",
              );
          }
        ...
        progressClassic: (row) =>
          library.campaigns[row.campaignKey]?.clears?.[row.levelId]
            ? "Cleared"
            : "",
        progressCustom: (binding) =>
          library.campaigns[binding.selection.campaignKey]?.clears?.[
            binding.selection.levelId
          ]
            ? "Cleared"
            : "",
      });
      ...
      const returnedRow = exactReturn
        ? result.library.missions.find((row) => {
            ...
            return (
              ...
              JSON.parse(row.campaignKey)[2] ===
                returnedSelection.campaignKey &&
              sourcePackId === (activeEntry.sourcePackId ?? null) &&
              row.modes.includes("solo") &&
      ...
      if (setup) {
        setup
          ...
          .append($("shell-mode-choice"));
        setup.classList.add("mission-library-setup");
        setup.open = false;
        setup.querySelector("summary").textContent =
          "Current Solo flight setup";
    ...
    try {
      ...
      if (revision === unifiedOpenRevision) {
        ...
        feedback.textContent =
          "Missions could not load. Choose Missions to retry.";
  ...
  if (journeyEnabled) {
    ...
    if (authoredRoute?.id === DEFAULT_JOURNEY_ROUTES.solo)
      $("journey-artwork-availability").textContent =
        "Balance review pending. Mission pictures need an internet connection. Preparing offline play does not yet include them; full game downloads do.";
    ...
    $("journey-save-export").onclick = async () => {
      try {
        await downloadJSON(
          JSON.parse(journeyProfile.export()),
          journeyProfile.backupFilename,
        );
      } catch (error) {
  ...
  if (journeyPreferences) {
    ...
    $("journey-preferences-retry").onclick = () => {
      const restoreFocus =
        document.activeElement === $("journey-preferences-retry");
      const snapshot = journeyPreferences.retry();
      if (
        snapshot.durable &&
        restoreFocus &&
        availableFocusTarget($("difficulty-select"))
      )
    ...
    $("journey-preferences-export").onclick = async () => {
      ...
      try {
        ...
        if (
          ticket === exportSequence &&
          !$("journey-preferences-recovery").hidden
        )
  ...
  optionalWorlds = attachOptionalChaptersPanel({
    ...
    sourceChapters: !practiceSession
      ? SOURCE_EXTERNAL_EDITIONS.map(
          ({ descriptor, name, description, mode, levels }) => ({
            id: descriptor.id,
            controlId:
            ...
            download: isRelease
              ? (options) =>
                  installSourceChapter(descriptor.id, null, {
                    ...options,
                    download: true,
                  })
              : null,
    ...
    playInstalled: (pack, options) => {
      if (
        SOURCE_EXTERNAL_EDITIONS.some(
          ({ descriptor }) => descriptor.id === pack.id,
        )
      )
    ...
    chooseInstalled: async (pack, { signal, launch, onStatus }) => {
      ...
      if (signal?.aborted)
        throw new DOMException("World selection cancelled.", "AbortError");
  ...
  gameShell = attachGameShell({
    ...
    initial:
      !practice && !courseSession && !packLaunchRequest && !libraryHandoff,
  ...
  if (authoredRoute?.id === DEFAULT_JOURNEY_ROUTES.solo)
    $("shell-title-team").querySelector(".game-mode-description").textContent =
      "12 Team missions";
    ...
    requestAnimationFrame(() => {
      ...
      if (
        canAutoStartPackLaunch({
          ...
          blocked:
            dialogOpen() || contentSwitchBusy || sessionBusy || document.hidden,
          started,
          paused,
  ...
  $("run-message").textContent = new URLSearchParams(location.search).has(
    "course",
  )
