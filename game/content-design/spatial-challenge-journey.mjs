...
export function createSpatialChallengeJourney({ artwork = false } = {}) {
  ...
  const replacement = study.missions.find(
    (mission) => mission.id === "twin-receivers",
  );
  const index = source.missions.findIndex(
    (mission) => mission.id === replacement.id,
  );
  ...
  source.maps = source.maps.filter(
    (map) =>
      !sameMap(map, previous.map) ||
      source.missions.some((mission) => sameMap(map, mission.map)),
  );
  const map = study.maps.find((candidate) =>
    sameMap(candidate, replacement.map),
  );
  if (!source.maps.some((candidate) => sameMap(candidate, map)))
