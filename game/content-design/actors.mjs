...
export function editContentActor(source, missionId, input) {
  ...
  required(['add', 'replace', 'remove'].includes(action), 'Choose an actor operation.');
  exactKeys(command, ['action', 'id', ...(action === 'remove' ? [] : ['actor'])], 'actor command');
  required(stableId(id), 'Give the actor a stable ID.');
  ...
  if (action !== 'remove') required(actor?.id === id, 'Actor identity must match the command.');
