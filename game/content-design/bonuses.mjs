...
export function editContentBonus(source, missionId, input) {
  ...
  required(['add', 'replace', 'remove'].includes(command.action), 'Choose a bonus operation.');
  exactKeys(
    command,
    ['action', 'id', ...(command.action === 'remove' ? [] : ['bonus'])],
    'bonus command',
  );
  required(stableId(command.id), 'Give the bonus a stable ID.');
  ...
  if (command.action !== 'remove')
    required(command.bonus?.id === command.id, 'Bonus identity must match the command.');
