...
import {
  editContentStructure,
  inspectContentRemoval,
...
import {
  createTeamTestPack,
  createTeamCampaignTestPack,
} from "../content-design/team-export.mjs";
...
const pacingInspector = createPacingInspector({
  document,
...
const imageWorkbench = createImageWorkbench({
  ...
  play: (source, missionId, difficulty) =>
    launchPreview(source, missionId, difficulty),
...
function discardSource() {
  return (
    !sourceChanged ||
    window.confirm(
      "Discard unapplied JSON edits? The applied draft is unchanged.",
    )
  );
}
...
function inspectBoard(trailCells = []) {
  ...
  const preset = journeyPreset(
    manifest.difficulty,
    session.current().difficultyCatalogId,
  );
  ...
  $("current-gameplay").textContent =
    `${effectiveGameplay.label}: craft ${Number(effectiveGameplay.playerSpeed.toFixed(3))} cells/s; ${effectiveGameplay.population.actualEnemies} enemies (${effectiveGameplay.population.addedKeepers} added keepers). ${effectiveGameplay.actors
      ...
      .map(
        (actor) =>
          `${actor.id}: ${Number(actor.speed.toFixed(3))} cells/s, ${actor.domain}`,
      )
      .join(
        "; ",
  ...
  $("capture").textContent = JSON.stringify(
    {
      ...
      ...(capture.affectedCombatIds
        ? { affectedCombatActors: capture.affectedCombatIds }
        : {}),
  ...
  $("play").disabled =
    !mission.modes.includes("solo") || !!mission.combat?.enabled;
  ...
  $("team-test-campaign").replaceChildren(
    ...[
      ...
      ...teamDraft.campaigns.filter(
        (campaign) =>
          !campaign.archived &&
          campaign.missionIds.some((id) => activeTeamIds.has(id)),
      ),
    ].map((campaign) => {
  ...
  if (
    [...$("team-test-campaign").options].some(
      (option) => option.value === selectedTeamCampaign,
    )
  )
...
function render(selected = $("mission").value) {
  ...
  $("pressure-edition").disabled =
    project.difficultyCatalogId === "journey-difficulty-v2";
  ...
  if (project.missions.some((m) => m.id === selected))
...
function syncStructure() {
  ...
  const fill = (id, rows, blank) => {
    ...
    if ([...$(id).options].some((option) => option.value === selected))
      $(id).value = selected;
  };
  ...
  for (const [id, visible] of [
    ...
    [
      "item-parent-row",
      kind !== "pack" &&
        !["rename", "set-band", "delete", "archive", "restore"].includes(
          action,
        ),
    ],
  ...
  if (kind === "campaign" && action === "set-band")
    $("item-band").value =
      project.campaigns.find((entry) => entry.id === $("item-target").value)
        ?.band ?? 1;
...
$("structure-form").onsubmit = guarded((event) => {
  ...
  const command = {
    ...
    $("item-parent").value
      ...
      : {}),
    ...(["earlier", "later"].includes(action)
      ? { offset: action === "earlier" ? -1 : 1 }
      : {}),
    ...(action === "delete"
      ? { confirmationId: $("item-confirm").value.trim() }
      : {}),
  };
...
$("apply").onclick = guarded(async () => {
  ...
  if (session.status().saving)
    throw new Error(
      "Wait for the current checkpoint save before replacing the project.",
    );
  clearTimeout(saveTimer);
  if (pending.head) {
    setSession(pending.head.project, pending.head.revision);
    if (
      JSON.stringify(pending.project) !== JSON.stringify(pending.head.project)
    )
      session.replace(pending.project);
  } else if (pending.project.id !== session.current().id)
    setSession(pending.project);
...
$("load").onclick = guarded(async () => {
  ...
  inspectSource({
    head: loaded.head,
    selectedRevision: loaded.selected.revision,
  });
});
$("new").onclick = guarded(() => {
  if (!discardSource()) return;
  ...
  if (id === session.current().id)
    throw new Error(
      "Choose a new project ID; this action does not reset an existing project.",
    );
...
$("pressure-edition").onclick = guarded(() => {
  ...
  $("source").value = JSON.stringify(
    withPressureDifficulty(session.current()),
    null,
    2,
  );
  sourceChanged = true;
...
$("border").onclick = guarded(() => {
  if (!discardSource()) return;
  $("source").value = JSON.stringify(
    createBorderCandidates({ artwork: true }),
    null,
    2,
  );
  sourceChanged = true;
...
$("signal").onclick = guarded(() => {
  if (!discardSource()) return;
  $("source").value = JSON.stringify(
    createSignalCandidates({ campaignTheme: true }),
    null,
    2,
  );
  sourceChanged = true;
...
$("fracture").onclick = guarded(() => {
  ...
  $("source").value = JSON.stringify(
    withCampaignPresentation(
      createFractureCandidates({ artwork: true }),
      "fracture",
    ),
    null,
    2,
...
$("livewire").onclick = guarded(() => {
  ...
  $("source").value = JSON.stringify(
    withCampaignPresentation(
      createLivewireCandidates({ artwork: true }),
      "livewire",
    ),
    null,
    2,
...
$("team-signal").onclick = guarded(() => {
  if (!discardSource()) return;
  $("source").value = JSON.stringify(
    createTeamSignalCandidates({ campaignTheme: true }),
    null,
    2,
  );
  sourceChanged = true;
...
$("crosswind").onclick = guarded(() => {
  ...
  $("source").value = JSON.stringify(
    withCampaignPresentation(
      createCrosswindCandidates({ artwork: true }),
      "crosswind",
    ),
    null,
    2,
...
$("sentinel").onclick = guarded(() => {
  ...
  $("source").value = JSON.stringify(
    withCampaignPresentation(
      createSentinelCandidates({ artwork: true }),
      "sentinel",
    ),
    null,
    2,
...
$("whole-spatial").onclick = guarded(() => {
  if (!discardSource()) return;
  $("source").value = JSON.stringify(
    createWholeSpatialCandidates({ artwork: true }),
    null,
    2,
  );
  sourceChanged = true;
...
$("whole-field").onclick = guarded(() => {
  if (!discardSource()) return;
  $("source").value = JSON.stringify(
    createWholeFieldCandidates({ artwork: true }),
    null,
    2,
  );
  sourceChanged = true;
...
$("whole-timed").onclick = guarded(() => {
  if (!discardSource()) return;
  $("source").value = JSON.stringify(
    createWholeTimedCandidates({ artwork: true }),
    null,
    2,
  );
  sourceChanged = true;
...
$("import").onchange = guarded(async () => {
  ...
  if (file.size > 4 * 1024 * 1024)
    throw new Error("Project JSON must be no larger than 4 MiB.");
  const current = inspections.begin();
  let text;
  try {
...
$("export").onclick = guarded(async () => {
  const result = await exportJSONFile(
    session.current(),
    `${session.current().id}-backup.json`,
  );
...
$("export-team").onclick = guarded(async () => {
  if (sourceChanged)
    throw new Error(
      "Inspect and apply source edits before exporting the selected mission.",
    );
  const mission = currentMission();
  ...
  const result = await exportJSONFile(
    pack,
    `${mission.id}-${difficulty}-team-test.json`,
  );
  status(
...
$("export-team-campaign").onclick = guarded(async () => {
  if (sourceChanged)
    throw new Error(
      "Inspect and apply source edits before exporting a Team campaign.",
    );
  ...
  const result = await exportJSONFile(
    pack,
    `${campaignId}-${difficulty}-team-test.json`,
  );
  status(
...
for (const id of ["mission", "difficulty"])
  $(id).onchange = guarded(() => inspectBoard());
...
$("geometry-form").onsubmit = guarded((event) => {
  ...
  if (![x, y, w, h].every(Number.isInteger))
    throw new Error("Geometry uses whole cells.");
  let changes;
  if (surface === "spawn" || surface === "spawn-team-two") {
    const spawnId =
      surface === "spawn-team-two"
        ? mission.team?.spawnIds[1]
        : mission.spawnId;
    if (!spawnId)
      throw new Error(
        "Choose a Team mission to move its second starting position.",
      );
    changes = {
      spawns: map.spawns.map((s) =>
        s.id === spawnId ? { ...s, x: x + 0.5, y: y + 0.5 } : s,
      ),
    };
...
$("inspect").onclick = guarded(() => {
  ...
  if (text.length > 16000)
    throw new Error("Trail input exceeds the board budget.");
  const cells = text
    ? text.split(",").map((cell) => {
        if (!/^\s*\d+\s*$/.test(cell))
...
async function launchPreview(source, missionId, difficulty) {
  ...
  try {
    ...
    const manifest = prepareContentPreview(project, missionId, {
      difficulty,
    }).manifest;
    ...
    result = prepareContentPreview(project, missionId, {
      difficulty,
      theme,
      artwork,
    });
  ...
  const url = new URL(`../?practice=1&revision=studio-${ticket}`, location.href)
    .href;
  $("preview").src = url;
  $("preview-status").textContent =
...
async function boot() {
  ...
  setSession(
    saved?.project ?? createStarterProject(projectId),
    saved?.revision ?? null,
  );
  render();
  if (storageError)
...
boot().catch((error) => {
  status(
    `Studio could not open: ${error.message}. Saved checkpoints were not changed.`,
    true,
  );
