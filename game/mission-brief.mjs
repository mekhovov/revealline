...
export function missionBriefing(
  ...
  const required = (level.objectives || []).filter(
    (item) => item.required,
  ).length;
  ...
  const otherRequired = multiShield
    ? Math.max(0, required - shieldCount - 1)
    : 0;
  const label = compact(objectiveLabel, 24).toLowerCase() || "objective";
  const plural = /[^aeiou]y$/.test(label)
    ? `${label.slice(0, -1)}ies`
    : `${label}s`;
  ...
  const classicHint = [
    ...
    ? [
        ...
        level.classic?.powerups?.length
          ? "Touch pickups to collect their effects."
          : "",
  ...
  const captureHint =
    rules.stopOnCapture === true
      ...
      : "";
  const wallHint =
    foundations && level.walls?.length
  ...
  const impactHint = level.classic?.lineImpact
    ...
    : "";
  const facts = [goal, encounterGoal, limits, recommendation]
    .filter(Boolean)
    .join("\n");
  return Object.freeze({
    ...
    copy: intro
      ...
      : [facts, captureHint, wallHint, impactHint, classicHint]
          .filter(Boolean)
          .join("\n"),
    fullBrief:
      authored ||
