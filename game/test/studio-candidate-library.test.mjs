...
test("six-map spatial successor stays in the existing combined edition selector", async () => {
  ...
  const host = await readFile(
    new URL("../studio/studio.mjs", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    handler,
    /'spatial-challenge-v1'\s*\? createSpatialChallengeJourney/,
  );
...
test("inner receiver search keeps the original edition and explicit Inspect in the same entry", () => {
  ...
  assert.equal(
    visible()[0],
    $("sentinel-spatial").closest("[data-library-entry]"),
  );
  assert.equal(
    $("sentinel-spatial-edition").closest("[data-library-entry]"),
    visible()[0],
  );
  const editions = [
    ...$("sentinel-spatial-edition").querySelectorAll("option"),
  ];
  assert.deepEqual(
...
test("outer-pocket search preserves the Shared windows edition selector beside its Inspect action", () => {
  ...
  assert.equal(
    visible()[0],
    $("team-timed-originals").closest("[data-library-entry]"),
  );
  assert.equal(
    $("team-window-edition").closest("[data-library-entry]"),
    visible()[0],
  );
});
...
test("Team pressure search keeps edition choice, original Inspect and bundled player together", () => {
  ...
  assert.equal(visible().length, 1);
  assert.equal(
    $("team-journey-edition").closest("[data-library-entry]"),
    visible()[0],
  );
...
test("all review routes keep explicit external-tab safety and distinct accessible labels", () => {
  ...
  const expected = journeys.flatMap((id) => [
    "../?journey=" + id,
    "../couch/?journey=" + id,
  ]);
  expected.push(
  ...
  assert.deepEqual(
    links.map((link) => link.getAttribute("href")).toSorted(),
    expected.toSorted(),
  );
...
test("category and search intersect, empty groups disappear, reset restores order and search focus", () => {
  ...
  assert(
    [...$("candidate-library").querySelectorAll("[data-library-group]")].every(
      (g) => g.hidden,
    ),
  );
  $("candidate-reset").emit("click");
...
test("host reports success only after compilation and clears feedback at every invalidation boundary", async () => {
  ...
  const css = await readFile(
    new URL("../studio/studio.css", import.meta.url),
    "utf8",
  );
