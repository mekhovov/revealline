...
for (const { difficulty, turnPolicy, rows } of fixture.sets.filter(
  ...
  test(`Apex candidate saves resume cuts, connections and telegraphed encounters: ${turnPolicy}`, async () => {
    for (const [id, identity, finalCheckpoint, segments] of rows) {
      ...
      for (const [index, input] of inputs.entries()) {
        ...
        const stage = run.events.some((e) => e.type === 'relay.opened')
          ? 'relay-opened'
          : run.events.some((e) => e.type === 'boss.warning')
            ? 'lane-warning'
      ...
      const expected = ['capture', 'live-trail'];
      if (run.relay.gates.length) expected.push('relay-opened');
      if (run.encounter) expected.push('shield-transition', 'exposed-warning');
      if (run.enemies.some((e) => e.type === 'lane-boss')) expected.push('lane-warning');
      assert.deepEqual([...milestones.keys()].sort(), expected.sort());
      const campaign = {
        id: `apex-${id}`,
      ...
      const saved = milestones.get('live-trail').saved;
