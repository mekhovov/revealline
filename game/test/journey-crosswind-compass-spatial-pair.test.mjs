...
test("Crosswind compass pair records two redesign dispositions and preserves historical identities", () => {
  ...
  assert.deepEqual(
    Object.keys(JOURNEY_CROSSWIND_COMPASS_SPATIAL_DISPOSITIONS),
    IDS,
  );
...
test("compass baffles and outer shelves preserve exact speed fields and governed topology", () => {
  ...
  for (const id of IDS) {
    ...
    assert.equal(
      currentMap.source.revision,
      "crosswind-compass-spatial-pair-1",
    );
    ...
    assert.deepEqual(
      currentMap.source.speedZones,
      previousMap.source.speedZones,
    );
    ...
    assert(
      currentMap.geometry.safeComponents.every(
        (item) => item.departures.length >= 20,
      ),
    );
    for (const difficulty of PRESETS) {
      ...
      assert.deepEqual(
        solo.level.directionalZones,
        previous.level.directionalZones,
      );
      ...
      assert.equal(
        capture.components[0].enemyIds.length,
        expected[id].retainers,
      );
      assert.deepEqual(
