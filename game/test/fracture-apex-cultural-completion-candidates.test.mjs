...
import {
  FRACTURE_APEX_CULTURAL_COMPLETION_REVISION,
  FRACTURE_APEX_CULTURAL_COMPLETION_SELECTIONS,
  FRACTURE_APEX_CULTURAL_COMPLETION_SOURCES,
  createFractureApexCulturalCompletionCandidates,
} from '../content-design/fracture-apex-cultural-completion-candidates.mjs';
import { createCrosswindCulturalCompletionCandidates } from '../content-design/crosswind-cultural-completion-candidates.mjs';
...
const IDS = FRACTURE_APEX_CULTURAL_COMPLETION_SELECTIONS.map((item) => item.id);
const PRESETS = ['gentle', 'standard', 'expert'];
const beforeSource = createCrosswindCulturalCompletionCandidates({ artwork: true });
const source = createFractureApexCulturalCompletionCandidates({ artwork: true });
...
test('selection completes two Fracture and one Apex identity with bounded attribution', () => {
  assert.deepEqual(IDS, ['bank-the-crossing', 'five-anchors', 'final-broadcast']);
  for (const selection of FRACTURE_APEX_CULTURAL_COMPLETION_SELECTIONS) {
    ...
    assert(selection.sourceIds.every((id) => FRACTURE_APEX_CULTURAL_COMPLETION_SOURCES[id]));
  }
  for (const item of Object.values(FRACTURE_APEX_CULTURAL_COMPLETION_SOURCES)) {
    assert.match(item.url, /(opishne-museum\.gov\.ua|ich\.unesco\.org|museum\.mincult\.gov\.ua)/);
...
for (const artwork of [false, true])
  test(`copy-on-write changes only selected missions and owners: artwork=${artwork}`, () => {
    ...
    const revised = createFractureApexCulturalCompletionCandidates({ artwork });
    assert.deepEqual(createCrosswindCulturalCompletionCandidates({ artwork }), snapshot);
    assert.equal(revised.revision, FRACTURE_APEX_CULTURAL_COMPLETION_REVISION);
    ...
    for (const current of revised.missions) {
      ...
      if (!IDS.includes(current.id)) assert.deepEqual(current, previous);
        ...
        assert.equal(current.revision, FRACTURE_APEX_CULTURAL_COMPLETION_REVISION);
        for (const key of [
          'id',
          'name',
          'spawnId',
...
test('replacement geometry keeps objectives, relay gates and retained fields usable', () => {
  const expected = {
    'bank-the-crossing': { walls: 8, foundations: 4, terrain: 2, safe: 4 },
    'five-anchors': { walls: 12, foundations: 6, terrain: 2, safe: 6 },
    'final-broadcast': { walls: 8, foundations: 11, terrain: 0, safe: 8 },
  };
  for (const id of IDS) {
...
test('the successor removes three more open-plain identities without treating openness as a defect', () => {
...
test('rules, roles, objectives, relays and bonuses remain exact across presets and modes', () => {
  for (const id of IDS)
    for (const difficulty of PRESETS) {
...
function closeRoute(run, id, approach, sidecars = {}) {
  ...
  if (id === 'bank-the-crossing' && approach === 'centre-jig-first') close('down');
  else if (id === 'bank-the-crossing' && approach === 'equipment-pad-first') {
    position('right', (state) => state.player.x >= 58.5);
    close('down');
  } else if (id === 'five-anchors' && approach === 'central-cross-first') close('down');
  else if (id === 'five-anchors' && approach === 'west-panel-first') {
    position('left', (state) => state.player.x <= 16.5);
    close('down');
  } else if (id === 'final-broadcast' && approach === 'upper-link-first') close('up');
  else if (id === 'final-broadcast' && approach === 'lower-link-first') close('down');
...
test('registered v28 successor preserves v27 and remains an explicit review route', async () => {
  const current = createAuthoredJourneyRoute('whole-spatial-v28');
  const previous = createAuthoredJourneyRoute('whole-spatial-v27');
  assert.deepEqual(await loadAuthoredJourneyRoute(current.id), current);
  assert.equal(current.profileKey, 'journey-whole-spatial-v28');
  assert.equal(current.sessionKey, 'revealline.suspended.journey-whole-spatial.v28');
  ...
  assert.equal(authoredJourneyModeHref(current.id, 'solo'), '../?journey=whole-spatial-v28');
  assert.equal(
    authoredJourneyModeHref(current.id, 'versus'),
    'couch/?journey=whole-spatial-v28&return=solo',
  );
});
