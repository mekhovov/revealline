...
test('all five Apex originals have exact unique pins and verified opt-in previews', async () => {
  const source = createApexCandidates({ artwork: true });
  const project = compileContentProject(source);
  const grey = compileContentProject(createApexCandidates());
  const theme = JSON.parse(
    await readFile(new URL('../content-design/themes.json', import.meta.url)),
  ).themes.find((row) => row.id === 'horizon');
  assert.equal(APEX_ART_CANDIDATES.length, 5);
  assert.equal(
    new Set(JOURNEY_ART_CANDIDATES.map((asset) => asset.sha256)).size,
    JOURNEY_ART_CANDIDATES.length,
  );
  for (const asset of APEX_ART_CANDIDATES) {
    ...
    assert.equal(asset.id, `apex-polar-${consumers[0].id}`);
  ...
  const second = createApexCandidates({ artwork: true });
  second.assets[0].alt = 'Local edit';
  assert.notEqual(second.assets[0].alt, APEX_ART_CANDIDATES[0].alt);
});

test('Apex prompts and pending reviews correspond to the original pins', async () => {
  const record = JSON.parse(
    await readFile(new URL('../../docs/research/apex-original-art-prompts.json', import.meta.url)),
  );
  assert.equal(record.tool, 'built-in-imagegen');
  assert.equal(record.assets.length, APEX_ART_CANDIDATES.length);
  assert.equal(new Set(record.assets.map((entry) => entry.missionId)).size, 5);
  for (const entry of record.assets) {
    const asset = APEX_ART_CANDIDATES.find((row) => `game/${row.path}` === entry.path);
    assert(asset);
...
test('Apex pictured edition preserves all30 historical clear checkpoints', async () => {
  const fixture = JSON.parse(
    await readFile(new URL('./fixtures/apex-clear-routes.json', import.meta.url)),
  );
  const project = compileContentProject(createApexCandidates({ artwork: true }));
  let checked = 0;
...
test('Studio inspects pictured Apex candidates without bypassing explicit Apply', async () => {
  const studio = await readFile(new URL('../studio/studio.mjs', import.meta.url), 'utf8');
  const handler = studio.match(/\$\('apex'\)\.onclick = guarded\(\(\) => \{([\s\S]*?)\n\}\);/)[1];
  assert.match(handler, /discardSource\(\)/);
  assert.match(handler, /createApexCandidates\(\{ artwork: true \}\)/);
