...
test('Signal optional design goals each have one exact legal Standard route, not an automatic award', () => {
  ...
  for (const [id, identity, checkpoint, segments] of fixture.rows) {
    const mission = source.missions.find((m) => m.id === id),
      ...
      manifest = resolveMission(project, id);
    assert.equal(manifest.simulationIdentity, identity, id);
    const options = { seed: fixture.seed, classId: 'scout', turnPolicy: fixture.turnPolicy };
