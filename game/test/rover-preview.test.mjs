...
test('every Rover Studio preview resolves the exact shared mission and preset without changing the draft', () => {
  ...
  assert.throws(() => prepareContentPreview(source, 'sorting-yard', { mode: 'team' }), /mode/);
});

test('Rover CLI shares the revised mission and advisory campaign projection used by Studio', () => {
  const cli = (...args) =>
    spawnSync(
      process.execPath,
      [
  ...
  const mission = cli('--mission', 'sorting-yard', '--difficulty', 'expert');
  assert.equal(mission.status, 0, mission.stderr);
  assert.deepEqual(
    JSON.parse(mission.stdout),
    resolveMission(project, 'sorting-yard', { difficulty: 'expert' }),
  );
  const pacing = cli('--pacing', '--exclude-campaigns', 'rover-remixes');
  assert.equal(pacing.status, 0, pacing.stderr);
  assert.deepEqual(
    JSON.parse(pacing.stdout),
    inspectContentPacing(source, { excludedCampaignIds: ['rover-remixes'] }),
  );
});
