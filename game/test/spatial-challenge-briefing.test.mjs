...
test('protected first wall inspection has nonmodal return-surface guidance', () => {
  ...
  assert.match(brief.status, /Walls block craft and enemies; they never close a cut/);
  ...
  assert.doesNotMatch(missionBriefing(resolveMission(project, 'two-bays').level).status, /Walls/);
});
