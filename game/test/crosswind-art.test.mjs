...
test('all seven Crosswind originals have exact unique pins and verified opt-in previews', async () => {
  const source = createCrosswindCandidates({ artwork: true });
  const project = compileContentProject(source);
  const grey = compileContentProject(createCrosswindCandidates());
  const theme = JSON.parse(
    await readFile(new URL('../content-design/themes.json', import.meta.url)),
  ).themes.find((row) => row.id === 'horizon');
  assert.equal(CROSSWIND_ART_CANDIDATES.length, 7);
  assert.equal(
    new Set(JOURNEY_ART_CANDIDATES.map((asset) => asset.sha256)).size,
    JOURNEY_ART_CANDIDATES.length,
  );
  for (const asset of CROSSWIND_ART_CANDIDATES) {
    ...
    assert.equal(asset.id, `crosswind-highlands-${consumers[0].id}`);
  ...
  const second = createCrosswindCandidates({ artwork: true });
  second.assets[0].alt = 'Local edit';
  assert.notEqual(second.assets[0].alt, CROSSWIND_ART_CANDIDATES[0].alt);
});

test('Crosswind prompts and pending reviews correspond to the original pins', async () => {
  const record = JSON.parse(
    await readFile(
      new URL('../../docs/research/crosswind-original-art-prompts.json', import.meta.url),
    ),
  );
  assert.equal(record.tool, 'built-in-imagegen');
  assert.equal(record.assets.length, CROSSWIND_ART_CANDIDATES.length);
  assert.equal(new Set(record.assets.map((entry) => entry.missionId)).size, 7);
  for (const entry of record.assets) {
    const asset = CROSSWIND_ART_CANDIDATES.find((row) => `game/${row.path}` === entry.path);
    assert(asset);
...
test('Crosswind pictured edition preserves all42 historical clear checkpoints', async () => {
  const fixture = JSON.parse(
    await readFile(new URL('./fixtures/crosswind-clear-routes.json', import.meta.url)),
  );
  const project = compileContentProject(createCrosswindCandidates({ artwork: true }));
  let checked = 0;
...
test('Studio inspects pictured Crosswind candidates without bypassing explicit Apply', async () => {
  const studio = await readFile(new URL('../studio/studio.mjs', import.meta.url), 'utf8');
  const handler = studio.match(
    /\$\('crosswind'\)\.onclick = guarded\(\(\) => \{([\s\S]*?)\n\}\);/,
  )[1];
  assert.match(handler, /discardSource\(\)/);
  assert.match(handler, /createCrosswindCandidates\(\{ artwork: true \}\)/);
