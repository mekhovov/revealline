...
const fixture = JSON.parse(
  await readFile(new URL('./fixtures/sentinel-mastery-routes.json', import.meta.url)),
);
test('Sentinel optional-goal routes cover every preset, steering option and mission', () => {
  assert.equal(fixture.format, 'SentinelMasteryFeasibilityV1');
  assert.deepEqual(
...
for (const { difficulty, turnPolicy, rows } of fixture.sets)
  test(`Sentinel Crown optional goals with no-loss public replays: ${difficulty}/${turnPolicy}`, () => {
    for (const [missionId, identity, checkpoint, segments] of rows) {
      ...
      for (const [direction, ticks] of segments) {
        ...
        for (let tick = 0; tick < ticks; tick++) {
          ...
          if (!run.encounter.defeated) assert.notEqual(run.status, 'won', missionId);
        }
      }
      assert.equal(run.status, 'won', missionId);
      const goal = inspectSentinelGoal({ missionId, run, evidence });
