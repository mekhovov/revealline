...
const fixture = JSON.parse(
  await readFile(new URL('./fixtures/relay-mastery-routes.json', import.meta.url)),
);
test('relay optional-goal routes cover every preset, steering option and mission', () => {
  assert.equal(fixture.format, 'RelayMasteryFeasibilityV1');
  assert.deepEqual(
...
for (const { difficulty, turnPolicy, rows } of fixture.sets)
  test(`Relay Labyrinth optional goals with no-loss public replays: ${difficulty}/${turnPolicy}`, () => {
    for (const [missionId, identity, checkpoint, segments] of rows) {
      ...
      for (const [direction, ticks] of segments) {
        ...
        for (let tick = 0; tick < ticks; tick++) {
          ...
          const before = relayBeforeStep(run);
      ...
      const goal = inspectRelayGoal({ missionId, run, evidence });
      assert.equal(goal.achieved, true, `${missionId}: ${JSON.stringify(goal)}`);
      assert(evidence.used.size > 0, `${missionId}: no connector used`);
      assert(
