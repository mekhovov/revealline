...
test('ordinary Solo entry offers current and prior Journey editions plus 188 Classic missions and direct Start, Retry and Next', async (t) => {
  ...
  assert.equal(new URL(p.$('shell-playground').href).search, '?journey=whole-spatial-v22');
  ...
  assert.equal(journeyCards.length, 130);
  assert.equal(
...
test('ordinary Solo Continue uses its current Journey bookmark without converting old records', async (t) => {
  ...
  const current = createJourneyBackend({ ...memory, profileKey: 'journey-whole-spatial-v22' });
...
test('Legacy is explicitly accessible and its unified selector opens an exact New Journey mission', async (t) => {
  ...
  assert.equal(target.searchParams.get('journey'), 'whole-spatial-v22');
...
test('switching from a running default Journey to Legacy has an explicit Stay or Leave boundary', async (t) => {
  ...
  assert.equal(target.searchParams.get('library-mission'), card.dataset.missionId);
  assert(storage.getItem('revealline.suspended.journey-whole-spatial.v22'));
  assert.deepEqual(p.errors, []);
});
