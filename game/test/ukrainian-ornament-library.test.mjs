...
test('ornament opt-in leaves default, historic storage and all core continuation untouched', async () => {
  ...
  const previous = await loadAuthoredJourneyRoute('whole-spatial-v6');
...
test('one ornament selector keeps all three previous versions plus the six v5 alternatives, without media fetch', async (t) => {
  ...
  let selected;
  const previous = await createSpatialEditionSources({
  ...
  assert.equal(library.missions.length, 100);
  assert.equal(new Set(library.missions.map((row) => row.id)).size, 100);
  assert.equal(previous.sources.flatMap((s) => s.entries).length, 9);
  ...
  for (const id of ids) {
    ...
    assert.deepEqual(
      ...
      new Set([routeId, 'whole-spatial-v6']),
    );
    ...
    const old = matches.find((row) => row.editionId === 'whole-spatial-v6');
    for (const mode of ['solo', 'versus']) {
      ...
      assert.equal(href.searchParams.get('journey'), 'whole-spatial-v6');
...
test('previous v6 progress never leaks into the retained v5 edition', async () => {
  ...
  for (const row of library.missions)
    assert.equal(
      ...
      row.editionId === 'whole-spatial-v6' ? 'Cleared' : '',
    );
  previous.dispose();
});
