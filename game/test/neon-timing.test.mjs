...
const ordinary = await read('neon-clear-routes');
const supplemental = await read('neon-mastery-routes');
const timing = await read('neon-timing-routes');
const project = compileContentProject(createNeonCandidates());

test('all63 ordinary and supplemental Neon routes remain legal no-loss clears across five explicit seeds', () => {
...
test('every Neon greybox has a legal no-loss clear after each sampled initial decision delay', () => {
  assert.equal(timing.format, 'NeonTimingFeasibilityRoutesV1');
...
test('Five-second Four quarters delay needs a changed approach, not a promised safe default cut', () => {
