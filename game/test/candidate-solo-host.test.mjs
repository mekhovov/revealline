...
test('authored Solo clear and Next cross campaign boundaries without Legacy awards; Remix is optional', async (t) => {
  ...
  for (const [id, , checkpoint, segments] of rows.slice(0, 9)) {
  ...
  assert.equal(p.rendered.run.levelId, 'long-way-home');
  ...
  for (let attempt = 0; attempt < 200; attempt++) {
    ...
    if (Object.keys(persisted.clears.solo).length === 9) break;
    await new Promise((resolve) => setTimeout(resolve, 5));
  }
  assert.equal(Object.keys(persisted.clears.solo).length, 9);
  assert(
    !storage.writes.some(
