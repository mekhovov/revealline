...
async function enter(h) {
  ...
  assert.equal(
    h.$('profile-recovery-open').disabled,
    false,
    h.$('profile-recovery-entry-status').textContent,
  );
...
test('actual title Settings recovery Back and controller Back preserve the title opener and one input loop', async (t) => {
  ...
  h.frame(16);
  pulse(13); // Join with navigation; Confirm would start the default Featured install.
  h.frame(16);
