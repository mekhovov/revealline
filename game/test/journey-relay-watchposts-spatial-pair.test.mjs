...
function closeRoute(run, route) {
  if (route === 'upper-ring-first') {
    ...
    stepUntil(run, 'down', (state) => state.events.some((event) => event.type === 'cut.closed'));
...
test('Relay watchposts pair records two redesign dispositions and preserves historical identities', () => {
  ...
  assert.deepEqual(Object.keys(JOURNEY_RELAY_WATCHPOSTS_SPATIAL_DISPOSITIONS), IDS);
  for (const disposition of Object.values(JOURNEY_RELAY_WATCHPOSTS_SPATIAL_DISPOSITIONS)) {
...
test('nested rings and four watch sectors retain valid governed fields and relay topology', () => {
  ...
  for (const id of IDS) {
    ...
    assert.equal(currentMap.geometry.fieldComponents.length, expected[id].fields);
    ...
    assert(currentMap.geometry.safeComponents.every((item) => item.departures.length >= 60));
...
for (const [id, disposition] of Object.entries(JOURNEY_RELAY_WATCHPOSTS_SPATIAL_DISPOSITIONS))
  for (const difficulty of PRESETS)
...
for (const [route, signature] of Object.entries(relayRoutes)) {
  const id =
    route.includes('ring') || route.includes('inner') ? 'nested-relays' : 'watchpost-exchange';
  for (const difficulty of PRESETS)
    for (const turnPolicy of CONTROLS)
