...
for (const artwork of [false, true])
  test(`successor preserves the other 86 missions, identities, order and artwork: ${artwork}`, () => {
    ...
    const next = createSpatialChallengeCandidates({ artwork, source: freezeDesign(before) });
    ...
    for (const key of ['missions', 'campaigns', 'packs'])
      assert.deepEqual(next[key].map((item) => item.id), before[key].map((item) => item.id));
...
test('supplied Twin revision composes without being overwritten or mutating its owner', () => {
  ...
  assert.deepEqual(next.missions.find((mission) => mission.id === twin.id), twin);
});
...
test('all pilot presets preserve shared rules and identical Solo/Versus boards', () => {
  ...
  for (const difficulty of presets)
    for (const id of SPATIAL_CHALLENGE_MISSION_IDS) {
      ...
      assert.equal(inspectCaptureSnapshot(run).filledCells.length, 0, `${id}/${difficulty}`);
    }
});
...
test('unequal bays each retain enemies and neither bay alone supplies the unchanged quota', () => {
  ...
  assert.deepEqual(snapshot.components.map((component) => component.enemyIds), [
    ['west'],
    ['east', 'east-return'],
  ]);
  assert.deepEqual(snapshot.components.map((component) => component.cells.length), [835, 1407]);
  for (const component of snapshot.components)
    assert(component.cells.length / run.totalClaimable < run.level.goal.coverage);
});
...
test('offset return shelves change departure length without opening either retained bay', () => {
  ...
  assert.equal(at(21, 10), CELL.SAFE, 'Upper west shelf is reclaimed return ground');
  assert.equal(at(21, 25), CELL.FIELD, 'No matching lower west shelf');
  assert.equal(at(35, 25), CELL.SAFE, 'Lower east shelf is reclaimed return ground');
...
test('arrow baffles block later transfers while both opening approaches stay geometrically clear', () => {
  ...
  const at = (x, y) => run.cells[y * run.width + x];
  for (const [x, y] of [[22, 8], [23, 15], [47, 19], [48, 28]])
    assert.equal(at(x, y), CELL.WALL);
  for (const [x, y] of [[22, 7], [22, 16], [47, 18], [47, 29]])
    assert.equal(at(x, y), CELL.FIELD, 'Every staggered wall has a usable open end');
  for (let y = 1; y < 17; y++) {
    assert.equal(at(35, y), CELL.FIELD, 'First fast approach contains no wall or forced turn');
...
test('circuits remain field-connected through three different mouths', () => {
  ...
  const at = (x, y) => run.cells[y * run.width + x];
  for (const [x, y] of [[20, 13], [35, 15], [51, 13]]) assert.equal(at(x, y), CELL.FIELD);
  for (const [x, y] of [[9, 13], [35, 28], [62, 13]]) assert.equal(at(x, y), CELL.SAFE);
});

for (const difficulty of presets)
  for (const turnPolicy of controls)
    test(`protected wall contact never closes a cut or costs a life: ${difficulty}/${turnPolicy}`, () => {
      const run = createRun(tuned('broken-yard', difficulty), { seed: 1, turnPolicy });
      let closures = 0;
...
for (const difficulty of presets)
  for (const turnPolicy of controls)
    test(`arrow teaching crossing ends on a broad landing at current rates: ${difficulty}/${turnPolicy}`, () => {
      const run = createRun(tuned('read-the-arrows', difficulty), { seed: 1, turnPolicy });
      let closed = false;
      for (let tick = 0; tick < 500 && !closed && !run.classic.livesLost; tick++) {
