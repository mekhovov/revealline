...
for (const [name, create] of [
  ...
  test(`${name} report preserves authored facts and projects the actual fresh-attempt recipe`, () => {
    ...
    for (const row of report.rows) {
      ...
      assert(
        effective.actors.every((actor) => actor.domain === "unclaimed-field"),
      );
      assert(effective.actors.every((actor) => actor.retainsField));
      assert.equal(
        effective.population.addedKeepers,
        row.difficulty === "expert" ? 1 : 0,
      );
      assert.equal(
        effective.lives,
        { gentle: 5, standard: 3, expert: 2 }[row.difficulty],
      );
      assert.equal(effective.recipe.adminOverride, false);
      assert(Object.isFrozen(effective.actors[0].runtime));
      assert(
        effective.warnings.some(
          (warning) => warning.code === "projection-not-balance-evidence",
        ),
      );
    }
...
test("explicit admin settings report actual clamped rates without changing the authored edition", () => {
  ...
  assert.equal(
    result.actors.find((actor) => actor.id === "outer").domain,
    "outer-perimeter",
  );
  assert.equal(
    result.actors.find((actor) => actor.id === "outer").retainsField,
    false,
  );
  assert(
    result.warnings.some(
      (warning) => warning.code === "admin-playtest-no-awards",
    ),
  );
  ...
  assert.equal(
    inspectEffectiveGameplay(source, "nearby-shore").recipe.adminOverride,
    false,
  );
});
...
test("density inspection reports skipped additions when the retained chamber has no safe footprint", () => {
  ...
  assert(
    result.warnings.some(
      (warning) => warning.code === "density-target-not-reached",
    ),
  );
});
...
test("boss projection preserves sole field retention and the warning/release contract", () => {
  ...
  assert(
    result.warnings.some(
      (warning) => warning.code === "encounter-roster-preserved",
    ),
  );
});
