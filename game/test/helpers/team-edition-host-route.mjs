...
const fixture = JSON.parse(
  await readFile(new URL('../fixtures/current-team-edition-host-routes.json', import.meta.url)),
);

assert.equal(fixture.format, 'CurrentTeamEditionHostRoutesV1');
...
export function teamEditionRoute(source, missionId, difficulty = 'standard') {
  ...
  if (
    source.id === 'journey-team-timed-review' ||
    source.id === 'journey-team-timed-original-review'
  ) {
    assert.equal(pickups.length, 1);
    assert.equal(pickups[0].partnerCutting, true);
    assert(
      [0, 1].every(
        (seat) =>
          events.filter(
            (event) =>
              event.type === 'cut.closed' && event.reason === 'return' && event.player === seat,
          ).length >= 2,
      ),
    );
  }
...
export function playTeamEditionRoute(
  f,
  source,
  missionId,
  difficulty = 'standard',
  ...
  const reference = teamEditionRoute(source, missionId, difficulty);
