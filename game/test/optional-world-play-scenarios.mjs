...
// Run each independent real-app journey in a separate Node test process.
// Preserve every scenario and assert global fetch ownership after page cleanup.
const scenarios = [];
  ...
  class Picture {
    ...
    set src(value) {
      ...
      if (value.startsWith('blob:')) {
        // Object URLs are created by the real media owner. Read their actual
        // PNG header; a 1x1 fixture must not masquerade as a release frame.
        void fetchOutsidePages(value)
...
test('modeled Settings and return during download retire launch intent before any picture ticket exists', async (t) => {
  ...
  assert.equal(returnFocus, p.$('optional-worlds-cancel'));
  assert.equal(
    returnFocus.hidden,
    false,
    'Settings returns to the still-running cancellation action.',
  );
  gate.resolve();
  await pending;
  kept(p, before);
  assert.ok(installedPack(h), 'A completed background installation remains usable.');
  assert.equal(returnFocus.hidden, true, 'The completed operation no longer exposes Cancel.');
  assert.equal(
    p.doc.activeElement,
    p.$(primaryId),
    'Finishing an owned Cancel lease returns to the enabled card instead of hiding focus.',
  );
  assert.equal(p.$('optional-worlds-dialog').open, true);
  assert.equal(p.$(primaryId).disabled, false);
  assert.deepEqual(p.errors, []);
...
for (const outcome of ['success', 'failure'])
  test(`unassigned embedded original ${outcome} is fully decoded before the old attempt can change`, async (t) => {
    // This is an imported authored edition, deliberately outside the approved
    // release-owner table. Shipped unassigned artwork has a separate test.
    const imported = structuredClone(pack);
    imported.id = 'world-play-authored-import';
    imported.campaigns[0].id = 'world-play-authored-campaign';
    const { pack: authored } = await preparePack(JSON.stringify(imported), {
      decodeImage: async (source) => dimensions(source),
    });
    const h = await setup(t, {
        installed: true,
        installedPacks: [authored],
        destinationPicture: false,
      }),
      { p } = h;
    await openWorlds(p);
    const button = p.$(`optional-worlds-installed-choose-${authored.id}`);
    assert.ok(button, 'The imported edition has an ordinary Play card.');
    for (let i = 0; button.closest('[hidden]') && i < 20; i++) {
      assert.equal(p.$('optional-worlds-next').disabled, false);
      p.$('optional-worlds-next').click();
    }
    ...
    const before = snapshot(p),
      gate = deferred();
    t.after(gate.resolve);
    const source = authored.levelVisuals[0].visualOverrides.background.dataUrl;
    let drawable;
    ...
    try {
      ...
      error.message += ` ${JSON.stringify({
        status: p.$('optional-worlds-status').textContent,
        pictureState: p.doc.body.dataset.pictureState,
        flightState: p.doc.body.dataset.flightState,
        button: { disabled: button.disabled, text: button.textContent },
        images: h.images.map((image) => ({
          sourceKind: image.url?.slice(0, 40),
          width: image.width,
          height: image.height,
    ...
    assert.equal(before.image.releases, 1);
    assert.equal(p.$('pack-select').value, authored.id);
    assert.equal(h.requests.filter((url) => url.endsWith(chapter.path)).length, 0);
    assert.deepEqual(p.errors, []);
  });

export const optionalWorldPlayScenarios = Object.freeze(scenarios);
export function assertWorldPlayFetchRestored() {
  assert.equal(globalThis.fetch, fetchOutsidePages);
}

export async function approvedReleasePictureScenario(t) {
  const h = await setup(t, { installed: true, destinationPicture: false }),
    { p } = h;
  const compiled = await json('game/presentation/compiled/runtime.json');
  const level = destination.campaign.levels[0];
  const approved = releasePictureForIdentity(compiled, {
    baseCampaignKey: campaignKey(destination.campaign),
    levelId: level.id,
    levelRevision: level.revision,
    themeId: 'fpv',
  });
  assert.ok(approved, 'This shipped owner has an exact approved release picture.');
  const button = await openWorlds(p),
    before = snapshot(p);
  await activate(button);
  await running(p);
  assert.equal(p.rendered.backdrop.pin.kind, 'still');
  assert.equal(p.rendered.backdrop.pin.sha256, approved.asset.file.sha256);
  assert.equal(p.rendered.backdrop.image.width, approved.asset.file.width);
  assert.equal(p.rendered.backdrop.image.height, approved.asset.file.height);
  assert.equal(p.rendered.backdrop.image.releases, 0);
  assert.equal(before.image.releases, 1);
  assert.equal(p.$('pack-select').value, chapter.id);
  assert.equal(h.requests.filter((url) => url.endsWith(chapter.path)).length, 0);
  assert.deepEqual(p.errors, []);
