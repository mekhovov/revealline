...
async function setup(t, options = {}) {
  // Model native dialog opening/focus restoration, as in modal-navigation hosts.
...
function openGuide(page) {
  ...
  assert.equal(page.$('shell-workshop-dialog').open, true, 'Workshop keeps the Guide opener');
...
for (const turnPolicy of ['immediate', 'grid-center']) {
  test(`${turnPolicy}: actual guide preserves a live cut, Large text preference and held-input isolation through practice and return`, async (t) => {
    ...
    assert.equal(
      ...
      false,
      'Back returns from Workshop to Main menu',
    );
    assert.equal(page.$('shell-home').open, true);
