...
test('integrated67 retains complete58–66 picture and actor records and original bytes', async () => {
  const retained = [58, 59, 60, 61, 62, 63, 64, 65, 66];
  assert.equal(bundle.document.selection.theme.revision, 67);
  assert.deepEqual(
    COOP_SUPPORTED_PICTURE_BINDINGS.map((row) => row.themeRevision),
    [67, 67, ...retained.flatMap((revision) => [revision, revision])],
  );
  assert.deepEqual(
    COOP_RETAINED_PICTURE_BINDINGS,
  ...
  for (const slot of slots) {
    ...
    for (const revision of retained)
      assert.deepEqual(asset, snapshot(revision).resolved.assets[slot], `retained${revision} ${slot}`);
...
for (const revision of [58, 59, 60, 61, 62, 63, 64, 65, 66, 67]) {
...
test('finite host authority rejects54,57,68, other themes and collections before reads', async () => {
...
test('historical picture authority remains bounded to ten exact distinct revisions', () => {
  const policies = [58, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68].map((themeRevision) => ({
    ...COOP_RETAINED_HISTORICAL_IMPORT_PICTURE_POLICY,
    themeRevision,
  }));
  assert.throws(() => fixture(67, imported, imported.levels[0].id, policies), /item budget/);
});
