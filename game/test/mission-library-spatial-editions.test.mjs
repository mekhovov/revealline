...
const originalThemes = JSON.parse(
  await readFile(
    new URL("../content-design/themes.json", import.meta.url),
    "utf8",
  ),
).themes;
const revised = [
...
for (const [activeRouteId, alternateRouteId, label] of [
  ...
  [
    "whole-spatial-v5",
    "whole-spatial-v6",
    "Spatial challenge · balance pending",
  ],
])
  test(`${activeRouteId} offers only the six alternate editions with exact receiver identities and no media reads`, async (t) => {
    ...
    assert.deepEqual(
      library.missions.map(sourceId).sort(),
      [...revised].sort(),
    );
    ...
    const host = createCandidateVersusHost(route.source, {
      themes: journeyActorThemeCandidates(originalThemes, {
    ...
    for (const row of library.missions) {
      ...
      assert.equal(
        received.levelIndex,
        row.levelIndex,
        "Preserve full campaign position",
      );
      assert.deepEqual(
        library.card(row, "versus"),
        host.card(host.catalog.find(row.runtimeId)),
      );
    ...
    const twoBays = library.missions.find(
      (row) => sourceId(row) === "two-bays",
    );
...
test("alternate owner preserves handoff guards and scoped progress, rejects forged bindings and retires cleanly", async () => {
  ...
  assert.equal(
    received,
    context,
    "Preserve exact handoff object and callback ownership",
  );
  assert.equal(
    source.launch(source.entries[0], { ...context, isCurrent: () => false }),
    false,
  );
  assert.throws(
    () =>
      source.launch(source.entries[0], {
        ...context,
        libraryMissionId: "wrong-edition",
      }),
    /another display mission/,
  );
  assert.throws(
    () => source.launch({}, context),
    /Unknown Journey display binding/,
  );
  assert.throws(
...
for (const activeRouteId of ["whole-spatial-v6", "whole-spatial-v5"])
  test(`${activeRouteId} keeps same-name editions manually playable without looping at Journey completion`, async (t) => {
    ...
    for (const mission of host.catalog.missions) {
      const editions = library.missions.filter(
        (row) => row.runtimeId === mission.id,
      );
    ...
    const finalMission = library.missions.find(
      (row) =>
        row.editionId === activeRouteId &&
        row.runtimeId === host.catalog.missions.at(-1).id,
    );
