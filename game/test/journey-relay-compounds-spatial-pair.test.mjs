...
test("Relay compounds pair records two redesign dispositions and preserves historical identities", () => {
  assert.deepEqual(
    createJourneyRelaySpatialPairCandidates({ artwork: true }),
    oldSnapshot,
  );
  assert.deepEqual(
    Object.keys(JOURNEY_RELAY_COMPOUNDS_SPATIAL_DISPOSITIONS),
    IDS,
  );
...
for (const [route, signature] of Object.entries(relayRoutes)) {
  const id =
    route === "upper-relay-first" ? "three-compounds" : "spiral-stores";
  for (const difficulty of PRESETS)
    for (const turnPolicy of CONTROLS)
