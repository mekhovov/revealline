...
import { inspectNeonGoal } from './helpers/neon-goal.mjs';
...
const supplemental = JSON.parse(
  await readFile(new URL('./fixtures/neon-mastery-routes.json', import.meta.url)),
);
...
function replayGoal(row) {
  ...
  const recorder = createRecorder(
    manifest.level,
    options,
    'neon-optional-goal-feasibility-not-human-validation',
  );
  ...
  for (const [direction, ticks] of row.segments) {
    ...
    for (let tick = 0; tick < ticks; tick++) {
      ...
      if (run.claimedCount > before) closedTrails.push(trail);
    }
  }
...
test('every Neon optional goal has a pinned no-loss clear in all presets and steering settings', () => {
  assert.equal(supplemental.format, 'NeonMasteryFeasibilityV1');
  assert.equal(supplemental.rows.length, 21);
  const supplements = new Map(supplemental.rows.map((row) => [key(row), row]));
  assert.equal(supplements.size, 21);
  let checked = 0;
