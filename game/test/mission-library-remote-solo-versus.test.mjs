...
test('remote Solo/Versus inventory preserves 91 current, twelve prior and 188 Classic identities without images or saving', async (t) => {
  const reads = [],
    launches = [];
  ...
  assert.equal(library.missions.length, 291);
  assert.equal(library.forMode('solo').length, 291);
  assert.equal(library.forMode('versus').length, 291);
  assert.equal(library.forMode('team').length, 0);
  const route = await loadAuthoredJourneyRoute('whole-spatial-v13');
  ...
  for (const mode of ['solo', 'versus']) {
    ...
    assert.equal(
      ready.length,
      115,
      '91 current + twelve prior Journey +12 verified Base, not invented installed packs',
    );
