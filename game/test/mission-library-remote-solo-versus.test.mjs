...
test('remote Solo/Versus inventory preserves 91 current, twenty-four prior and 188 Classic identities without images or saving', async (t) => {
  const reads = [],
    launches = [];
  ...
  assert.equal(library.missions.length, 303);
  assert.equal(library.forMode('solo').length, 303);
  assert.equal(library.forMode('versus').length, 303);
  assert.equal(library.forMode('team').length, 0);
  const route = await loadAuthoredJourneyRoute('whole-spatial-v17');
  ...
  for (const mode of ['solo', 'versus']) {
    ...
    assert.equal(
      ready.length,
      127,
      '91 current + twenty-four prior Journey +12 verified Base, not invented installed packs',
    );
