...
test('remote Solo/Versus inventory preserves 91 current, twenty-seven prior and 188 Classic identities without images or saving', async (t) => {
  const reads = [],
    launches = [];
  ...
  assert.equal(library.missions.length, 306);
  assert.equal(library.forMode('solo').length, 306);
  assert.equal(library.forMode('versus').length, 306);
  assert.equal(library.forMode('team').length, 0);
  const route = await loadAuthoredJourneyRoute('whole-spatial-v18');
  ...
  for (const mode of ['solo', 'versus']) {
    ...
    assert.equal(
      ready.length,
      130,
      '91 current + twenty-seven prior Journey +12 verified Base, not invented installed packs',
    );
