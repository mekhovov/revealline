...
test('remote Solo/Versus inventory preserves 91 current, forty-two prior and 188 Classic identities without images or saving', async (t) => {
  const reads = [],
    launches = [];
  ...
  assert.equal(library.missions.length, 321);
  assert.equal(library.forMode('solo').length, 321);
  assert.equal(library.forMode('versus').length, 321);
  assert.equal(library.forMode('team').length, 0);
  const route = await loadAuthoredJourneyRoute('whole-spatial-v23');
