...
test('remote Solo/Versus inventory preserves 91 current, eighteen prior and 188 Classic identities without images or saving', async (t) => {
  const reads = [],
    launches = [];
  ...
  assert.equal(library.missions.length, 297);
  assert.equal(library.forMode('solo').length, 297);
  assert.equal(library.forMode('versus').length, 297);
  assert.equal(library.forMode('team').length, 0);
  const route = await loadAuthoredJourneyRoute('whole-spatial-v15');
  ...
  for (const mode of ['solo', 'versus']) {
    ...
    assert.equal(
      ready.length,
      121,
      '91 current + eighteen prior Journey +12 verified Base, not invented installed packs',
    );
