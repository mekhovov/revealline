...
test('remote Solo/Versus inventory preserves 91 current, fifteen prior and 188 Classic identities without images or saving', async (t) => {
  const reads = [],
    launches = [];
  ...
  assert.equal(library.missions.length, 294);
  assert.equal(library.forMode('solo').length, 294);
  assert.equal(library.forMode('versus').length, 294);
  assert.equal(library.forMode('team').length, 0);
  const route = await loadAuthoredJourneyRoute('whole-spatial-v14');
  ...
  for (const mode of ['solo', 'versus']) {
    ...
    assert.equal(
      ready.length,
      118,
      '91 current + fifteen prior Journey +12 verified Base, not invented installed packs',
    );
