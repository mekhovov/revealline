...
test('Crosswind clear fixtures cover every candidate, preset and steering pair', () => {
  assert.equal(fixture.format, 'CrosswindFeasibilityRoutesV1');
  assert.deepEqual(
...
for (const { difficulty, turnPolicy, rows } of fixture.sets) {
  test(`Crosswind Array prepared no-loss Solo clears, useful pre-victory relays and public replay: ${difficulty}/${turnPolicy}`, async (t) => {
    const preparer = createContentAttemptPreparer(createCrosswindCandidates(), { themes });
    t.after(() => preparer.dispose());
    const journey = preparer.catalog.journey(),
      sequence = createCandidateSequence(journey, ['journey-crosswind']);
    assert.equal(sequence.next(journey.missions.find((m) => m.levelId === 'long-wave').id), null);
    assert.equal(
      sequence.isCore(journey.missions.find((m) => m.levelId === 'crosswind-remix').id),
      false,
    );
    for (const [id, identity, checkpoint, segments] of rows) {
      ...
      assert.equal(prepared.run.ruleset, 'xonix-core.v8');
      assert.deepEqual(
      ...
      for (const [direction, ticks] of segments) {
        ...
        for (let tick = 0; tick < ticks; tick++) {
          ...
          const index = Math.floor(from.y) * run.width + Math.floor(from.x);
          const active =
            run.cells[index] === CELL.FIELD &&
            run.level.directionalFields.zones.some(
              (zone) =>
                from.x >= zone.x &&
                from.x < zone.x + zone.w &&
                from.y >= zone.y &&
                from.y < zone.y + zone.h,
            );
          recordInput(recorder, { direction });
          stepRun(run, { direction }, FIXED_DT);
          if (active && (run.player.x !== from.x || run.player.y !== from.y)) markedMovement++;
      ...
      assert(markedMovement > 0, `${id}: no actual marked-field movement`);
      ...
      assert(!opened.length || used.size > 0, `${id}: no opened connector traversed`);
      for (const gate of run.relay.gates)
  ...
  test(`Crosswind Array independent equal paired-board clears: ${difficulty}/${turnPolicy}`, () => {
