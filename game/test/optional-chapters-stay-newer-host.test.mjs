...
const scenario = optionalChapterScenarios.find(
  (item) =>
    item.name ===
    'More worlds early Stay restores its exact Play; late verification respects newer operation',
);
test(scenario.name, scenario.run);
test(
  'Closed chapter page restores the outer fetch after stay-newer-host',
  assertOptionalFetchRestored,
);
