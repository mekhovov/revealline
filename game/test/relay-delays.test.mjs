...
const fixture = JSON.parse(
  await readFile(new URL('./fixtures/relay-delay-routes.json', import.meta.url)),
);
test('relay delayed routes cover five waits and every candidate', () => {
  assert.equal(fixture.format, 'RelayDelayedRoutesV1');
...
for (const { delaySeconds, rows } of fixture.sets)
  test(`Relay Labyrinth no-loss full clears after ${delaySeconds}s idle: Standard/immediate`, () => {
    for (const [id, identity, checkpoint, segments] of rows) {
      ...
      assert(used.size > 0, `${id}: no connector used`);
      assert(
