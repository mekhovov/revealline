...
test('catalog requires all eight exact code-owned descriptors; foreign metadata and accessor authority refuse', async () => {
  assert.equal(
    prepareExternalCatalog(
  ...
  assert.equal(EXTERNAL_CATALOG.chapters.length, 8);
  assert.equal(EXTERNAL_CATALOG.chapters.at(-1).id, 'sentinel-circuit-coupa');
  for (const mutate of [
