...
const proof = JSON.parse(
  await readFile(new URL('../../authoring/library/countercurrent/routes.json', import.meta.url)),
);
...
async function open(p) {
  ...
  await waitInventory(p, 'Open More worlds and authenticate installed Countercurrent originals', {
...
function showCard(p, e) {
  ...
  filter.onchange();
  for (let n = 0; p.$(id(e, 'card')).hidden && n < 9; n++) {
