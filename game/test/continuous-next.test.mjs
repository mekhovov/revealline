...
test("Next crosses campaigns, packs and collections without using filters or completion", () => {
  const library = createMissionLibrary([
    ...
    source('["custom","mine"]', "Custom", [
      { id: "a", campaignKey: "custom@1" },
    ]),
  ]);
...
test("Next stays in the current mode and retains an unavailable successor for explicit retry", () => {
  ...
  assert.equal(
    librarySuccessor(library, library.missions[0], "solo"),
    library.missions[2],
  );
  assert.throws(
    () => librarySuccessor(library, library.missions[0], "team"),
    /mode/,
  );
...
test("Next skips browse-only editions without removing manual selection or changing Classic continuation", () => {
  ...
  const [current, previous, previousLast, classic, classicLast, custom] =
    library.missions;
  ...
  assert.equal(
    library.search("Same name").length,
    6,
    "Browse-only missions stay discoverable",
  );
});
...
test("stale, absent and ambiguous owners never silently launch a same-name replacement", () => {
  const owner = source('["classic","base",null]', "Classic", [
    { id: "a", campaignKey: "base@1" },
  ]);
  const library = createMissionLibrary([owner]);
  ...
  library.register(
    source('["classic","archived",null]', "Classic", [
      { id: "a", campaignKey: "base@1" },
    ]),
  );
  assert.throws(
