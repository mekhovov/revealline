...
const editions = [
  {
    routeId: 'whole-spatial-v10',
    priorId: 'whole-spatial-v9',
    version: 9,
    revised: ['stepping-stones', 'return-pocket', 'neutral-ground'],
  },
  {
    routeId: 'whole-spatial-v11',
    priorId: 'whole-spatial-v10',
    version: 10,
    revised: ['island-outpost', 'long-way-home', 'horizon-remix'],
  },
];

for (const { routeId, priorId, version, revised } of editions)
  test(`${routeId} and manually selectable prior editions translate without replacing their owners`, async (context) => {
    ...
    for (const id of revised) {
      ...
      for (const row of editions) {
        ...
        if (row.editionId === priorId) {
          assert.equal(display.edition, `Попередня Подорож · v${version}`);
    ...
    for (const row of library.missions.filter((item) => item.editionId === priorId))
      assert.equal(library.presentation(row).edition, `Previous Journey · v${version}`);
  });
