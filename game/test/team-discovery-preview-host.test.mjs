...
async function fixture(t, extra = {}) {
  ...
  const f = await page(t, {
    ...
    beforeImport({ $, doc, install }) {
      ...
      assert.ok(canvas, 'Authored full preview Canvas is present');
...
function assertFull(f, hash) {
  ...
  for (const [x, y] of [
    ...
    assert.equal(pixelAt(trace, x, y), draw.args[0], 'Full preview shows the complete original');
}
function retained(f) {
  return {
    hud: [
...
test('Team cards show exact concealed teasers; explicit full preview returns through both Back destinations without awards', async (t) => {
  ...
  await ready(f);
  assertFull(f, yard.picture.sha256);
...
test('a newer Back focus survives full preview readiness without automatic navigation', async (t) => {
  const gate = deferred();
  let hold = false,
    held = false;
  const f = await fixture(t, {
...
test('a cancelled old visit cannot overwrite a newer full preview or its focus', async (t) => {
  const gate = deferred();
  let hold = false,
    held = false;
  const f = await fixture(t, {
  ...
  gate.resolve();
  await ready(f);
  assertFull(f, first.picture.sha256);
  assert.equal(f.doc.activeElement.id, 'coop-discovery-back');
  assert.equal(f.drawImages.length, 0);
  assert.equal(f.$('coop-level').value, 'first-connection');
});
