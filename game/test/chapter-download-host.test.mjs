...
const r3 = await load('fpv-arcade-r3');
...
test('featured download failure is visible inside the retained title and its button can deliberately retry', async (t) => {
  ...
  let available = false,
    requests = 0;
  chapterFetch(t, async (url, request) => {
    if (url === 'content/packs/fpv-arcade-r3.json') {
      requests++;
      if (!available) throw new TypeError('Failed to fetch');
    }
    return request();
  ...
  assert.match(status.textContent, /First Light R3/);
  ...
  assert.equal(page.$('pack-select').value, r3.id);
  assert.deepEqual(page.rendered.run.level, normalizedLevel(r3.campaigns[0].levels[0]));
  assert.deepEqual(page.errors, []);
});
