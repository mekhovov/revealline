...
test('Fractured Grid full-clear fixtures cover all seven missions and six preset/control pairs', () => {
  assert.equal(fixture.format, 'FractureFeasibilityRoutesV1');
  assert.deepEqual(
...
for (const { difficulty, turnPolicy, rows } of fixture.sets) {
  test(`Fractured Grid legal Solo clears, no quota cleanup, and public replays: ${difficulty}/${turnPolicy}`, () => {
    for (const [id, identity, checkpoint, segments] of rows) {
      ...
      for (const [direction, ticks] of segments) {
        ...
        for (let tick = 0; tick < ticks; tick++) {
          ...
          if (run.events.some((event) => event.type === 'cells.eroded')) erosions++;
      ...
      if (['first-fracture', 'island-reserve', 'two-districts'].includes(id))
        assert(erosions > 0, `${id}: erosion lesson must actually occur`);
  ...
  test(`Fractured Grid equal independent untimed paired-board clears: ${difficulty}/${turnPolicy}`, () => {
