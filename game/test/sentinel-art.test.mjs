...
test('all five Sentinel originals have exact unique pins and verified opt-in previews', async () => {
  const source = createSentinelCandidates({ artwork: true });
  const project = compileContentProject(source);
  const grey = compileContentProject(createSentinelCandidates());
  const theme = JSON.parse(
    await readFile(new URL('../content-design/themes.json', import.meta.url)),
  ).themes.find((row) => row.id === 'horizon');
  assert.equal(SENTINEL_ART_CANDIDATES.length, 5);
  assert.equal(
    new Set(JOURNEY_ART_CANDIDATES.map((asset) => asset.sha256)).size,
    JOURNEY_ART_CANDIDATES.length,
  );
  for (const asset of SENTINEL_ART_CANDIDATES) {
    ...
    assert.equal(asset.id, `sentinel-citadel-${consumers[0].id}`);
  ...
  const second = createSentinelCandidates({ artwork: true });
  second.assets[0].alt = 'Local edit';
  assert.notEqual(second.assets[0].alt, SENTINEL_ART_CANDIDATES[0].alt);
});

test('Sentinel prompts and pending reviews correspond to the original pins', async () => {
  const record = JSON.parse(
    await readFile(
      new URL('../../docs/research/sentinel-original-art-prompts.json', import.meta.url),
    ),
  );
  assert.equal(record.tool, 'built-in-imagegen');
  assert.equal(record.assets.length, SENTINEL_ART_CANDIDATES.length);
  assert.equal(new Set(record.assets.map((entry) => entry.missionId)).size, 5);
  for (const entry of record.assets) {
    const asset = SENTINEL_ART_CANDIDATES.find((row) => `game/${row.path}` === entry.path);
    assert(asset);
...
test('Sentinel pictured edition preserves all30 historical clear checkpoints', async () => {
  const fixture = JSON.parse(
    await readFile(new URL('./fixtures/sentinel-clear-routes.json', import.meta.url)),
  );
  const project = compileContentProject(createSentinelCandidates({ artwork: true }));
  let checked = 0;
...
test('Studio inspects pictured Sentinel candidates without bypassing explicit Apply', async () => {
  const studio = await readFile(new URL('../studio/studio.mjs', import.meta.url), 'utf8');
  const handler = studio.match(
    /\$\('sentinel'\)\.onclick = guarded\(\(\) => \{([\s\S]*?)\n\}\);/,
  )[1];
  assert.match(handler, /discardSource\(\)/);
  assert.match(handler, /createSentinelCandidates\(\{ artwork: true \}\)/);
