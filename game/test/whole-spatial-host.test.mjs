...
const routeId = 'whole-spatial-v1';
const profileKey = 'journey-whole-spatial-v1';
const sessionKey = 'revealline.suspended.journey-whole-spatial.v1';
...
test('spatial Solo ignores historical progress, clears and skips in its namespace, then restores exact unfinished flight', async (t) => {
  ...
  await t.test('first clear, Next, two-activation Skip, scoped save and mode link', async (t) => {
    ...
    await settle(() =>
      globalThis.location.href.includes('couch/?return=solo&journey=whole-spatial-v1'),
    );
...
test('spatial Versus keeps equal owned boards, Next/Skip and matching Solo destination without legacy progress', async (t) => {
  const disk = managedIndexedDB();
  const backend = createJourneyBackend({ ...disk, profileKey });
