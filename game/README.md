...
Run `npm run dev` from the repository root and open [solo play](http://127.0.0.1:8768/game/). The page uses local Phaser 4.2.1 and a browser-independent [simulation core](core/README.md). The latest frozen release is [v0.15.0](http://127.0.0.1:8767/releases/v0.15.0/site/game/), with 29 maps: 12 base campaign maps and 17 optional expansion maps across Night Shift, Living Threads, Fieldcraft, the illustrated Homeward Skies chapter, Equipment Workshop and Sentinel Relay. It retains optional Toggle controller Boost with Hold as the default. All eighteen earlier releases remain independently preserved.
...
The **v0.15.0 frozen release** gives solo flight buttons a consistent 44px minimum, keeps short-landscape controls beside the board in normal page flow, and wraps long authored labels. Playground's manual **Control geometry** capture helps authors compare target rectangles, spacing and actual viewport conditions. See [Flight controls](../docs/flight-controls.md). Normal vertical scrolling may be needed; input bindings and simulation remain unchanged.

The **v0.16.0 working source** implements the optional [First Flight course](../docs/first-flight.md): **Close a line**, **Find the empty side** and **Bring the picture home**, with a checked saved-flight handoff and actual Playground/Controller Lab previews. Course progress and settings stay within the visit and grant no campaign rewards. Source browser play completed all three lessons with Immediate steering and verified saved-flight return; final source gates and a frozen v0.16.0 artifact remain pending. The latest independently frozen release is still v0.15.0.
