...
Run `npm run dev` from the repository root and open [solo play](http://127.0.0.1:8768/game/). The page uses local Phaser 4.2.1 and a browser-independent [simulation core](core/README.md). The latest frozen release is [v0.16.0](http://127.0.0.1:8767/releases/v0.16.0/site/game/), with 29 maps: 12 base campaign maps and 17 optional expansion maps across Night Shift, Living Threads, Fieldcraft, the illustrated Homeward Skies chapter, Equipment Workshop and Sentinel Relay. It retains optional Toggle controller Boost with Hold as the default. All nineteen earlier releases remain independently preserved. Three optional First Flight lessons are separate from those campaign maps.
...
The **v0.16.0 frozen release**, source `8fb6b5782a6fee85211b9bbbfb3d13a1d1a60c43`, adds the optional [First Flight course](../docs/first-flight.md): **Close a line**, **Find the empty side** and **Bring the picture home**, with a checked saved-flight handoff and actual Playground/Controller Lab previews. Course progress and settings stay within the visit and grant no campaign rewards. [Source browser journeys](../docs/verification/round-26/source-browser.md) completed all three lessons in both steering modes and checked retained-flight return; [frozen browser checks](../docs/verification/round-26/frozen-browser.md) record the saved build's offline behavior separately.
...
The frozen v0.16.0 archive [rebuilds all 142 output files identically](../docs/verification/round-26/integrity-notes.md), with 135 offline files totaling 20,624,650 bytes. The [six-gate run](../docs/verification/round-26/source-gates.md) passed **1,521 unique tests** before the three-file landing-label correction; the [follow-up](../docs/verification/round-26/source-label-followup.md) passed 23 existing CLI tests plus lint, formatting and validation on the final source. Those tests are a scoped rerun, not 23 additional unique cases. The [frozen browser report](../docs/verification/round-26/frozen-browser.md) records a first offline Grid course win, byte-identical post-entry/returned backups, resumed ordinary play and a compact offline Playground lesson win. Its measured Ready layout required scrolling for Pause, Restart and Sound; one Next pointer attempt did not advance, while Enter did. The [source report](../docs/verification/round-26/source-browser.md) retains the approximately four-pixel compact-board clipping and unexplained embedded Settings pointer observations; physical inputs and native execution remain separate checks.
