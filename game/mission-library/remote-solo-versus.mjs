...
} from "./journey-presentation.mjs";
import {
  classicLibrarySources,
  prepareMissionLibraryIndex,
} from "./classic-source.mjs";
...
export async function createRemoteSoloVersusLibrarySources({
  ...
  if (typeof launch !== "function" || typeof request !== "function")
    throw new TypeError(
      "Remote browsing needs a metadata reader and exact handoff.",
    );
  const root = new URL(baseURL);
  if (
    !["http:", "https:", "file:"].includes(root.protocol) ||
    root.username ||
    root.password
  )
    throw new TypeError(
      "Remote browsing needs the fixed same-game content root.",
    );
  const check = () => {
    if (signal?.aborted)
  ...
  async function read(path, maxBytes = 512 * 1024) {
    ...
    if (!response.ok)
      throw new Error(`Mission metadata is unavailable (${response.status}).`);
  ...
  const [route, indexFile, themeFile, campaignFile, classesFile] =
    await Promise.all([
  ...
  try {
    ...
    const sourceFor = (host, manifest) => ({
      ...journeyLibrarySource({
        ...
        details: (mission) =>
          journeyMissionDetails(manifest(mission, difficulty())),
        tags: (mission) =>
          authoredJourneyMissionTags(mission, manifest(mission, "standard")),
    ...
    const journey = combineJourneyLibrarySources([
      ...
      {
        mode: "versus",
        source: sourceFor(versus, (mission, preset) =>
          versus.manifest(mission, preset),
        ),
      },
    ]);
    const baseCampaign = {
    ...
    for (const row of index.missions.filter((item) => item.source === "base")) {
      ...
      if (
        ...
        throw new Error(
          "Base mission metadata differs from the retained source edition.",
        );
      const modes = new Set();
      const config = {
        classRecipes: classesFile.value,
        classId: classesFile.value[0].id,
      };
    ...
    if (installed) {
      ...
      const owner = await installedSources({
        ...
        installed,
        loadBaseThemes: async () =>
          (await read("content/themes.json")).value.themes,
    ...
    return Object.freeze({
      ...
      state: () => ({
        ready: false,
        reason: "Installed chapters have not been checked.",
      }),
      dispose() {
        spatialEditions.dispose();
        solo.preparer.dispose();
...
async function installedSources({
  ...
  const empty = await inspectPackLibraryMetadata({
  ...
  function proofCurrent(row) {
    ...
    return Boolean(
      proof &&
        proof.epoch === refreshEpoch &&
        proof.inventory === currentInventory(),
    );
  }
  async function confirm(row, context) {
    ...
    if (row?.source === "external") {
      const ready = await getInstaller().inspectExternal(row, {
        signal: context.signal,
      });
      check(context.signal);
      if (!ready.ready)
        throw new Error(ready.reason || "Original pictures need preparation.");
      await inventory.confirm(context.inventory, { signal: context.signal });
    }
  ...
  try {
    ...
    factory = await createMetadataInstalledMissionLibrary({
      ...
      availabilityClassic(row, pack) {
        if (!state().ready)
          return { state: "unavailable", reason: state().reason };
      ...
      async prepareClassic(row, context) {
        ...
        if (["bundled", "archived"].includes(row.source))
          result = await getInstaller().installIndexed(row, {
            signal: context.signal,
          });
          ...
          const summary = catalog.packs.find(
            (entry) => entry.id === row.packId,
          );
          if (!summary)
            throw new Error(
              "This chapter is absent from the exact release catalogue.",
            );
          result = await getInstaller().install(summary, {
            signal: context.signal,
          });
        } else if (row.source === "external")
          result = before.packs.some((pack) => pack.id === row.packId)
            ? await getInstaller().inspectExternal(row, {
                signal: context.signal,
              })
            : await getInstaller().installExternal(row, {
                signal: context.signal,
              });
        ...
        if (!result.committed && epoch !== refreshEpoch)
          throw new Error(
            "Installed inventory changed while preparing. Retry this mission.",
          );
        if (!result.committed)
          await inventory.confirm(before, { signal: context.signal });
        if (!state().ready) throw new Error(state().reason);
        if (row.source === "external") {
          if (!result.ready)
            throw new Error(
              result.reason || "Original pictures need checking.",
            );
          externalProofs.set(row.packId, {
