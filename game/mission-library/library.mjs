...
export function libraryMissionId({
  owner,
  edition,
  campaign,
  mission,
...
function readiness(value) {
  ...
  if (
    ...
    throw new TypeError(
      "A downloadable mission needs its exact positive byte size.",
    );
  if (value.state === "unavailable")
    text(value.reason, "an unavailable reason");
...
export function createMissionLibrary(sources = []) {
  ...
  function requireRow(row, mode) {
    ...
    if (disposed || !binding || owners.get(binding.owner.id) !== binding.owner)
      throw new Error(
        "This mission selection is stale. Refresh the library and try again.",
      );
    if (
      mode !== undefined &&
      (!LIBRARY_MODES.includes(mode) || !row.modes.includes(mode))
    )
  ...
  function cancelOwner(owner) {
    for (const row of owner.rows) {
      for (const controller of pending.get(row)?.values() ?? [])
        controller.abort();
      failures.delete(row);
    }
  }
  function register(source) {
    ...
    if (
      ...
      throw new TypeError(
        "Mission source automatic continuation must be a boolean.",
      );
    if (
      ...
      throw new TypeError(
        "A mission source needs collection, entries and owner adapters.",
      );
    ...
    for (const entry of source.entries) {
      ...
      if (ids.has(id))
        throw new TypeError("A source contains a duplicate mission identity.");
      ids.add(id);
      if (
      ...
      if (
        ...
        throw new TypeError(
          "A mission has an invalid or conflicting collection tag.",
        );
    ...
    if (count > 4096)
      throw new TypeError("The mission library contains too many missions.");
  ...
  function availability(row, mode = "solo") {
    ...
    if (scoped(pending, row, mode))
      return Object.freeze({ state: "preparing" });
  ...
  return Object.freeze({
    ...
    forMode(mode) {
      if (!LIBRARY_MODES.includes(mode))
        throw new TypeError("Unknown library mode.");
      return rows.filter((row) => row.modes.includes(mode));
    },
    search(
      query = "",
      { mode = "solo", collection = "", campaign = "", tag = "" } = {},
    ) {
      if (
    ...
    async prepare(row, { mode = "solo", signal } = {}) {
      ...
      if (state.state === "preparing")
        throw new Error("This mission is already preparing.");
      if (
      ...
      try {
        ...
        if (scoped(pending, row, mode) === controller)
          clearScoped(pending, row, mode);
        emit();
      }
    },
    cancel(row, { mode } = {}) {
