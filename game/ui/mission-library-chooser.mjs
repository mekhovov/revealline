...
export function attachMissionLibraryChooser({
  ...
  function render() {
    ...
    localizedText(status, () =>`${matches.length} mission${matches.length === 1 ? '' : 's'} · ${modeLabel(modeFilter.value)}${message ? ` · ${message}` : ''}`);
    const filtersActive = !!collection.value || !!campaign.value || modeFilter.value !== mode;
    localizedText(filterSummary, () =>filtersActive ? t("interface:filtersActive") : t("interface:filters"));
