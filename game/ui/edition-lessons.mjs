...
export async function mountEditionLessons({
  ...
  function openLesson(lesson, run = null) {
    ...
    workbench = mountCompanyWorkbench(content, {
      ...
      onChange: (next) => {
        ...
        void proofs
          .prove({ attempt: next, replay })
          .then((proof) => {
            if (disposed) return;
            if (!proofs.saveVerified(proof))
              report(
  ...
  upload.onchange = async () => {
    try {
      ...
      const backup = boundedJSON(await file.text(), {
