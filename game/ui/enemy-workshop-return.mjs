...
export function attachEnemyWorkshopReturnHost({
  ...
  if (!destinations.includes(returnTo)) throw new TypeError(t("interface:unknownPracticeReturnDestination"));
...
export function attachEnemyWorkshopReturn({
  ...
  localizedText(button, () =>returnTo === 'enemy-guide' ? t("interface:returnToFieldGuide") : t("interface:returnToWorkshop"));
  let disposed = false;
  button.onclick = () => {
