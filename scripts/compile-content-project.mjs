...
async function main() {
  ...
  if (!path)
    throw new Error(
      'Usage: compile-content-project.mjs <project.json|-> [--check | --mission ID --mode solo|versus --difficulty gentle|standard|expert]',
    );
  const options = {},
    seen = new Set();
  let check = false,
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (
      !['--mission', '--mode', '--difficulty'].includes(arg) ||
      !args[i + 1] ||
      args[i + 1].startsWith('--')
    )
  ...
  if (check && (missionId || Object.keys(options).length))
    throw new Error('--check cannot resolve a specific mission.');
  if (!check && !missionId) throw new Error('Choose --check or --mission ID.');
  const project = compileContentProject(await input(path));
  const result = check
    ? {
