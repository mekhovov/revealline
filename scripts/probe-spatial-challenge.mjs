...
const difficulties = list("presets", "standard", [
  "gentle",
  "standard",
  "expert",
]);
    ...
    for (const turnPolicy of controls)
      for (const seed of seeds) {
        ...
        if (input) {
          ...
          if (
            !route ||
            route.simulationIdentity !== prepared.simulationIdentity
          )
            throw new Error(
              `Missing or stale route: ${id}/${difficulty}/${turnPolicy}/${seed}`,
            );
          prefix = route.segments.map((segment) =>
            Array.isArray(segment)
              ? { direction: segment[0], ticks: segment[1] }
              : segment,
          );
