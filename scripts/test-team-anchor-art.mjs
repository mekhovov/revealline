...
import {
  buildTeamAnchorArt,
  produceTeamAnchorArt,
  TEAM_ANCHOR_ART_DIRECTORY,
} from './produce-team-anchor-art.mjs';
import { teamAnchorSlotSpecs } from '../game/presentation/team-anchor-slots.mjs';

const root = fileURLToPath(new URL('../', import.meta.url));
const hash = (bytes) => createHash('sha256').update(bytes).digest('hex');
async function fixture(t) {
  const directory = await mkdtemp(join(tmpdir(), 'rl-anchor-art-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const source = join(directory, TEAM_ANCHOR_ART_DIRECTORY, 'source.mjs');
  await mkdir(join(directory, TEAM_ANCHOR_ART_DIRECTORY), { recursive: true });
  await writeFile(source, await readFile(join(root, TEAM_ANCHOR_ART_DIRECTORY, 'source.mjs')));
  return { directory, source, output: join(directory, TEAM_ANCHOR_ART_DIRECTORY, 'prepared') };
}
...
test('produced pair reproduces retained PNGs and stays below exact slot budgets', async () => {
  const { manifest, files } = await buildTeamAnchorArt();
  const slots = teamAnchorSlotSpecs();
  assert.deepEqual(
  ...
  for (const asset of manifest.assets) {
    ...
    assert.deepEqual(
      bytes,
      await readFile(join(root, TEAM_ANCHOR_ART_DIRECTORY, 'prepared', asset.file)),
    );
  ...
  await produceTeamAnchorArt({ check: true });
});

test('check-only missing output never creates a production directory', async (t) => {
  const f = await fixture(t);
  await assert.rejects(
    produceTeamAnchorArt({ projectRoot: f.directory, check: true }),
    /Missing produced/,
  );
...
test('mismatch preflight retains existing pixels without writing the other state', async (t) => {
  const f = await fixture(t),
    bytes = Buffer.from('retained captured original');
  await mkdir(f.output);
  await writeFile(join(f.output, 'captured.png'), bytes);
  await assert.rejects(
    produceTeamAnchorArt({ projectRoot: f.directory }),
    /Immutable Team anchor output differs/,
  );
  assert.deepEqual(await readFile(join(f.output, 'captured.png')), bytes);
  await assert.rejects(access(join(f.output, 'available.png')), { code: 'ENOENT' });
});
...
test('fresh generation and repeat check retain the entire pair byte-for-byte', async (t) => {
  const f = await fixture(t);
  const first = await produceTeamAnchorArt({ projectRoot: f.directory });
  const second = await produceTeamAnchorArt({ projectRoot: f.directory, check: true });
