...
In **Saves & loads**, choose **Prepare game and originals backup**. The current writable game pauses before this surface opens. Preparation reads the current game-data snapshot and the saved shared-origin picture, story and music inventories, one component at a time. It offers five short native links: **Download game data**, **Download pictures**, **Download stories**, **Download music** and **Download coverage report**. Each row keeps its preparation/download-request state and byte size. The optional **File names** disclosure lists the exact wrapping filenames; the short labels do not change the names downloaded or recorded in coverage. With File names open, the first Escape or controller Back collapses only that detail and keeps focus on its summary; a second Back closes Saves through its existing origin. Prepared files remain available after collapsing the detail. One prefix is created per preparation: `RevealLine-backup-<UTC timestamp>-<random ID>`, with compact ASCII time and a fresh 32-character random ID. Every filename and report reference in that set uses the same prefix; even preparations in the same millisecond receive different IDs. In the list below, `<prefix>` means that full generated prefix:
...
Focused source tests exercise actual finite PNG, MP4 and MP3 bytes through the unchanged serializers, concurrent generation/profile changes, detached/missing originals, cancellation, native-link cleanup and the actual paused solo host. DOM/media boundaries are modeled. The separate [native download record](verification/guided-backup-a858-downloads.md) verifies all five actual files from the earlier fixed-name `a85817ce` source for an empty player profile/default saved metadata, including the CLI on both Node versions. The separate [050a02a prefixed-name observation](verification/guided-backup-050a02a-downloads.md) verifies five actual Downloads files directly with the strict checker on both Node versions, for the same empty-player/default-metadata scope. The later short purpose labels, File names disclosure and responsive layout still need native review. Custom-uploaded media, fresh-origin restore, codec playback, audible music and cold-offline journeys remain unqualified.
