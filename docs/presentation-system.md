...
The model limits presentation metadata to 4 MiB, transfers to 32 MiB, individual files to 4 MiB, images to 1920 pixels per side and 2,073,600 total pixels. Slot budgets can be lower: small sprites default to 128 KiB and fonts to 2 MiB. The exact saved-picture pipeline additionally retains its own 1920×1080, contain/nearest contract. New FPV art needs asset/presentation revisions, not new map or campaign identities. Owner slots are authoring destinations, not an automatic rewrite of existing saved/earned picture pins.
