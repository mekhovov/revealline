...
The new solo host opts into stored still-image assignments. This source integration is under verification; it is not a public release or a video implementation. Core movement, capture, collision, score authority and recorded input stay unchanged.
...
The nine new host cases and seven helper cases pass on Node 22.22.2 and Node 20.19.5. Pure tests cover cancellation and late image release, all-theme immutable choices, explicit legacy behavior and exact prospective owner catalogs. An initial affected batch passed 124 cases. A broader 205-case run exposed 14 outdated timing/session-version expectations in three existing host files; all 19 cases in those files passed after adapting them to the asynchronous start barrier and v3 saves. Historical replay and saved-session fixtures were not changed. The broader batch has not yet been rerun as one final aggregate. Scoped lint and formatting pass. These checks do not certify real browser disk, PNG raster quality, physical controls or devices. The final browser journey must separately upload/assign A, play and save, assign B, restore A, win and inspect Collection, export/import originals and profile data, and repeat after stopping the server.
