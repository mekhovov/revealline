...
Implemented in the working source for **v0.14.0**. The latest frozen release is **v0.13.0**, which retains its earlier result text. This guide does not declare v0.14 published or verified as a frozen distribution.
...
Six projection tests check owned data, status suppression and context without duplicating every string. Twenty-six integration tests include all seven causes in both steering modes, real replay/checkpoint verification, restoration during recovery, eventual wins with retained causes, terminal-session rejection, prior collection preservation and original frozen route expectations. Fixtures use normal fixed-tick commands; they are not manual play or new historical oracles. Browser checks must separately cover the actual reader, consequence visibility, held Confirm, touch Retry and successful-picture behavior at compact and desktop sizes. See the [accepted plan](round-24-retry-plan.md); human comprehension, physical hardware and enjoyment remain unmeasured without corresponding evidence.
