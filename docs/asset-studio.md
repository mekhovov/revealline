...
### Transferring retained revision history

Import retains the incoming theme families, their tokens, parent references and exact historical slot bindings, plus collection revisions, required slots and names. Independent replacements made before the selected revision remain available in slot history, with their source originals downloadable and only previously bound revisions eligible for **Bind this revision**. The selected imported collection keeps its authored name; theme-only imports use the theme name.

Identical local asset records and wholly identical theme/collection families keep their existing identities. Conflicting families receive collision-safe namespaced IDs and contiguous local revision numbers, with every parent and binding reference remapped. Sparse incoming revision numbers are mapped without inventing missing records. One imported theme family adds one Theme choice, regardless of its revision count. A foreign root inherits the receiving workspace's base so it remains selectable, while its own tokens and bindings take precedence; this is a compatibility adaptation, not byte-identical theme metadata. Different families retain their authored names; when names coincide, the Theme selector includes each exact ID so local and imported choices remain distinguishable. Original media bytes stay exact, and the current local history is never rewritten.

All existing asset, theme, collection, manifest and byte limits still apply to the combined workspace. An import that exceeds them fails before staging; it never drops old revisions to make room. Atomic transitions may append a contiguous batch of immutable revisions, but cannot skip a new revision, duplicate an identity or rewrite a retained record. This matches the local store's saved-history checks.

Maintainer prompt: “Stage two independent uploads A and B for one slot, export, then import into a fresh Studio. Find both exact historical bindings and downloadable originals, bind A, re-export and verify every payload byte. Retain authored collection labels, foreign parent chains and tokens, including sparse revisions and more than 128 theme revisions without one menu choice per revision. Cover namespace collisions, exact-record reuse, unbound same-ID records, all immutable append boundaries and combined-limit rejection without partial mutation. Keep native file chooser, image decoding, download, local-save and responsive evidence separate from public API tests; do not change player data or publication defaults.”
