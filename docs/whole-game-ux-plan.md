...
Updated 17 September 2026. This is the current implementation order approved by the user. It replaces the previous ordering; historical release records remain unchanged. Source inspection began at `a53d646153fc9dfb2d8f152fdc9dbde607782777` (accepted Recovery v0.60.4). Settings PR #94 merged as `7db26c148cced9b51315dc24902569234061b17d`, following Motion acceptance PR #99. This checkpoint incorporates the reviewed Replay/navigation production-metadata repair; frozen source commits and historical release evidence remain unchanged.
...
**Queued correction status.** Replay v0.60.7 and navigation/backup v0.60.8 inherited a stale production ledger; their initial exact-source attempts stopped before tests or freeze. The [reviewed metadata repair](verification/cross-mode/replay-disabled-actions/recipe-review.md) is now composed into both sources, and fresh full qualification remains pending. The original failed attempts remain historical evidence, not product-test results. The separate five-path Motion slider-label proposal passes **38/38 on Node 20 and Node 22**; isolated Chromium range, recipe and layout checks are in progress. Those checks remain distinct from CUA, physical-device, assistive-technology and public acceptance.
...
| 3 | P03/P05 navigation | Shared Home/lobbies/Pause/Help/Workshop; actual opener restoration, responsive focus order and device prompts. | Partial. Replay and Missions/backup corrections are prepared; metadata repair is composed; fresh qualification and release/public gates remain. |
