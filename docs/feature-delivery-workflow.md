...
Main and archive Pages uploads must enable `include-hidden-files: true` for the strictly constructed distribution. Include `.nojekyll` and historical `.xonix-build.json` in the exact public inventory; do not skip them to obtain a passing audit. Validate the finite hidden-file allowlist and reject repository internals, unexpected hidden directories, links and special files before upload. Preserve an initial failed inventory or missing-public-file result, correct the upload, redeploy and audit into a new output directory.
