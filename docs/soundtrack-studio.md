...
The current edition uses the same rich v3 media manager for audio in ordinary play, Playground practice and First Flight. Practice and training retain authored artwork without managed picture pins or profile/session/award writes. Opening audio can upgrade a v1/v2 database to v3; reading and playing preserve its existing domain rows and original bytes, and playlist changes still require explicit Save. Older immutable editions using a lower database version cannot reopen v3: their recovery message is a compatibility boundary, not a reason to delete data or attempt a downgrade. Use the current edition to access or export the shared library.
