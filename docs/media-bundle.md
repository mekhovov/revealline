...
`game/media-bundle.mjs` provides `.rlmedia` export, validation and explicit target-store restore. The edition-matched still workshop now exposes review/Restore and Prepare originals → visible native Download controls. The game consumes those stored originals through [live pins](flight-pictures.md) and [earned Collection](earned-picture-view.md), keeping the transport API independent of playback and awards. Source/browser qualification passes at the [documented scope](feature-delivery-workflow.md#current-still-integration-evidence); frozen/public P5 delivery remains pending.
...
Undo can explicitly restore an earlier assignment set using a new current-generation review, while keeping all later originals/history. A stale review cannot be reused after a successful commit. Neither ordinary import nor Undo deletes history or reclaims bytes. GC remains unimplemented. Durable earned receipts and live-session pins are implemented separately in versioned player/session data; assignment Undo never rewrites them.

Focused tests cover binary corruption and strict owners, original-byte round trips, removed-pack restart hydration, assignment restore/Undo, immutable conflicts, cancellation, stale writers, quota/transaction refusal, original MP3 preservation and generic-reference portability. They use finite modeled IndexedDB and injected decoders. Those initial checks are model evidence. The actual source-browser receipt now records native `.rlmedia` download, fresh-origin review/restore, byte-identical re-export and paired JSON recovery/Undo with live/Collection A preserved. Cold offline paired recovery, blocked/quota lifecycle stress, frozen/public P5 and physical devices remain separate gates.
