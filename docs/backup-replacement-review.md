...
Keyboard Escape while reviewing belongs to the replacement operation and is consumed before the native dialog close request. Native close-watcher cancel events can be non-cancelable after earlier requests; relying only on preventDefault there can close Library unexpectedly. A later Escape after review ends keeps the ordinary modal lifecycle. Nested dialogs, composition, modified keys and already-handled events retain their own owners. [HTML close-watcher behavior](https://html.spec.whatwg.org/multipage/interaction.html#close-watcher-infrastructure)
