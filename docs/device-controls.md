...
## Narrow Solo flight counters — P05 HUD correction

In running Solo, landscape widths 521–680 CSS pixels and heights up to 600 CSS pixels use two telemetry columns in the existing space beside Game menu, fullscreen and Pause. The header reserve follows the selected counter/secondary text tokens; the complete arena is fitted below it without cropping or stretching. Fonts, values, coverage target, caption ownership, actions and target sizes are unchanged. Event/score updates do not change the reserve. Training (`first-flight-session`), portrait, wider layouts and Couch keep their existing rules. This changes presentation only: capture-stop, steering, simulation, scores, saves, recordings and explicit Resume retain their authorities.

The [original public baseline](verification/cross-mode/solo-flight-hud/public-baseline.original.json) observed Large/Plain at 600×400, score `01700`, overlapping the fullscreen button band by 6.875px. The [corrected source-preview observation](verification/cross-mode/solo-flight-hud/native-observation.original.json) used exact source `3e47f60b9f9b4c5088410536d649ff595b2667eb` with only this CSS override (SHA-256 `0ebd933c111686f487cd4d75098f20c5a78334ca08f47f8c9f6e15b762537ecf`). Its ordinary installed Orchard Crossing flight had `00000`, 0.0% and three lives; no game state was injected. Native rendered geometry separated counters from 44×44 fullscreen/Pause controls at 600×400 with Large/Plain and controls hidden or a shown D-pad. Additional checks covered 521×400 and 680×400 with a right D-pad, portrait 390×844, and Standard/Theme at 600×400 with the existing floating-stick overlay. Portrait Pause → Field details → Escape restored the opener.

The tradeoff is less arena height: at 600×400 Large, the complete 2:1 canvas measures about 382.59×191.30 pixels, compared with the baseline 528×264. Standard/Theme measures about 447.80×223.90. A shown D-pad retains its existing placement beside the arena; the floating stick deliberately retains its existing overlay. Long encounter text still uses the existing ellipsis and Field details fallback; this correction does not claim a replacement for every critical field channel.

These are scoped desktop-browser native observations, not a physical phone/controller, browser zoom, all-score-length, all-encounter or play-comfort qualification. The candidate preview did not earn a nonzero score; the public baseline's `01700` observation remains separate. Integrating onto the newer Team source preserves that distinction: final integrated-source gates and affected public journeys remain required. P03/P05 remain open.

For future changes, measure actual text and control rectangles at both text sizes, both typefaces, short landscape/portrait, and conditional controls. Preserve at least 44px action targets, full board geometry and authored warnings/actions. Recheck legal score/time/life updates, Pause/Back/explicit Resume and both steering hands without inventing values. Do not fix crowding by shrinking selected text, hiding critical facts, changing numeric score authority or adopting static hide-all prototype CSS. This applies [W3C text-resizing guidance](https://www.w3.org/WAI/WCAG22/Understanding/resize-text.html) and [Xbox readable-text guidance](https://learn.microsoft.com/en-us/gaming/accessibility/xbox-accessibility-guidelines/101); the Large preference alone does not establish 200% zoom conformance.
