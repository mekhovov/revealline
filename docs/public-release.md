...
The v0.8.0 working source supplies 25 maps, seven gameplay roles, four presentation themes, four installable expansions and three optional [equipment seals](equipment-seals.md). Homeward Skies adds three original illustrated picture rewards; controller configuration, continuation and reviewed earlier-release copying extend the player library. The preserved v0.7.0 [browser/offline report](verification/round-17/frozen-browser.md), [source gates](verification/round-17/source-gates.md) and [integrity record](verification/round-17/integrity-notes.md) identify the preceding frozen artifact. A newer source version is not itself release evidence. Automated results do not certify every target below.
