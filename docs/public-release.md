...
The preserved v0.8.0 source supplies 25 maps, seven gameplay roles, four presentation themes, four installable expansions and three optional [equipment seals](equipment-seals.md). Homeward Skies adds three original illustrated picture rewards; controller configuration, continuation and reviewed earlier-release copying extend the player library. Its [browser/offline report](verification/round-18/frozen-browser.md), [source gates](verification/round-18/source-gates.md) and [integrity record](verification/round-18/integrity-notes.md) identify the frozen artifact. Ongoing source changes are not themselves release evidence. Automated results do not certify every target below.
