...
The latest independently frozen release is [v0.16.0](http://127.0.0.1:8767/releases/v0.16.0/site/game/), tagged at `8fb6b5782a6fee85211b9bbbfb3d13a1d1a60c43`. It adds [First Flight](first-flight.md): three optional training lessons using the actual simulation, a checked saved-flight handoff and finite Playground/Controller Lab previews. Lessons and practice settings stay in the current visit and grant no campaign reward. The campaign inventory remains **29 maps across the base campaign and six expansions**; training adds three separate lessons.

The [six-gate run](verification/round-26/source-gates.md) passed **1,521 unique tests** before a three-file landing-label correction. The [affected-check follow-up](verification/round-26/source-label-followup.md) passed 23 existing CLI tests, lint, formatting and validation on the final source; those tests are not additional unique cases or a repeat of the full suite. [Independent archive verification](verification/round-26/integrity-notes.md) matches all **331 final source inputs**, **142 loose output files**, **138 manifest assets** and **139 ZIP entries**. All nineteen older release trees and twenty prior tags match the original baseline after the recorded deployment-script archive restoration.

The verified offline inventory is **135 files / 20,624,650 bytes**, build ID `88feeb938b2a1742a483a7cdb650de7e94ba05da85863c9a6f8fa4cdbe55763d`. [Frozen browser/offline checks](verification/round-26/frozen-browser.md) recorded a first course visit and Grid Close a line win after the server stopped. Return retained byte-identical library, packs and suspended-flight data against the post-entry backup; the normal flight restored paused and continued through explicit Resume. The first offline Playground course visit also completed Close a line at 320×640. Three utility controls required scrolling in its captured Ready layout, and the initial Next pointer attempt required Enter to advance; no flawless pointer or empty-cache offline-install claim is made. The [source browser report](verification/round-26/source-browser.md) retains compact interaction clipping of about four pixels and the unexplained embedded Settings pointer anomaly. These local results do not establish a public-host deployment, physical-controller/touch behavior, native execution or beginner comprehension.
