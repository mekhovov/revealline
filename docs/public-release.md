...
The latest public and independently frozen version is **v0.18.0**, source `3eb42d7cb118d443e474682ee382484e0bf3c7f4`, available at [Reveal / Line](https://mekhovov.github.io/revealline/). The deployment task verified its published build metadata, fresh pack install/play journeys, mobile and invalid-route checks, and clear page logs after [workflow 34708917074](https://github.com/mekhovov/revealline/actions/runs/34708917074) passed. The separate local [archive audit](verification/round-28/v018-preservation.md) reproduced all 144 output files, 141 ZIP entries and 140 manifest assets; its 137-file offline inventory totals 31,798,763 bytes. Archive integrity does not substitute for a stopped-server offline-browser test.

v0.18 adds Pack/Level selectors and campaign launch links, protects asynchronous pack changes, and includes the [gallery-clock repair](verification/round-28/gallery-clock.md). Working-source **v0.19** adds cancellable current/saved attempt export, with [120 affected tests](verification/round-28/attempt-export.md) and separate [source browser acceptance](verification/round-28/attempt-export-browser.md). Its full source gates, frozen artifact and public deployment are not yet claimed here.
...
| Public website                | v0.18 deployed on GitHub Pages; published metadata, pack journeys, mobile/invalid-route checks and page logs verified | Verify later deployments and host-specific MIME/CSP/cache, privacy and rollback behavior not covered by that evidence |
