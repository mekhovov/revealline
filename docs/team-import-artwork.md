...
| Field      | Value                                                              |
| ---------- | ------------------------------------------------------------------ |
| Theme      | `fpv`, exact revision in the selected source policy                |
| Collection | `null`                                                             |
...
`readCoopPack`, historical pack fields and historical level fields stay closed. Adding `picture`, `presentation`, a URL or embedded image fields to those records remains invalid. Custom artwork uses the separate [versioned `.rlteam` presentation envelope](team-custom-artwork-envelope.md); it never adds fields to historical packs. The generic policy does not qualify local artwork or other themes and collections. Native-browser and release qualification of the custom-art path remain separate.
...
> Qualify historical Team import artwork through the actual host and production picture reader. First record the exact source SHA, compiled FPV theme revision and code-owned historical-import policy revision; require both theme identities to agree. Preserve the closed pack/level schemas and that explicit null-collection policy for scene.reveal.wide.field-kit revision 2: 1152 × 576 PNG, 52,720 bytes, SHA-256 53f1206a11a8791892f5c844c0641529acbc2c09c8d558676d0d801d72113850. Keep exact whole-pack/level hashes and typed string/numeric level revisions. Prove changed starter-namespace content cannot use generic artwork. Hold a replacement read and decode, choose Stop waiting, retry, replace the draft and complete stale work; retain the prior pack/image until successful adoption and require separate Start. Earn an imported multiple-stronghold result, use Next to a coverage map, change inactive setup drafts, Retry and earn the result again. Verify accepted picture identity, authored order, no built-in preference writes and no automatic Start from recovery. Inspect light/dark scenery, obstacles and objective cues without changing simulation geometry. If regeneration advances the theme revision, revalidate both starter bindings and the historical-import policy against the generated snapshot while preserving exact picture descriptors and historical records. Retain refusal of older/newer unbound snapshots; never derive an accepted revision from runtime “latest.” Run the complete real-byte binding and historical-import lease tests, then verify both starter arenas prepare and start through the native host. Preserve existing image bytes; report source tests, native review, physical devices, offline and public acceptance separately. Qualify custom-art envelopes through their separate reader and ownership contract; keep other theme collections and all unverified browser/release gates explicitly pending.
