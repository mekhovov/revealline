...
The held05 clean 390×844 native capture had a 362×181 arena: inherited labels rendered near three CSS pixels, and pilot body/rotor art clipped at the outer edge. The native record retains seven events and six JPEGs, including an inconclusive transitional capture and a mismatched landscape/DPR capture; neither is a new gameplay defect. A legal Relay Yard loop banked 0.9%, but did not prove full-win or rescue behavior. Backing plates alone did not establish readability. The P08-B source successor now passes existing catalog `bodyRecord` surface-motion accents to matching FPV Team bodies. Team `drifter`, `hunter`, and `claimed-rover` keep their actual behavior while borrowing the already-declared `bouncer`, `border-patrol`, and `claimed-rover` visual treatments. It reuses the bounded catalog without decoding a second body image; unavailable metadata retains the prepared body and functional cues. Frozen-build, public-motion, and physical-device review remain open.
...
## Crawling and active rescue states — source candidate

The Team adapter derives normal, cutting, downed, crawling, rescuing and recovery presentation states from the current run. A small Team-only heading history follows observed displacement across active and downed movement. A change into or out of the downed state does not turn a teleport to the safe anchor into apparent crawling. Blocked direction intent is not movement. The adapter applies the final heading before calculating the prepared body's rotated bounds and cosmetic edge offset.

Downed craft keep stopped rotor and surface phases while their body can face a real crawl. Pause and repeated paints cannot advance heading; reduced effects removes interpolation without hiding direction. Replacement presentation, fresh attempt and backwards simulation time retire the history. Core movement, contacts, rescue timing, checkpoints, scores and player input are unchanged.

An active rescuer gets a static, dark-backed `RESCUE 1` or `RESCUE 2` label identifying the downed partner. It accompanies the existing player number and circle/diamond; it does not replace them. The existing HUD and Support action remain the sole percentage display. No second timer, radius or Scan-like pulse is introduced. When real input release, cancellation, recovery or completion removes the rescue channel, the marker disappears. The cue uses the existing bounded placement helper and a 14 CSS-pixel minimum; text is not shrunk to fit.

These are procedural state corrections around existing prepared body slots. They do not constitute new complete Team character sets or close the registry requirement. Explicit versioned Team pilot/objective recipes, five-edition coverage and immutable collection adoption remain required follow-up work; historical snapshots and compiled bindings are not rewritten by this feature.

Qualification must retain genuine command-earned downed/crawl/rescue/recovery evidence for both seats. Compare painted and unpainted simulation outcomes, test blocked movement and knockdown teleports, verify paused/reduced and backwards/reset history, and inspect rotated asymmetric pivots at narrow and wide canvas sizes. Modeled canvas calls prove placement and ownership, not decoded pixels, actual readability, physical controls or public acceptance. Complete the ordinary source/release gates and real-browser review over light and dark artwork before acceptance.

Authoring/review prompt:

> Review Team pilot states using the same approved compact/detailed bodies as live play. Reach both players' downed, crawling, held Support rescue and recovery states through legal commands. Retain stopped downed rotors, exact contact centres, rotated body containment, player number plus circle/diamond and static partner-labelled rescue cues. Read real progress from the existing HUD; do not draw a second percentage or decorative radius. Verify both players, blocked intent versus movement, actual pause cancellation, reduced effects and explicit Resume. Keep the simulation, input ownership, authored arena identity and original artwork unchanged. Report source tests, decoded browser views, physical input and public checks as separate evidence. Do not claim complete Team recipe registration or production acceptance from this bounded correction.
