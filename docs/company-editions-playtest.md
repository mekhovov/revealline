...
The inventory contains **six current candidate raster pictures**: the five Coupa campaign openings and the first Netherlands workshop mission. Three additional raster pictures belong to the historical Portuguese pilot. **Sixty current missions still lack dedicated raster pictures: 25 Coupa and 35 Netherlands.** Distinct campaign palettes and procedural fallback are interim presentation, not final mission illustrations. The new Coupa city/network and Netherlands workshop pilots still need human review. Do not produce the remaining bulk artwork until this formative gate has observations and a recorded design decision.
...
| Historical PT Workshop Lights | Not tested                | Recognizable community workshop; safe return and required objective                 |
| Historical PT Community Relay | Not tested                | Collection/packing story and differentiated route decisions                         |
| Historical PT After the Storm | Not tested                | Fictional support narrative understood; slow terrain and two threats remain legible |

For the Netherlands expansion, all six chapter-opening observations are also pending. Each
...
Use `game/company.html?edition=<suffix>`. No observation has been collected for these chapters.
...
The progression is foundations → moving frontier and capture-activated roamer → eroder with spare returns → finite trail/heading lock and warning lane → numbered objective/return gates → directional fields and shield/core stages. Gates span actual gaps between safe surfaces; a matching objective opens them without an extra fill. In the finale, capture the visible shields, read the core's vertical attack, then close fresh trail during CORE OPEN or isolate the core. The final two missions add return gates, and the final mission combines those with flow fields. These use canonical engine rules and role cues; scenery does not create collision rules. No human observations yet support pacing or comprehensibility.
