...
Opening Field Guide from the title keeps that title underneath the native dialog; Close guide, Back and Escape return focus to its original button. Returning from isolated practice restores the guide first. Main menu remains the return screen when entered during an already-started flight too; closing Main menu separately reveals that unchanged paused flight, which still requires explicit Resume. This intentionally replaces the earlier Guide shortcut that closed Main menu. A direct flight opener, if supplied, still returns to its own paused origin. Destinations that can replace or start the campaign retain their existing title-closing behavior.
