...
Ordinary `xonix-level.v1` maps retain `xonix-core.v2` and `xonix-replay.v3`. Encounter `xonix-level.v2` maps use `xonix-core.v3` and `xonix-replay.v4`; the enclosing formats dispatch explicitly instead of reinterpreting old recordings. Preserve old executable builds for old replay versions. A saved-attempt envelope also needs the matching campaign and class roster before it can resume. Use a [full backup](full-backup.md), or export player libraries, pack libraries and unfinished attempts separately when comparing or moving installations; [library/pack workflows](library-and-packs.md) and [replays](replays.md) describe the boundaries.
...
Starting with v0.2.1, each packaged release uses its exact build label in the player-library, installed-pack and suspended-slot channel: for example `revealline.library.release-v0.2.1.v1`. Solo and couch resolve the same pack channel. This prevents new preference schemas from altering an immutable older release's stored profile. A fresh release starts a separate collection; use **Export complete backup** in the older version and load that file in the newer version to transfer progress, pictures, packs and flight together. Older archives cannot read every newer schema; transfer forward into the validating new reader. The development channel remains separate and migrates old preferences on load.

Before taking a release snapshot, validate content, run relevant behavior tests, inspect the built browser game and record actual devices exercised. Keep experimental motion-lab profiles separate. A version in package metadata describes source; it does not prove a saved snapshot, remote deployment or native store package exists. The frozen v0.4.1 [integrity record](verification/round-14/integrity-v0.4.1-notes.md) and [browser report](verification/round-14/release-v0.4.1-browser.md) identify the actual verified artifacts. See [development](development.md), [deployment](deployment.md) and the [public-release gate](public-release.md).
