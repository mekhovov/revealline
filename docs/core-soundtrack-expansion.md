...
with a metal variant, CC BY 4.0) and
[The Wreck](https://opengameart.org/content/the-wreck) (thrash/hardcore metal,
CC BY 3.0). These remain source-page research leads only: neither is acquired,
technically verified, listening-approved, admitted or placed in defaults.
...
only **175 distinct compositions**.

Shared local free space recovered to approximately **1.13 GiB** after coordinated
release cleanup. That is above the 1 GiB guard but leaves only a narrow margin.
Small, verified intake can continue one file at a time; large packs, local builds
and source archives such as The Wreck remain deferred. Preserve user changes,
frozen releases, worktrees and evidence.
