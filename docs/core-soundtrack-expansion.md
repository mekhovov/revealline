...
| New archive previews     | 163 public archive recordings across 24 collections; the immutable 104-recording baseline and its later additions retain separate publication evidence                                                                                            | Full listening, taste approval and game admission remain; zero new game admissions                                           |
...
the primary Pages selector serves it. Authoritative game `main` later advanced to
`5de31970401b9659c1ffacadbaf469c6dacf845d` with PR #612's touch-steering repair;
v0.132.3 qualification is active and no later soundtrack merge may race it. Archive-directory
client PR #604 was rebased cleanly onto that exact main at
`dd9d93f9ffb9f3889ab9d8b9bf63e8c89386a5e0`. Its combined catalogue, panel and
player cohort passes 177/177, formatting checks pass, and the broader
host suite exposes the same timing timeout on exact current `main`; this remains a
baseline test-stability issue rather than evidence of a soundtrack regression.
Opening-theme fallback PR #617 is rebased onto the same main at
`df37c08403567670dca54d24ed7ac6ab9ca4c535` and passes 7/7 focused tests plus
formatting checks. Both remain draft and unallocated. Their latest hosted runs pass
preflight/reconciliation but skip the held product jobs and fail `release-ready`
because no release is assigned; those skips are exclusions, not passes.
...
game-unadmitted and excluded from defaults. New substantial batches wait for PR
#604 to become public and Archive 02 to be created. HexaPuppies remains a
...
Local free space is about **3.4 GiB**, above the required 1 GiB floor but still too
...
8. **Archive scale-out:** archive
   ...
   a hardcoded fallback. It is rebased on authoritative main
   `5de31970401b9659c1ffacadbaf469c6dacf845d` at exact head
   `dd9d93f9ffb9f3889ab9d8b9bf63e8c89386a5e0`. The 177/177 catalogue, panel and
   player cohort plus formatting pass; the wider host suite retains a current-main
   timing flake. Hosted preflight and reconciliation passed while build/focused/full
   tests remain skipped under the unallocated release hold. Create Archive 02 only
   after that client is public. Archive PR #41 raises Archive 01 to 887,503,800
   audio bytes, leaving 12,496,200 bytes under its guard; do not place another
   substantial batch there.

The latest public/general GitHub release at this checkpoint is
[v0.132.2](https://github.com/mekhovov/revealline/releases/tag/v0.132.2). The
...
Authoritative `main` is `5de31970401b9659c1ffacadbaf469c6dacf845d`; its PR #612
product delta is qualifying for v0.132.3 and is not yet a new soundtrack release.
v0.132.0 and its
...
- Local free space is **about 3.4 GiB** at this checkpoint. Preserve reachable
...
- Archive directory [PR #34](https://github.com/mekhovov/revealline-soundtracks-01/pull/34)
  ...
  is rebased on authoritative main `5de31970401b9659c1ffacadbaf469c6dacf845d`
  at exact head `dd9d93f9ffb9f3889ab9d8b9bf63e8c89386a5e0`; its focused catalogue,
  panel and player cohort passes 177/177. The pushed exact-head hosted run remains
  held by the draft's missing release allocation. The next action is an allocated
...
6. **Archive scale-out:** the public archive now carries 887,503,800 audio bytes.
   ...
   guard preserves 12,496,200 bytes of internal headroom.

This plan checkpoint is reconciled through authoritative game main
`5de31970401b9659c1ffacadbaf469c6dacf845d`. It records archive publication and
