...
1. **v0.132 Pages completion:** the immutable GitHub release and annotated tag are
   published from current `main`, but the public Pages inventory still ends at
   v0.131.0. The v0.132 edition route and stable `/app/` launcher return 404. The
   sole release publisher owns the separate reviewed selector/controller change,
   deployment and direct public acceptance. This publication gap blocks new
   v0.132 cold-offline soundtrack evidence; it does not invalidate v0.130 desktop
   soundtrack acceptance.
2. **M8 stream recovery:** desktop delivery is complete in v0.130.0. Merged test-only
   ...
   catalogue 503 → Refresh recovery. Physical iPhone/controller and cold-offline
   checks remain and do not reopen the released desktop repair.
...
Pages remains selected to v0.131.0: `release.json` reports v0.131.0 while
`/releases/v0.132.0/site/game/` and `/app/` return 404. Release-trigger
[run 36213375672](https://github.com/mekhovov/revealline/actions/runs/36213375672)
passed its selector check but skipped the publisher dispatch because the reviewed
selector did not yet match v0.132.0. Treat v0.132 as a published GitHub release and
an incomplete Pages deployment until the separate selector flow succeeds.
...
- Cold-offline browser acceptance must reuse the stable `/app/` launcher and official
  content store merged through [PR #536](https://github.com/mekhovov/revealline/pull/536)
  and released in v0.132.0, rather than adding a second launcher to an edition
  worker. Public verification cannot start until the v0.132 selector deploy makes
  `/app/` and the immutable edition route available. Its zero-MP3 required core
  keeps Shchedryk in the optional soundtrack group; verify the resulting first-run
  fallback and installed-theme behavior rather than assuming the earlier bundled
  opening-theme exception is present offline.
...
| M8  | Public desktop compatibility/stall repair complete; real-player recovery regression merged; current 140-row catalogue loads and a new Reckless object plays in v0.130                    | v0.132 Pages selector/deployment and physical device access                         | Keep the persistent remote deck, watchdog and mixed queues green; after the v0.132 `/app/` route is public, verify cold restart, optional Shchedryk installation/fallback, then finish iPhone/controller checks | About 0.5 day after v0.132 Pages publication, plus device access                                               | PR #370, v0.111.0, Archive76; merged repair [PR #523](https://github.com/mekhovov/revealline/pull/523); merged regression [PR #555](https://github.com/mekhovov/revealline/pull/555); merged offline runtime [PR #536](https://github.com/mekhovov/revealline/pull/536); v0.130 140-row browser evidence and v0.132 release/404 evidence above                | v0.130.0 desktop accepted; v0.132.0 GitHub release published, Pages pending  |
