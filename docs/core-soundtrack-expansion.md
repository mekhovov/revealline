...
2. **M8 stream recovery:** desktop delivery is complete in v0.130.0. Merged test-only
   ...
   the pending attempt. Repair and release that first-run fallback before the
   physical iPhone/controller and cold-offline checks.
...
- Public v0.132 first-run evidence shows the optional Shchedryk acquisition can time
  out while the explicit opening playlist owns the queue. The synthesized fallback
  is then not selected and Next cannot cancel the visible pending attempt. Preserve
  the default opening-theme intent, but bound failure and hand control to available
  built-in music.
...
| M8  | Public desktop compatibility/stall repair complete; real-player recovery regression merged; fresh v0.132 exposed a separate opening-theme timeout/fallback defect                        | Small source repair, release sequencing, physical device access and cold-offline execution | Preserve the default Shchedryk attempt but recover to available built-in music after bounded acquisition failure; ensure Next cancels a pending attempt; then verify cold restart and iPhone/controller behavior | About 0.5–1 day for repair and release, plus device access                                                     | PR #370, v0.111.0, Archive76; merged repair [PR #523](https://github.com/mekhovov/revealline/pull/523); merged regression [PR #555](https://github.com/mekhovov/revealline/pull/555); merged offline runtime [PR #536](https://github.com/mekhovov/revealline/pull/536); v0.132 selector [PR #592](https://github.com/mekhovov/revealline/pull/592), Pages run 36214429148 | v0.130.0 desktop accepted; v0.132.0 public launcher available; first-run repair pending |
