...
`sourceId` is the exact channel, such as `release-v0.3.0`, `release-0.3.0` or legacy `release`. Do not reconstruct source keys from UI text. Discovery checks at most 4,096 localStorage keys and returns at most 32 distinct candidates without loading their values. It unions only exact `revealline.library.<channel>.v1` and `revealline.suspended.<channel>.v1` keys; both keys for one channel produce one candidate. Writer, lock and recovery-journal keys never create candidates. Sessions use localStorage; installed image packs and backup journals use the existing asset IndexedDB reader. No new database enumeration or storage format is introduced.
