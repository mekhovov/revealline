...
BINDING = json.loads((CACHE / 'preview-intent-binding.json').read_text())
BASE = BINDING['base']
...
(CACHE / 'server-intent.json').write_text(json.dumps({
    'port': server.server_port, 'base': BASE,
    'bindingSha256': hashlib.sha256((CACHE / 'preview-intent-binding.json').read_bytes()).hexdigest(),
    'scope': 'Four held runtime overlays; every other response from exact base Git, no build.'
}, indent=2) + '\n')
