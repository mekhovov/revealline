...
http_errors=[x for x in rows if x.get('event')=='response' and x.get('status',200)>=400]
assert all(x['status']==404 and x['url']=='https://mekhovov.github.io/favicon.ico' for x in http_errors)
...
review={
 ...
 'observer':{'counts':counts,'bytes':(R/'online-1-cdp.jsonl').stat().st_size,'runtimeExceptions':0,'httpResponsesAtLeast400':http_errors,'requiredRuntimeResponsesAtLeast400':0,'capped':False,'stopReason':health['reason'],'minimumFreeBytes':min(x['freeBytes'] for x in capacity),'maximumProfileBytes':max(x['profileBytes'] for x in capacity),'explicitClosure':read('139-closure-verification.json')},
