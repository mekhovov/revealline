...
if(!launch.profile.endsWith('/.cache/cross-mode/p01/v0571/library-rail-r3-native/run/profile')||!launch.endpoint.startsWith('ws://127.0.0.1:'))throw Error('Wrong owned browser');
...
fs.writeFileSync(out.replace('close.json','targets-at-close.json'),JSON.stringify(await call('Target.getTargets'),null,2)+'\n',{flag:'wx'});const args=(await call('Browser.getBrowserCommandLine')).arguments;if(!args.includes('--user-data-dir='+launch.profile))throw Error('Actual browser profile mismatch');
