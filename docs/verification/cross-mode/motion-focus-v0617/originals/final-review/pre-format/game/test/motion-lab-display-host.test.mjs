...
for (const condition of ['hidden', 'inert', 'disabled', 'fieldset', 'background', 'away', 'disposed'])
  test(`Motion rotation cannot reveal an ${condition} current control`, async (t) => {
    ...
    t.mock.method(field, 'scrollIntoView', () => assert.fail('An ineligible control must not scroll.'));
...
test('Motion resize cannot scroll after measurement changes the focused owner or loses foreground', async (t) => {
  ...
  t.mock.method(field, 'scrollIntoView', () => assert.fail('Stale focus cannot reveal the old field.'));
...
test('Motion resize reveals the control when its labeled field is taller than the visible settings area', async (t) => {
  ...
  target.focus();
  const calls = [];
  t.mock.method(field, 'scrollIntoView', () => assert.fail('An oversized field cannot hide its control.'));
