...
for row in git('ls-tree', '-r', '-z', COMMIT).split(b'\0'):
    ...
    name = name.decode()
    if mode in ('100644', '100755') and kind == 'blob' and name.startswith(('game/', 'authoring/')):
        paths[name] = oid
overrides = {}
...
binding = {'commit': COMMIT, 'tree': TREE, 'pid': os.getpid(), 'port': server.server_port,
           'url': 'http://127.0.0.1:' + str(server.server_port) + '/authoring/motion-lab/',
