...
for row in git('ls-tree','-r','-z',COMMIT).split(b'\0'):
    ...
    name=name.decode()
    if mode in ('100644','100755') and kind=='blob' and name.startswith(('game/','authoring/','docs/','site/')): paths[name]=oid
...
FAULT_PATHS=('releases/index.json','site/landing.mjs')
assert list(FAULT_PATHS)==PLAN['faultPaths'], 'Use the exact reviewed About release catalogue paths'
def record(event):
...
def read_faults():
    ...
    if not isinstance(delays,dict) or set(delays)-set(FAULT_PATHS): raise ValueError('Fault delay outside reviewed About release catalogue')
    if not isinstance(failures,list) or len(failures)>1 or any(not isinstance(x,str) or x not in FAULT_PATHS for x in failures): raise ValueError('Fault failure outside reviewed About release catalogue')
    if len(set(failures))!=len(failures): raise ValueError('Duplicate catalogue failure')
