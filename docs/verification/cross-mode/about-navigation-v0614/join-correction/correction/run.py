...
def budget():
 ...
 packet_size=sum(p.stat().st_blocks*512 for p in root.rglob('*') if p.is_file())
 assert packet_size<1024**2 and free>=reserve,(packet_size,free)
...
before=inputs(); ident=identity(); assert ident['branch']=='codex/p03-about-controller-v0614'; initial=budget(); argv=[node,'--max-old-space-size=1024','--test','--test-concurrency=1',*tests]
