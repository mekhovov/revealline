...
import subprocess, mimetypes, hashlib, json, os, threading, re, shutil, time

ROOT = Path(__file__).resolve().parents[3]
OUT = Path(__file__).resolve().parent
BASE = '3f934b7508645d2cad03466276d20e8d038f9d76'
HOLD = 'e92141bfea272954915778b99386ce9f4567f4eb937c201519afdefe45579687'
DELAY = {'path': 'game/presentation/compiled/assets/c1aedf89bc3433dc1cb60998fa1e2563480a590c38586332f444c4c12c772fce.png', 'seconds': 6}
NORMAL_BINDING_SHA256 = 'f9d92707a05e75cc9ee45dcf22c8c792f6c98b9490f9518b69ae938d6cd77cf1'
LIMIT = 64 * 1024 * 1024
...
held = json.loads(raw)
if held['baseline'] != BASE:
    raise RuntimeError('Wrong preview base')
rows = {row['path']: row for row in held['sourceFiles']}
overlays = {}
...
for line in subprocess.check_output(['git', 'ls-tree', '-r', '-z', BASE], cwd=ROOT).split(b'\0'):
    ...
    if mode in ('100644', '100755') and name.startswith(('game/', 'authoring/', 'docs/')):
        regular[name] = oid
if DELAY['path'] not in regular or DELAY['path'] in overlays:
    raise RuntimeError('Delay requires the exact original Git PNG')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def serve(self, send_body):
        with requests:
            ...
            try:
                body = git_body(name)
                if send_body and name == DELAY['path']:
                    time.sleep(6)  # HTTP delay only; bytes and game clocks remain unchanged.
                length = len(body)
...
binding = {'base': BASE, 'baseTree': tree, 'sourceHoldSha256': HOLD,
           'overlays': [rows[name] for name in overlays], 'incomingDelta': held['incomingDelta'],
           ...
           'mode': 'delayed-original-png', 'delay': DELAY, 'normalBindingSha256': NORMAL_BINDING_SHA256,
           'scope': 'Exact Git3f934b7 P05-A baseline with four static P08 production immutable memory overlays pinned at e921 source-held-01. All other regular assets come from Git3f934b7; current Title/Missions/Library/Team departure, mode rows, Ready focus and shared display remain. The only timing fixture waits six seconds before headers/body for every GET of the one exact original c1aedf89 PNG, including ranges; HEAD is immediate. Original bytes and application clocks/state are unchanged. No scripted responses, P05-B or later P03 operation-focus correction. Normal64092 remains separate and unchanged. Finished-to-Next prior-result preservation remains P07. LRU cap describes stored Git blobs, not total process RSS.'}
