...
FAULT_PATHS=('game/presentation/compiled/studio.json',)
assert list(FAULT_PATHS)==PLAN['faultPaths'], 'Use the exact reviewed Studio collection paths'
def record(event):
...
def read_faults():
    ...
    if not isinstance(delays,dict) or set(delays)-set(FAULT_PATHS): raise ValueError('Fault delay outside reviewed Studio collection')
    if not isinstance(failures,list) or len(failures)>1 or any(not isinstance(x,str) or x not in FAULT_PATHS for x in failures): raise ValueError('Fault failure outside reviewed Studio collection')
