...
Source `39eaea27` subsequently reported 5,282/5,292 passing in each complete hosted family. Ten unique leaf cases across four files exceeded a five-second chapter-preparation wait; no tests were cancelled, skipped or marked TODO. Preflight and ordinary PR build success did not qualify the source: manual freeze was skipped and its artifact inventory is empty. [Navigation recovery evidence](navigation-recovery/README.md) records the bounded chapter-specific wait correction and a separate cached-picture Resume race reproduced with four failing baseline cases. Earlier native selection/foreground observations retain their original limits. Final-source and public gates remain outstanding.
