...
async function setup(t, initial = { textFace: 'plain', textSize: 'large', reducedEffects: true }) {
  ...
  const htmlURL = new URL('../../authoring/still-media/index.html', import.meta.url),
    ...
    context = createContext({ document: h.doc, window: h.win }),
    modules = new Map();
  ...
  for (const url of entries) {
    ...
    await module.link((specifier, parent) => {
      assert.ok(specifier.startsWith('.'), 'The reading graph stays local.');
  ...
  return {
    ...
    external(value, notify = true) {
      ...
      if (notify)
        h.win.emit('storage', { key: DISPLAY_PREFERENCES_KEY, newValue: raw, storageArea: storage });
    },
    flush() {
