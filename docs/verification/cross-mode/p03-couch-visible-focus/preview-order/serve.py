...
BASE = '063ff5d650b6e071d2b59e89fa6e9fb5058b3f08'
HOLD = 'b56ec366d73eaf51067443ebedca035e139399cb146d506f77da461ecc8bbb9b'
LIMIT = 64 * 1024 * 1024
if shutil.disk_usage(OUT).free < 256 * 1024 * 1024:
    raise RuntimeError('Insufficient metadata reserve')
raw = (OUT.parent / 'source-held-02.json').read_bytes()
...
binding = {'base': BASE, 'baseTree': tree, 'sourceHoldSha256': HOLD,
           'overlays': [rows[name] for name in overlays], 'candidateCouchVisibleFocus': True,
           'pid': os.getpid(), 'port': server.server_port, 'url': f'http://127.0.0.1:{server.server_port}/game/couch/',
           'gitBlobCacheBytes': LIMIT, 'maxConcurrentRequests': 4,
           'helperSha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
           'scope': 'Exact Git063ff5d combined P05-B/P08 static source with one immutable Couch shell overlay from source-held-02, including synchronous result content before transition focus. Normal fresh origin, no app state, game-clock or delayed-response fixture. Team host adoption and later P03 work are absent. Root owns actual responsive browser proof; this binding is not native, phase or public acceptance. The 64 MiB LRU is stored Git bodies, not a total RSS claim.'}
