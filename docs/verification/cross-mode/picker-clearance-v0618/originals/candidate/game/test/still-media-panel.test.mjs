...
test('settled chosen preview keeps the focused original picker below its sticky status rail', async (t) => {
  ...
  layout.flush();
  assert.equal(h.panel.dialog.scrollTop, 682.28125);
  assert.equal(h.$('file').getBoundingClientRect().top, 244.59375);
  ...
  layout.resize();
  layout.flush();
  assert.equal(h.$('file').getBoundingClientRect().top, 177);
