...
// Finite layout rectangles from the public 390×844 picker finding. These tests
// establish the mounted clearance owner, not browser paint or physical input.
...
test('settled chosen preview keeps the focused original picker and label below its sticky status rail', async (t) => {
  ...
  h.$('operations')._rect = { x: 36, y: 31, width: 772, height: 138 };
  h.panel.dialog.scrollTop = 1000;
  layout.resize();
  layout.flush();
  assert.equal(h.$('file').parentNode.getBoundingClientRect().top, 177);
  assert.equal(h.$('file').getBoundingClientRect().top, 204.59375);
