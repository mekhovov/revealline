...
r=Path.cwd();c=r/'.cache/p05-picker-clearance-next';d=c/'label-clearance-r2';w=r/'.cache/worktrees/p05-picker-clearance-next';source='4fd8e2dac4fdc851d0d2bf0b21e77162d9d405c9';tree='d16edac0127fc96139445ae2365cc74499f80921'
...
for name in ['binding.json','runtime-overrides.json','serve.py']:
 put(d/'native-r2-server'/name,(c/'native-server-r2'/name).read_bytes())
records=[]
for v in ['20.19.5','22.22.2']:
 q=json.loads((d/f'exact-rectangles-node{v}.json').read_text());assert q['exitCode']==0 and q['counts']=={'tests':103,'pass':103,'fail':0,'cancelled':0,'skipped':0,'todo':0};records.append({**q,'original':pin(d/f'exact-rectangles-node{v}.log')})
obj(d/'verification-summary.json',{'status':'EXACT_PORTRAIT_AND_LANDSCAPE_SOURCE_REGRESSIONS_PASS_NATIVE_REVIEW_PENDING','tests':records,'retainedRed':pin(d/'label-regression-red-node22.log'),'originalNativeFinding':pin(d/'root-native-r1.original.json'),'originalR1Candidate':pin(c/'ready.json'),'nativeCandidateRunByThisAgent':False,'fullSuiteRun':False,'buildRun':False,'sourceVersionChanged':False})
