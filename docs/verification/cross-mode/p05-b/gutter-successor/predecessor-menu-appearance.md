...
unchanged. Browser inspection must still confirm decoration stays outside text
and focus outlines after layout and scrolling.
...
unchanged finite input pins, static checks and earlier diagnostics.
...
Before browser acceptance, inspect all three modes' Title/lobby, Options, paused
and confirmation menus in both text faces and sizes, including a late Music
Library dialog. Exercise all ornament choices, keyboard focus and disabled/loading,
primary, danger and hover states with the actual compiled frame assets. Measure
computed colours/opacity, text, outlines and target sizes, then narrow/short layout
and actual browser zoom. Verify real cross-page storage, denied saving and the
unchanged system-motion policy separately. Resume remains explicit. Physical
