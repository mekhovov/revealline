...
repo=root.parents[2]
source=repo/'.cache/worktrees/p03-enemy-workshop-v0613'
...
before=inputs(); ident=identity(); assert ident['branch']=='codex/p03-enemy-workshop-v0613'; initial=budget(); argv=[node,'--max-old-space-size=1024','--test','--test-concurrency=1',*tests]
start=time.monotonic();process=subprocess.Popen(argv,cwd=source,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,start_new_session=True,env={**os.environ,'GIT_NO_LAZY_FETCH':'1'})
