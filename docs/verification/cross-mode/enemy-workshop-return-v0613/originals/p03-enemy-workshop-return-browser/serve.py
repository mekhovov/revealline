...
FAULT_PATHS=('game/content/themes.json',)
assert list(FAULT_PATHS)==PLAN['faultPaths'], 'Use the exact reviewed Enemy Workshop theme index paths'
def record(event):
...
def read_faults():
    ...
    if not isinstance(delays,dict) or set(delays)-set(FAULT_PATHS): raise ValueError('Fault delay outside reviewed Enemy Workshop theme index')
    if not isinstance(failures,list) or len(failures)>1 or any(not isinstance(x,str) or x not in FAULT_PATHS for x in failures): raise ValueError('Fault failure outside reviewed Enemy Workshop theme index')
    if len(set(failures))!=len(failures): raise ValueError('Duplicate theme failure')
