...
BASE = '4397b367b3f1fa17bf4a0989c90febaca9de9710'
HOLD = '1294a7841e93db192a88c2fe858b0111ad5f4282e6bd333b2b9df3cdc468bad6'
LIMIT = 64 * 1024 * 1024
...
overlays = {}
for name in ('game/app.mjs', 'game/ui/library-panel.mjs'):
    ...
    overlays[name] = body
html = subprocess.check_output(['git', 'show', BASE + ':game/index.html'], cwd=ROOT)
if hashlib.sha256(html).hexdigest() != rows['game/index.html']['sha256']:
    raise RuntimeError('Base HTML mismatch')
...
binding = {'base': BASE, 'baseTree': tree, 'sourceHoldSha256': HOLD,
           'overlays': [rows[name] for name in overlays], 'baseHtml': rows['game/index.html'],
           ...
           'scope': 'Exact Git4397 with two immutable memory overlays pinned at held02. HTML and all other regular assets from Git. No simulated state/responses, timing fixtures, P05 CSS, P08 or A2/R5 overlays. LRU cap describes stored Git blobs, not total process RSS.'}
