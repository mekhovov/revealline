...
def pins():
 ...
 return out
before=pins();(cache/'difficulty-whole-before.json').write_text(json.dumps(before,indent=2)+'\n')
tests=['difficulty-host'];runs=[]
for version in ['22.22.2','20.19.5']:
 ...
 node=f'/Users/oleksandr.mekhovov/.local/share/mise/installs/node/{version}/bin/node';args=[node,'--test']+[f'game/test/{n}.test.mjs' for n in tests];name='difficulty-whole-node'+version.split('.')[0]+'.log';start=time.monotonic()
 ...
 row={'node':version,'argv':args,'exitCode':r.returncode,'wallSeconds':elapsed,'freeBefore':free,'log':name};runs.append(row);(cache/'difficulty-whole-execution.json').write_text(json.dumps({'before':'difficulty-whole-before.json','inputsUnchanged':True,'runs':runs},indent=2)+'\n');print(json.dumps(row),flush=True)
 if r.returncode:break
(cache/'difficulty-whole-after.json').write_text(json.dumps(pins(),indent=2)+'\n')
