...
BASE='b6502bfc71d838ee1500de98f2103d7cfcc4ac49'
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  try:
   ...
   if name == 'game/replay-theater/data/fieldcraft-01.replay.json':
    with (HERE/'source-delay.jsonl').open('a') as log:log.write(json.dumps({'event':'request','path':name,'held':(HERE/'hold-example').exists(),'time':time.time()})+'\n')
    deadline=time.monotonic()+120
    while (HERE/'hold-example').exists() and time.monotonic()<deadline: time.sleep(0.05)
   with lock:
