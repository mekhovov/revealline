...
const RETAINED_ADMISSION_SHA256 = {
  ...
  "archive-31": "139d06e0ca92f6f3c3ce0c840d8d4170c676787c79a89e05375cc74a176c5fbc",
  "archive-32": "cbbb72dfe75d69be866263a7540084d5253eae3d328eea685de4121465ae6293",
  "archive-33": "0f7e4c0e27712bb72e8ec6ab3144a7c328e18185cbc95fc6c50aae9ee21f6cad",
...
export function validateAdmissions(configuration, receipt) {
  require(Array.isArray(configuration.admissions) && configuration.admissions.length === 37 &&
    receipt.admittedArchives === 37 && Array.isArray(receipt.observations) && receipt.observations.length === 37,
    'Exactly37 archive admissions and observations required.');
  const expectedIds = Array.from({ length: 37 }, (_, i) => 'archive-' + String(i + 1).padStart(2, '0'));
  const ids = configuration.admissions.map(a => a.id), observedIds = receipt.observations.map(a => a.archiveId);
  require(new Set(ids).size === 37 && new Set(observedIds).size === 37 &&
  ...
  for (const a of configuration.admissions) {
    ...
    for (const e of a.evidence) {
      safePath(e.path);
      require((!['archive-36', 'archive-37'].includes(a.id) || e.path.startsWith('evidence/' + a.id + '/')) && !paths.has(e.path) && sha256(e.sha256) &&
