...
The first v0.57.2 candidate failed one production-history capacity test locally and on GitHub. Its registry fits the unchanged 4 MiB limit; conservative JSON accounting incorrectly rejected a complete immutable replacement. The [capacity correction](verification/cross-mode/p01/production-json-capacity.md) preserves every revision and byte budget and passes all 224 focused checks. The corrected source needs fresh qualification; the failed commit and its other passing gates cannot qualify the release.
