...
The game includes 70 creator recordings in 15 built-in albums. Open **Settings → Audio → Music library & playlists**, choose **Save & use album**, then **Play music**. Album selection preserves intentional Pause and master mute. Music styles and My Mix also include these recordings; built-in albums do not consume your custom playlist or upload slots.

For a custom sequence, clone an album in the playlist editor, add or remove songs, choose ordered or shuffled playback and a repeat mode, then save. You can mix catalogue songs with your own MP3 uploads. Now Playing shows the audible song, artist, original filename and creator/license websites.
