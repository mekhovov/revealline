...
milestone into the title. It preserves an existing `input` role when the
milestone version changes, and removes the prefix when the version milestone is
removed. It never chooses or increments a version. Assign the milestone first;
add `input` only when the PR is deliberately one part of a future aggregate.
