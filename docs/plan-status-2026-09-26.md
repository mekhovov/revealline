...
The second Team slice now has **10/10** focused checks. Every changed mission keeps one connected
...
This proves a concrete partner benefit; it does not prove full-clear pacing or fun.
...
|        2 | Complete second Team slice             | Full-clear/late-cleanup evidence, then separate/current edition, library/Studio/Next, EN/UK and version wiring             |                                              **12–24 hours** source work |
...
2. **Long-suite waiver:** focused checks are real; skipped long suites are not passes. Exact source,
   ...
   remain mandatory.
3. **Second Team cleanup:** inherited full-clear route logs do not fit the redesigned geometry.
   New full routes must be authored; opening solvability is not enough to rule out tedious cleanup.
...
3. Author and pin full-clear/cleanup routes for the three second-slice Team maps.
4. Only after those routes are acceptable, wire the v2 Team edition through entry, library, Studio,
   localization and exact Next ownership.
