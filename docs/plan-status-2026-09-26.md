...
- Latest GitHub release: **v0.132.4**. Its public frozen-build acceptance was not repeated in this
  lane, so the last independently recorded accepted public baseline here remains **v0.132.3**.
- PR #654 merged the v0.132.5 source, and PR #660 then repaired its stale Field Kit review ledger.
  ...
  public delivery.
- Corrective PR #663 is now running exact-head focused qualification for the v0.132.5 Team picture
  revision-90 binding. Its predecessor run failed closed before release creation, so no v0.132.5
  tag, immutable assets or public deployment exist yet.
...
| Crosswind cultural completion / PR #665          | Clean explicit v27 draft      | Redesigns Survey markers, Compass array and Outer loop with original wall fields informed by three official museum records. Preserves v25/default and v26 ownership, actors, foundations, arrows and objectives; reduces the post-opening open/plain queue from 18 to 15. Exact pushed head `07b670bcb` passes the sequential candidate/route/bootstrap/host cohort 155/155 with zero skips, repository validation, targeted lint/format and diff checks.                |
| Fracture/Apex cultural completion                | Local explicit v28 candidate  | Redesigns Bank the crossing, Five anchors and Final broadcast with original spatial fields informed by official Opishne museum, UNESCO Crimean Tatar Örnek and Museum Fund records. Preserves v25/default and v27 history, gameplay rules, objectives, relays, bonuses and actor roles; reduces the open/plain review queue from 15 to 12. Focused candidate and route-loader checks pass 103/103 with zero skips; host and repository qualification remain before push. |
...
8. **First cultural gate:** PR #530's rebase-triggered run passed preflight but failed four Team
   composite cases because current `main` advertises presentation revision 90 while the exact Team
   picture authority still stops at revision 88. The earlier Solo compact-filter case was not the
   cause of this run. Corrective PR #663 owns that mainline release defect and is in focused CI;
   PR #530 must rerun after the correction lands. This is a real hosted failure, not a pass.
...
12. **v28 remains local and opt-in:** the Fracture/Apex candidate preserves v25 as default and v27
    as history. Its focused 103-test candidate/route cohort passes, but host integration, repository
    validation, review, promotion and public evidence are still required.
...
3. Let PR #663 correct Team picture revision-90 authority, then rerun PR #530 from the accepted main;
   do not reinterpret its four Team failures as a cultural-route defect or a pass.
...
8. Finish host and repository qualification for the v28 Fracture/Apex batch, push its bounded PR,
   then select the next small batch from the 12 remaining audit candidates. Keep intentionally open
   boards when openness is the distinguishing decision and never add geometry merely to satisfy a
   count.
