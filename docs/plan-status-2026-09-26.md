...
| Crosswind cultural completion / v27 preparation  | Local explicit review slice   | Redesigns Survey markers, Compass array and Outer loop with original wall fields informed by three official museum records. Preserves v25/default and v26 ownership, actors, foundations, arrows and objectives; reduces the post-opening open/plain queue from 18 to 15. The sequential candidate/route/bootstrap/host cohort passes 155/155 with zero skips; validation, targeted lint/format and diff checks pass. |
...
|        4 | Whole-Journey pacing and accessibility        | PR #661 establishes the exact-current campaign/spatial inventory; PR #664 and the local v27 Crosswind slice remove two three-mission repetition clusters. Redesign the remaining 15 candidates selectively, then qualify reduced effects, contrast, small screens, performance and deterministic compatibility |       **2–4 working days** for further targeted batches and accessibility qualification |
...
11. **v27 remains local and opt-in:** the Crosswind completion slice registers a separate v27 review
    identity while keeping v25 as default. Its 155-test sequential cohort and repository validation
    pass, but it still needs a clean commit, stacked PR and hosted exact-head qualification.
...
8. Finish the v27 Crosswind completion PR, then select the next small batch from the 15 remaining
   audit candidates; keep intentionally open boards when openness is the distinguishing decision and
   never add geometry merely to satisfy a count.
