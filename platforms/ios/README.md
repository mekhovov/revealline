...
The development-only `@capacitor/cli → xcode@3.0.1 → uuid` dependency is overridden to `uuid@11.1.1`, the first patched CommonJS-compatible release for [GHSA-w5hq-g745-h8pq](https://github.com/uuidjs/uuid/security/advisories/GHSA-w5hq-g745-h8pq). The advisory affects the `v3`, `v5` and `v6` buffer APIs; this Xcode parser uses only `v4()` without a supplied buffer. The override stays scoped to this exact parent version because its upstream dependency still requests uuid 7. Capacitor and native plugin versions stay pinned. Reassess the override when upgrading the CLI or Xcode parser; do not replace it with an unqualified latest-major override. [Xcode dependency record](https://registry.npmjs.org/xcode/3.0.1), [patched uuid record](https://registry.npmjs.org/uuid/11.1.1)

For this tooling change, locked installation, the existing iOS Node tests, CommonJS project parsing/serialization and ID generation, and a bridge rebuild are the relevant local checks. Neither `xcode` nor `uuid` enters the browser game or native bridge bundle. These checks do not establish Xcode compilation or device behavior; the native acceptance gates below remain open.
