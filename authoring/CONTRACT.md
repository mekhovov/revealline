...
This is a concrete, validated **design foundation**, not the game runtime. It lets us review theme families, art choices and rule combinations before implementing them. The catalog explicitly records that its behaviors are specified and not implemented. All four example packs are drafts and all referenced production assets are planned briefs.

The [Round 05 gameplay audit](../docs/round-05-gameplay-direction.md) adds design recommendations, not new v0.1 capabilities. Local terrain, changing-contour patrols, claimed-space enemies, erosion, contact pickups, timed statuses and actual medal evaluation remain [explicit extension requests](challenges/round-05-reference-adjustments.md). The existing schema and behavior IDs retain their meanings.
