...
Generation stores a versioned template, bounded variant, seed, runtime seed, mission ID and gameplay policy. `creator-crossing.v1` has center/west variants of First crossing. Image bytes and filenames do not affect geometry. Automated evidence is regenerated on import; it identifies actual simulation identities, presets, steering and runtime seeds. It establishes configuration feasibility only.
