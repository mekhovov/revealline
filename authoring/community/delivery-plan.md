...
| 2     | Batch images, ordering/pacing, groups, checkpoints and bulk approval                          | 1/12/50 items, duplicates, portraits, per-item failure, cancellation, bounded memory and capacity splitting     | Pending                                                                |
...
See the [Phase 1 walkthrough](image-campaign-guide.md), [portable/source format reference](creator-bundle-reference.md), and [exact acceptance observations and open gates](phase1-acceptance.md). Phase 1 currently supports one image, one mission and twelve verified layout variants across six families. Batches, video, hosted publication and multiplayer are not advertised by the candidate.
