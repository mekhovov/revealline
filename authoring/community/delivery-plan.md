...
`main` at `260303221144aa0430fa78f5fdcda639d1251431` after the accepted `v0.132.3` selector and now carries source version
...
| Phases 4–5           | Community account, upload, validation, publication, catalog and immutable install/update/offline source paths are implemented and covered by the combined runtime/service suites. Exact-edition offload retains the immutable manifest, saves and rewards, deletes only unreferenced managed bytes, and reinstalls from a verified retained package without network access.                                                                                                                        | Complete the built-in-browser offload/reinstall pass, then deploy the service and pass the live PostgreSQL/proxy/mail/blob restore and two-user publish/install/offline gates. | Browser pass: under 1 hour; deployment: 0.5–1 day after infrastructure is available            |
...
| Follow-up review     | The implementation is rebased on current `main`; focused audio tests pass 34/34 with a real built-in-browser conversion, and offload tests pass 15/15 plus 61/61 related lower-level checks. Earlier exact source gates passed 524/524 creator/runtime tests, 43/43 service checks, validation, lint and formatting before these final slices.                                                                                                                                                     | Run all combined gates on the final exact head, push it, and complete hosted review and CI.                                                                                    | About 1–2 focused hours plus CI/reviewer time                                                  |
...
- PR #564 is allocated `v0.141.0`. Its new exact head must be pushed and pass
