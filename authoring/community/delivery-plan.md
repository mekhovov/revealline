...
| 6     | Versus qualification                                                                          | Both boards, equal conditions, results, Retry/Next and transfer                                                 | Generated equal-board replay and portable-package qualification merged; installed project-backed Versus campaign route is in progress         |
...
`main` at `c45cd78bdc522bf95c267615fe3ad93dd77dff98` (`0.132.1`) and is assigned to release
...
| Phase 6 Versus       | All twelve current generated variants have equal-board replay evidence for three difficulties and both steering policies, and `.rlpack` import repeats that evidence.                                                                                                                                                                                                                                                             | Finish and accept installed project-backed Versus selection, Retry/Next, progression, transfer and reload behavior.                                   | In progress; 0.5–1 focused engineering day                                            |
...
| Follow-up review     | The hardening commits are rebased on current `main`; 520/520 creator/runtime checks, 43/43 service checks, the formerly failing 194/194 navigation cohort, validation for 1,243 files, localization, lint and formatting pass.                                                                                                                                                                                                    | Commit and push the focused input-handoff repair; complete hosted review and the assigned v0.141.0 CI build.                                          | 1–2 hours plus CI/reviewer time                                                       |
...
- The rebased navigation failure is resolved locally: the exact four-file CI command passes
  **194/194**. The repair also closes a runtime handoff bug where controller Confirm could remain
  ...
  pushed commit.
- Installed creator play currently follows the project-backed Solo path. Equal-board route proof in
  a generated package does not yet establish installed Versus campaign UX, progression or recovery.
