...
`12978e5fd3fe0ce70bbee96aa543f569f64622d4` and remains the current Pages release,
...
The latest stable GitHub release is
[`v0.141.3`](https://github.com/mekhovov/revealline/releases/tag/v0.141.3), published by coordinator
run `36254346397` from PR #679's exact merged source
...
| S3/AWS expansion     | The package-store byte boundary is tested, and PostgreSQL already coordinates tus locks, expiry leases and publication state independently of storage. This workstream does not block the supported single-host filesystem deployment.                                                                                                                                                                                                                                | Add a storage factory, stream-safe verified staging, real tus S3 datastore and cleanup, S3 readiness and recovery, MinIO integration, and an AWS smoke run. | 4–6 focused engineering days, including 0.5–1 day with AWS access |
...
| Publication          | Stable `v0.141.3` is published from exact PR #679 source `7509d790…` with nine assets through coordinator run `36254346397`. Archive 95 preserves `v0.141.0`. Pages still selects `v0.141.2` through PR #677 at `34f45503…`; deployment `6680716352` / run `36252239829` passed with 1,888 files, 630,471,797 bytes, zero retries and zero failures.                                                                                                                  | Select and audit one cumulative Pages release after the v0.141.4 aggregate is published.                                                                    | Coupled to the v0.141.4 release gate                              |
...
- S3 is a separate post-deployment workstream for multi-host or AWS operation; it does not block the
  initial single-host launch. Executable S3 support requires a shared storage factory, bounded
  stream-safe verification before immutable package publication, the maintained tus S3 datastore
  with deterministic completed-upload cleanup, dependency readiness probes, S3-aware backup and
  restore, a MinIO end-to-end rehearsal, and a real AWS smoke run. Estimate 4–6 focused engineering
  days: 2–3 for runtime/configuration and verified staging, 1.5–2 for recovery and MinIO coverage,
  and 0.5–1 with provisioned AWS buckets and IAM access for the final smoke evidence.
...
- `v0.141.3` is the highest published stable version; Pages still selects `v0.141.2`. Earlier global working allocations are
