...
`main` at `80911bf5ec0f6df711d7e70b36a11005d2be4c3a`, including the accepted `v0.132.3`
selector, Team artwork timeout adjustment and Fastline inspection-authority repairs. It carries
...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Phases 0–3           | The guide, image and batch generation, mixed video campaigns, `.rlpack`, and installed Solo Custom progression are merged through PR #465. Current generation uses twelve verified threatened variants with enemies and collision obstacles. The combined candidate adds complete mixed-media review cards, explicit include/exclude/reorder/split controls, and conflict-safe `.rlsource` recovery of exact images, videos, pairings, posters and playback ranges. Built-in-browser AVC+AAC upload/playback, active-playback reload, a 1–5 second range, exact full-video package retention, separate-origin import and explicit unsupported-codec refusal pass. Automated acceptance covers 1/12/50 items, duplicates, portraits, failed items, cancellation and capacity splitting. | Complete the combined PR/release gates and the final built-in-browser batch edge-case record. Physical-speaker audio remains follow-up acceptance; missing-original recovery is accepted at deterministic byte-store scope. | Local implementation complete; browser record about 1 focused hour                          |
...
| Phase 8 media editor | Silent MP4/WebM input and single-track AVC+AAC MP4 can be physically trimmed or converted to AVC MP4 with bounded Balanced/Compact resize and compression. Exact duration, dimensions, orientation, codec, hash, bitrate, decoded audio windows, A/V endpoints and visual boundaries are verified. The owned AAC fixture passed a real built-in-browser 1–5 second conversion; automated matrix coverage now includes Balanced and Compact landscape and portrait transforms. | Record the Balanced portrait path in the built-in browser; qualify Firefox, Safari, broader formats and physical mobile playback.                               | Built-in-browser record about 1 focused hour; other browsers/devices 1–2 days when available   |
| Follow-up review     | The implementation is rebased on current `main`. After the rebase, the focused creator/media matrix passes 127/127; the added batch/orientation slice separately passes 36/36 plus its split re-import case, lint and formatting. Earlier complete-head evidence remains 530 top-level/532 total creator/runtime tests, 43/43 service checks, validation, both dependency audits and the 1,303-file build, and must be rerun on the final exact head. | Rerun complete local gates after the predecessor selector lands, push the rebased exact head, and complete hosted review and CI.                                | 2–4 focused hours plus hosted CI                                                               |
| Publication          | `v0.132.3` is published and selected. Archive 92 now preserves it and passed a complete 1,307-file/611,465,266-byte public audit. The `v0.132.4` predecessor is in a corrected Fastline qualification/publish sequence. PR #564 retains the assigned `v0.141.0` target and is rebased onto current protected `main`. | Publish/select/verify v0.132.4, rebase once onto that selector, merge #564, then freeze, publish, select and publicly verify v0.141.0.                           | 4–8 publisher hours plus hosted CI                                                             |

## Blockers and concerns

- The `v0.132.3` prerequisite is complete: the release has nine exact assets, selector PR #633
  merged, Pages deployed, and the public byte audit reported zero failures. Archive 92 preserves
  that edition and its complete public audit also reports zero failures.
- `v0.132.4` is the immediate release dependency. Its first resume exposed two Fastline evidence
  authority defects. PRs #644 and #645 repaired them without weakening exact-source or duplicate
  authority checks; the corrected latest-main qualification is running before publication resumes.
- PR #564 is allocated `v0.141.0`. The local branch is rebased on the latest protected main and is
  deliberately not force-pushed until the predecessor selector stops moving. Its eventual exact
  pushed head must pass the hosted `release-ready` gate; checks from earlier heads are supporting
  evidence only.
