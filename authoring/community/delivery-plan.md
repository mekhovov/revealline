...
| S3/AWS expansion     | The executable storage selector now provides stream-verified immutable S3 package publication, the pinned maintained tus S3 datastore, bounded multipart work, deterministic completed-upload cleanup, and fail-closed object/multipart readiness. PostgreSQL continues to coordinate tus locks, expiry leases and publication state independently of storage. This workstream does not block the supported single-host filesystem deployment.                        | Add S3-aware backup/restore and a MinIO end-to-end rehearsal, then run the credentialed AWS smoke gate.                                            | 2–3 focused engineering days, including 0.5–1 day with AWS access |
...
- S3 is a separate post-deployment workstream for multi-host or AWS operation; it does not block the
  ...
  multipart work and deterministic completed-upload cleanup, and readiness proves object plus
  multipart operations. S3-aware backup/restore and a MinIO end-to-end rehearsal remain 1.5–2
  focused days. The real AWS smoke run remains 0.5–1 day after a private bucket and scoped IAM
