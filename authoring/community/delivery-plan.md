...
See the [Phase 1 walkthrough](image-campaign-guide.md), [batch campaign walkthrough](batch-image-campaign-guide.md), [portable/source format reference](creator-bundle-reference.md), [Phase 1 acceptance record](phase1-acceptance.md), and [Phase 2 candidate acceptance record](phase2-acceptance.md). The Phase 1 candidate supports one image and one mission selected from twelve verified variants across six families. Phase 2 batch behavior is not advertised until its separate release passes every recorded gate.
