...
| Community hardening  | PostgreSQL-backed admission, tus locks and leases, isolated validation, exact blob checks, account mail flows, moderation APIs, backup and restore are released. The current follow-up adds a bounded administrator queue, explicit preview, resolve and unlist workflow.                                                                                                            | Run live multi-process PostgreSQL, proxy interruption, shared blob/S3, mail delivery, database/blob restore, moderation and the two-user journey. | 0.5–1 day after infrastructure is available                   |
