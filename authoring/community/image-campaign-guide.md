...
3. Choose **Generate and review**. The creator prepares an oriented 1280 × 640 PNG, a small thumbnail, and a bounded First crossing template. It verifies a legal route at the actual compiled identity and runtime seed for Gentle, Standard and Expert, with both steering modes.
4. Review the picture, map, title and credits. The introductory template has no enemies: reaching safe ground after crossing the field reveals the picture. Automated completion establishes feasibility for these configurations, not human playtesting or a quality rating.
...
- Titles, credits and artwork can be revised without changing simulation identity, but the resulting pack still needs visual review. Gameplay changes cannot borrow the template's old route evidence. Keep the edit as a draft, or choose **Regenerate template gameplay** to return to a verified variant. Supplying an arbitrary custom completion recording is not exposed by the Phase 1 UI.
