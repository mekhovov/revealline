...
The batch review controller is isolated in `game/creator/batch-ui.mjs`. Its `prepareItem`, `approveBatch`, `capacityFor`, and `onSplit` functions are injected so the UI never invents package semantics. `studio.mjs` currently keeps multi-file intake gated while `batchApprovalAdapter` is `null`. The Phase 2 batch compiler must supply `batchApprovalAdapter.approve(items, settings)` before enabling intake; each item includes its exact prepared one-mission bundle and normalized image result. That adapter must assemble and approve one exact campaign edition, or return an item-specific error without mutating the reviewed results.
