...
For the scoped Studio readability candidate, use **Interface** to compare Theme/Plain and Standard/Large host text while keeping the selected asset specimen unchanged. Select the second filtered inventory row with Enter, then Tab; desktop focus should continue from its selected replacement, while a narrow foreground selection hands off to Inspector. Adjust Interface settings with a real replacement prepared and a sprite dirty without changing that work. Follow the [complete bounded prompt](cross-mode-delivery.md#qualify-studio-readability-and-draft-preservation) for exact font bytes, motion/storage ownership and evidence limits. This does not establish complete Studio controller or physical-device acceptance.
