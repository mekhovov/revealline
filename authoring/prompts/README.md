...
For applied changes to the playable `game/`, pair these briefs with [Runtime Maintainer](../skills/xonix-runtime-maintainer/SKILL.md) and the [actual asset/configuration formats](../../docs/assets-and-configuration.md). The shared CLI has 124 templates. Reference Importer's [10 prose examples](../skills/xonix-reference-importer/references/import-and-style.md) are a separate guide, not additional CLI IDs. Legacy draft packs, media libraries, motion-lab fixtures and playable scenarios are different formats; select the target before adapting a prompt.

Version 0.1.0 · 12 September 2026 · 124 reusable examples (56 base + 16 source/asset variations + 16 animation variants + 16 character-collection templates + 20 ability/world templates) · templates, not executed runs.

The catalog supports four themed families: **FPV FRONT** (main Ukrainian military theme), **UKRAINE ATLAS**, **1994 FOREVER**, and **NAVI NETWORK**. The same gameplay overlay can sit above pixel-art, flat illustration, painterly or photographic reveal images. Background art never defines collision geometry or actual enemy positions.
...
Skills are repository-local authoring workflows. The library does not imply that a game runtime, a music generator, a layout fixture or an Aseprite project already exists. Inspect required inputs before selecting an execution prompt.
