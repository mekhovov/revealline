...
The current [authoring contract](../CONTRACT.md) and registered primitive catalog win over any example wording about richer mechanics. Seeds are optional generation provenance for already materialized grids; no generator runs today. Optional medals, preview modules, true pursuers, swept hazards and linked-pair mechanics are design proposals until their primitives exist. A stationary illustrated emitter can instead be a current-schema capturable marker that clears a modifier or disables tagged enemies. Do not add unknown schema properties to make a prose example appear implemented. `shared-15` takes `LEVEL_COUNT` explicitly.
