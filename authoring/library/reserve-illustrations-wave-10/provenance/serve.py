"""Serve only the wave 9/wave 10 comparison and eight selected original PNGs."""
import argparse
import json
import stat
...
def main():
    ...
    parser.add_argument('--previous', type=Path, default=cohort.parent / 'reserve-illustrations-wave-9')
    args = parser.parse_args()
    current = json.loads((cohort / 'manifest.json').read_text())
    previous = json.loads((args.previous / 'manifest.json').read_text())
    if current['wave'] != 10 or previous['wave'] != 9:
        raise ValueError('Expected exact wave 9 / wave 10 manifests')
