"""Serve only the wave 3/wave 4 comparison and eight selected original PNGs."""
import argparse
import json
import stat
...
def main():
    ...
    parser.add_argument('--previous', type=Path, default=cohort.parent / 'reserve-illustrations-wave-3')
    args = parser.parse_args()
    current = json.loads((cohort / 'manifest.json').read_text())
    previous = json.loads((args.previous / 'manifest.json').read_text())
    if current['wave'] != 4 or previous['wave'] != 3:
        raise ValueError('Expected exact wave 3 / wave 4 manifests')
