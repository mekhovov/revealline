...
**Successor integration:** this exact prepared pack is now listed in the source **More worlds** catalog for explicit Install and separate Choose. Its complete body joins the optional manifest/ZIP distribution, while raw originals and the body stay outside automatic core-cache preparation. Active/archive indexes and default Pressure Lines remain unchanged. The original preparation description below, the pack's creation-stage metadata and `manual-optional-import` distribution record are retained without rewriting any pack/proof/original bytes. Frozen/public installation, earned-picture and cold-offline acceptance of this integration remain separate gates. See the [current supported list and storage limits](../../../docs/optional-worlds.md).
