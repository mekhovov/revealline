...
import { buildCountercurrent, IDS, CLASS_IDS } from './build.mjs';
import { routeCommands, drive } from './controls.mjs';
...
export async function executeCountercurrentRoute(
  source,
  context,
  identities,
  levelId,
    ...
    routeCommands(levelId, which, context.difficulty),
    {
      ...
      onTick({ run, recorder, input }) {
        ...
        for (const enemy of run.enemies)
          if (enemy.type === 'lane-boss' && ['warning', 'active'].includes(enemy.bossPhase))
            labels.push(`${enemy.id}-${enemy.bossPhase}`);
        const index = Math.floor(run.player.y) * run.width + Math.floor(run.player.x);
        if (run.player.cutting && run.classic.terrain[index] === 1) labels.push('slow-crossing');
        ...
        for (const label of labels)
          if (!boundaries.has(label)) {
            ...
            const session = suspendSession({
              ...
              runId: `cc-${context.difficulty}-${turnPolicy}-${levelId}-${classId}-${which}`,
  ...
  assert.equal(run.classId, classId);
  assert.equal(run.activeClassId, classId);
  assert.ok(boundaries.has('unfinished'), 'Actual unfinished prefix is required.');
  if (['north', 'south', 'recovered'].includes(which))
    assert.ok(metrics.stopChecks > 0, 'Win must exercise capture stopping.');
  if (['north', 'south'].includes(which))
    assert.equal(
      metrics.events['powerup.collected'] ?? 0,
      0,
      'Ordinary solutions must not require a bonus.',
    );
  ...
  return {
    id: `${context.difficulty}/${turnPolicy}/${levelId}/${classId}/${which}`,
    levelId,
    classId,
    route: which,
...
/** Finite authored matrix, no search. Recording refuses until every required context passes. */
export async function verifyCountercurrent({ candidate, record = false } = {}) {
  ...
  if (owned) {
    exactKeys(owned, ['pack', 'proof'], 'Countercurrent request');
    assert.ok(plainObject(owned.pack) && plainObject(owned.proof));
  }
  const source = await buildCountercurrent();
  if (owned) assert.deepEqual(owned.pack, source.pack, 'Exact Countercurrent source required.');
  ...
  const header = {
    format: 'revealline-countercurrent-proof.v1',
  ...
  const ids = ['standard', 'gentle'].flatMap((d) =>
    ['immediate', 'grid-center'].flatMap((t) =>
      IDS.flatMap((l) =>
        CLASS_IDS.flatMap((c) =>
          (c === 'scout' ? ROUTES : ['north', 'south']).map((r) => `${d}/${t}/${l}/${c}/${r}`),
        ),
      ),
    ),
  );
  if (expected) {
    ...
    assert.deepEqual(
      ...
      ids,
      'Exact 192-context matrix and all seven selectable classes required.',
    );
    assert.deepEqual(
      expected.gameOverControls?.map((r) => r.id),
      ['immediate', 'grid-center'].flatMap((t) =>
        IDS.map((l) => `standard/${t}/${l}/scout/gameover`),
      ),
    ...
    for (const policy of ['immediate', 'grid-center'])
      for (const levelId of IDS)
        for (const classId of CLASS_IDS)
          for (const which of classId === 'scout' ? ROUTES : ['north', 'south'])
            routes.push(
              await executeCountercurrentRoute(
                source,
                context,
                identities,
                levelId,
  ...
  const proof = {
    ...
    summary: {
      ...
      selectableClasses: [...CLASS_IDS],
      ordinaryContextsPerClass: 24,
  ...
  if (expected)
    assert.deepEqual(
      proof,
      expected,
      'Exact actual Countercurrent outcomes and saved suffixes required.',
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  console.log(
    JSON.stringify(
      (await verifyCountercurrent({ record: args[0] === '--record' })).summary,
      null,
      2,
    ),
  );
