...
export const PROOF_FILE = 'authoring/library/fracture-theme-chapters/routes.json';
export const descriptorFile = (themeId) =>
  `authoring/library/fracture-theme-chapters/descriptors/${themeId}.json`;
...
export async function verifyFractureThemes({ candidate, record = false } = {}) {
  ...
  if (owned) {
    exactKeys(owned, ['descriptors', 'proof'], 'Fracture theme request');
    assert.ok(
      Array.isArray(owned.descriptors) &&
        owned.descriptors.length === 3 &&
  ...
  const header = {
    format: 'revealline-fracture-theme-proof.v1',
    sourceProof: { path: SOURCE_PROOF, sha256: INPUTS[SOURCE_PROOF] },
    sourcePackSha256: SOURCE_PACK_SHA,
  };
  if (proof) {
    exactKeys(proof, [...Object.keys(header), 'editions', 'summary'], 'Fracture theme proof');
  ...
  for (const [i, themeId] of THEME_IDS.entries()) {
    const world = await buildFractureTheme(themeId);
    ...
    for (const key of [
      ...
      assert.equal(summary[key], base.summary[key], key);
  ...
  assert.equal(new Set(descriptors.map((d) => d.campaignKey)).size, 3);
  assert.equal(new Set(descriptors.flatMap((d) => d.originals.map((o) => o.sha256))).size, 9);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const proof = await verifyFractureThemes({ record: process.argv[2] === '--record' });
