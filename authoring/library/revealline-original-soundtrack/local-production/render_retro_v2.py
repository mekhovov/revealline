...
VERSION = "sample-free-retro-renderer.2"
SR = 48000
BLOCK = SR // 2
MIB = 1024 ** 2
GIB = 1024 ** 3
...
def voice(inst, pitch, held, seed, articulation="normal", cents=0):
    ...
    oversample = 4 if inst == "guitar" else 2 if inst in {"lead", "bass", "keys"} else 1
    sr = SR * oversample
    ...
    if inst in {"guitar", "clean", "pluck"}:
        ...
        # Short, clear FM keys with a hollow transient and restrained overtones.
        phase = 2*np.pi*frequency*t
        modulation = 1.65*np.exp(-t/.075)+.16
        x = np.sin(phase+modulation*np.sin(phase*2))
        x += .12*np.sin(phase*3)*np.exp(-t/.07)
        x *= np.exp(-t/.25)*envelope(t,held,.0015,.065)
    elif inst == "lead":
        # Two parallel FM carriers give a brassy arcade hook a clean centre.
        # Operator envelopes decay independently rather than sweeping a saw pad.
        phase = 2*np.pi*frequency*t + .018*np.sin(2*np.pi*5.1*t)*np.minimum(1,t/.25)
        index = .70 + 1.5*np.exp(-t/.05)
        x = .78*np.sin(phase + index*np.sin(2*phase))
        x += .17*np.sin(phase + .5*np.sin(3*phase))*np.exp(-t/.40)
        x += .08*np.sin(phase*.5)
        x = lowpass(x, 7200, sr)
        x *= envelope(t,held,.0025,.065) * (.86+.14*np.exp(-t/.04))
    elif inst == "wind":
        ...
        phase = 2*np.pi*frequency*t
        # Fundamental plus percussive FM upper body: syncopation reads on small
        # speakers while preserving a centred, controlled low-frequency lane.
        x = .72*np.sin(phase)+.28*np.sin(phase+(1.8*np.exp(-t/.05)+.3)*np.sin(phase*2))
        x += .17*np.sin(phase*2)*np.exp(-t/.12)
        x = np.tanh(x*1.35)/1.1
        x = lowpass(x, 3100, sr)
        x *= envelope(t,held,.002,.035)*(.76+.24*np.exp(-t/.055))
    elif inst == "kick":
    ...
    if inst in {"lead", "bass", "keys"}:
        x = signal.resample_poly(x, 1, oversample)
    # Every event has a zero-edge attack/release, including synthesized cymbals.
    edge = min(48 if inst in {"kick", "snare", "hat", "bass", "lead", "keys"} else 240,len(x)//2)
...
def render(score, raw_path, scratch):
    ...
    for offset in range(0,frames,BLOCK):
        ...
        while cursor<len(events) and round(events[cursor]["beat"]*beat_seconds*SR)<offset+count:
            ...
            wet = {'bass':0,'kick':0,'hat':0,'snare':.12,'clap':.12,
                   'lead':.19,'keys':.24,'pad':.30}.get(e['instrument'],.22)
...
def produce(score_path, scratch, encoder_path):
    ...
    composer_hash = digest(ROOT/'compose_retro_v2.py')
    ...
    if (renderer_hash != digest(Path(__file__)) or score_hash != digest(score_path)
            or composer_hash != digest(ROOT/'compose_retro_v2.py')):
    ...
    receipt = dict(format='revealline-local-recording.v1',status='candidate-unreviewed',
        ...
        sources=dict(renderer=Path(__file__).name,composer='compose_retro_v2.py',score=str(score_path.relative_to(ROOT))),
