"""Serve only the wave 8/wave 9 comparison and eight selected original PNGs."""
import argparse
import json
import stat
...
def main():
    ...
    parser.add_argument('--previous', type=Path, default=cohort.parent / 'reserve-illustrations-wave-8')
    args = parser.parse_args()
    current = json.loads((cohort / 'manifest.json').read_text())
    previous = json.loads((args.previous / 'manifest.json').read_text())
    if current['wave'] != 9 or previous['wave'] != 8:
        raise ValueError('Expected exact wave 8 / wave 9 manifests')
