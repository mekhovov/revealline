...
This collection contains **70 third-party recordings in 15 optional albums**, not original RevealLine compositions. Recording licenses, exact bytes and codec structure were inspected; musical quality, full listening, loop seams and the in-game mix remain unapproved. All 70 are preview candidates, with **zero listening approvals** and an unchanged, empty [game publication list](publication.json). The nine new album titles include **Preview**; the six older albums have the same review boundary.

The separate [licensed MP3 preview archive](https://mekhovov.github.io/revealline-soundtracks-01/) is live for individual browser playback and downloads. **GitHub Pages deployment passed, and all 70 public MP3 URLs passed exact byte-count and SHA-256 verification.** Its 70 exact delivery MP3s total **354,986,122 bytes**; the 15 local `.rlsound` packs are not published there. The archive carries [per-recording credits and license notices](https://mekhovov.github.io/revealline-soundtracks-01/CREDITS.md), [preview metadata](https://mekhovov.github.io/revealline-soundtracks-01/preview-catalogue.json) and a [hash-pinned inventory](https://mekhovov.github.io/revealline-soundtracks-01/inventory.json). Public previews do not automatically join the game's catalogue or Community list. [Download and import guide](../../../docs/music-library-upload-guide.md).

Preview publication has separate [explicit authorization](preview-authorization.json), bound to both source registers and every exact recording hash. It does not assert full or repeated listening, in-game transitions, or musical approval. The [standalone preview builder](../../../scripts/build-licensed-preview-archive.mjs) validates this scope, CC0/CC BY evidence, hash-bound web/redistribution permissions, original and derivative lineage, MP3 facts, attribution and disk reserve without modifying the approved-publication compiler or game catalogue. AI music generation remains paused; rejected sketches and unknown-rights UA-FPV recordings are excluded. UA-FPV's private packs retain their [separate local-only guide](../../../docs/ua-fpv-upload-guide.md).
...
For review, import selected MP3 downloads manually, or use **Add album file to draft** for an existing local `.rlsound` pack in the new framework, then **Save all changes**. The [guide](../../../docs/music-library-upload-guide.md) distinguishes older-edition MP3 support from the new additive pack workflow. Local packs preserve album metadata and playlists; raw MP3 import requires entering credits and genre tags separately. Create a personal playlist or use the new style choices/My mix after tagging. Imported album playlists shuffle and repeat. Turn sound on to allow menu music, subject to the browser's first-interaction requirement. Pausing music preserves sound effects. Assess full and repeated listening before game catalogue admission.

Manually imported preview files are not trusted Community downloads. To reclaim their storage, back up first and remove unwanted tracks/playlists through the editor; those deletions affect references. **Remove offline download / Download again** preserve identities and references only for trusted albums identified by those controls. Do not assume public preview URLs grant that restoration capability. Keep downloaded originals and credits outside browser storage. Website links and original filenames help identify recordings; a filename is not a rights grant.
