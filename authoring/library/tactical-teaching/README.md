...
Only the three JSON files under [scenarios](scenarios/) enter the next build/offline allowlist. They remain outside all campaign pack catalogs; their builder and proof traces stay source-only. Native Playground load buttons use the existing validated import flow, followed by a separate Play configuration action. Integrated browser review remains pending. Each has its own level ID/revision and saved-context identity; historical Fieldcraft, Arcade and replay fixtures are untouched.
