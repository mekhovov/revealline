"""Serve only the wave 5/wave 6 comparison and eight selected original PNGs."""
import argparse
import json
import stat
...
def main():
    ...
    parser.add_argument('--previous', type=Path, default=cohort.parent / 'reserve-illustrations-wave-5')
    args = parser.parse_args()
    current = json.loads((cohort / 'manifest.json').read_text())
    previous = json.loads((args.previous / 'manifest.json').read_text())
    if current['wave'] != 6 or previous['wave'] != 5:
        raise ValueError('Expected exact wave 5 / wave 6 manifests')
