...
export const PROOF_FILE = 'authoring/library/sentinel-theme-chapters/routes.json';
export const DESCRIPTOR_FILES = THEME_IDS.map(
  (theme) => `authoring/library/sentinel-theme-chapters/descriptors/${theme}.json`,
);
const sha = (value) =>
  createHash('sha256')
...
async function execute(world, context, level, source, image) {
  ...
  const run = createRun(level, setup),
    recorder = createRecorder(level, setup, `sentinel-theme-${world.descriptor.themeId}-proof`);
    ...
    for (let n = 0; n < segment.ticks; n++) {
      ...
      if (run.tick === source.savedPrefix.tick) {
        ...
        saved = suspendSession({
          ...
          runId: `sentinel-${world.descriptor.themeId}-${source.id.replaceAll('/', '-')}`,
...
/** The fixed base trace is referenced, never duplicated into this compact receipt. */
export async function verifySentinelThemes({ candidate, record = false } = {}) {
  ...
  if (owned) {
    exactKeys(owned, ['descriptors', 'proof'], 'Sentinel theme request');
    assert.ok(Array.isArray(owned.descriptors) && plainObject(owned.proof));
    assert.equal(owned.descriptors.length, 3);
  }
  if (record)
    for (const file of [PROOF_FILE, ...DESCRIPTOR_FILES])
  ...
  const descriptors = worlds.map((world) => world.descriptor);
  if (owned) assert.deepEqual(owned.descriptors, descriptors);
  const proof = owned
    ? owned.proof
    : record
      ? null
      : JSON.parse(await boundedFile(PROOF_FILE, 2 * 1024 * 1024));
  const header = {
    format: 'revealline-sentinel-theme-proof.v1',
    sourceProof: { path: SOURCE_PROOF, sha256: SOURCE_PROOF_SHA },
    sourcePackSha256: SOURCE_PACK_SHA,
    themeOrder: THEME_IDS,
    editions: worlds.map((world) => ({
      descriptor: world.descriptor,
      inputPins: world.inputPins,
      images: world.imageProofs,
    })),
    geometry: 'Three existing Sentinel layouts reused across three themes; zero new geometries.',
    tracePolicy:
      'Inputs come only from the exact retained base sourceProof route identified by sourceRouteId. Each complete replay, event/metric comparison, physical checkpoint, unfinished save/restore and suffix is executed; this receipt retains their hashes and exact new picture pins instead of copying trace arrays.',
  };
  if (proof) {
    exactKeys(proof, [...Object.keys(header), 'routes', 'summary'], 'Sentinel theme proof');
  ...
  for (const world of worlds) {
    for (const route of source.routes) {
      ...
      const actual = await execute(world, context, level, route, world.descriptor.originals[index]);
      const compact = {
        id: `${world.descriptor.themeId}/${route.id}`,
        ...
        setupSha256: sha(route.setup),
        classId: route.setup.classId,
        turnPolicy: route.setup.turnPolicy,
        expected: actual.expected,
        checkpointSha256: sha(actual.checkpoint),
        metricsSha256: sha(actual.metrics),
        replaySha256: actual.replaySha256,
        savedPrefix: {
          tick: actual.savedPrefix.tick,
          format: actual.savedPrefix.format,
          bytes: actual.savedPrefix.bytes,
          sha256: actual.savedPrefix.sha256,
          checkpointSha256: sha(actual.savedPrefix.checkpoint),
          continuation: actual.savedPrefix.continuation,
          picturePin: actual.savedPrefix.presentationPins.choices[0].picture,
          storyPin: actual.savedPrefix.presentationPins.choices[0].story,
          replaySuffixByteIdentical: actual.savedPrefix.replaySuffixByteIdentical,
        },
      };
      if (proof) assert.deepEqual(proof.routes[routes.length], compact);
      routes.push(compact);
    }
  }
  const summary = {
    ...
    reusedGeometries: 3,
    newGeometries: 0,
    nullStoryPins: routes.every((r) => r.savedPrefix.storyPin === null),
  };
  assert.deepEqual(
    [
      summary.contexts,
  ...
  if (record) {
    for (const [file, value] of [
      [PROOF_FILE, result],
      ...DESCRIPTOR_FILES.map((f, i) => [f, descriptors[i]]),
    ])
      await writeFile(path.join(ROOT, file), JSON.stringify(value, null, 2) + '\n', { flag: 'wx' });
  } else {
    assert.deepEqual(proof, result);
    for (const [i, file] of DESCRIPTOR_FILES.entries())
      assert.deepEqual(JSON.parse(await boundedFile(file, 16384)), descriptors[i]);
  }
  return result;
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const result = await verifySentinelThemes({ record: args.length === 1 });
  console.log(
    JSON.stringify(
      {
        passed: true,
        ...result.summary,
        limits:
          'Actual source core/replay/saved-pin proofs only; no native install, Collection, browser, offline, public or hardware qualification.',
      },
      null,
      2,
    ),
