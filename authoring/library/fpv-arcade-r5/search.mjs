/** R5 bounded input-only search. Copies are simulation branches, never patched states. Each captured cut consumes one release tick before a fresh direction. */
...
function candidates(state, bent = false) {
  const { width, height } = geometryForRun(state);
  const idx = (x, y) => y * width + x;
  ...
  for (const origin of reachable)
    for (const d of directions) {
      ...
      while (x >= 0 && x < width && y >= 0 && y < height && state.cells[idx(x, y)] === CELL.FIELD) {
        length++;
        if (bent)
          for (const side of directions.filter((side) => side.dx * d.dx + side.dy * d.dy === 0)) {
            let bx = x + side.dx,
              by = y + side.dy,
              extra = 0;
            while (
              ...
              by >= 0 &&
              by < height &&
              state.cells[idx(bx, by)] === CELL.FIELD
            ) {
              extra++;
              if (bent === 2) {
                let ex = bx - d.dx,
                ...
                while (
                  ...
                  ey >= 0 &&
                  ey < height &&
                  state.cells[idx(ex, ey)] === CELL.FIELD
                ) {
                  back++;
                  ex -= d.dx;
                  ey -= d.dy;
                ...
                if (
                  ...
                  ey >= 0 &&
                  ey < height &&
                  state.cells[idx(ex, ey)] === CELL.SAFE
                )
                  result.push({
                    path,
                    origin,
                    via: [idx(x, y), idx(bx, by)],
                    end: idx(ex, ey),
                    direction: d.name,
                    length: length + extra + back,
                  });
            ...
            if (
              ...
              by >= 0 &&
              by < height &&
              state.cells[idx(bx, by)] === CELL.SAFE
            )
              result.push({
                path,
                origin,
      ...
      if (x < 0 || x >= width || y < 0 || y >= height || state.cells[idx(x, y)] !== CELL.SAFE)
        continue;
      if (!bent) result.push({ path, origin, end: idx(x, y), direction: d.name, length });
    }
  return result;
}
...
function travel(state, target, segments, { counter } = {}) {
  ...
  for (let i = 0; i < maximumTicks && state.status === 'running'; i++) {
    ...
    if (
      ...
      const released = { direction: null };
      try {
        stepRun(state, released, FIXED_DT);
      } catch (error) {
        error.trial = {
          segments: structuredClone(segments),
          failedInput: released,
          state: {
            tick: state.tick,
            time: state.time,
            status: state.status,
...
function tryCandidate(original, candidate, counter) {
  ...
  for (const via of candidate.via ?? [])
    if (!travel(state, center(via, state.width), segments, { counter })) return null;
...
export function findRoute(
  ...
  classes,
  turnPolicy,
  { maxCuts = 14, maxMilliseconds = 12000, classId = 'scout' } = {},
) {
  ...
  for (let cut = 0; cut < maxCuts && state.status !== 'won'; cut++) {
    let best = null,
      bestScore = -Infinity;
    for (const bent of [0, 1, 2]) {
      for (const candidate of candidates(state, bent)) {
        ...
        try {
          ...
          error.trace = { completedSegments: structuredClone(segments), candidate, ...error.trial };
          throw error;
        }
        examined++;
        ...
        if (trial.state.status === 'won') break;
      }
      if (best) break;
    }
    if (!best)
      return {
        won: false,
        reason: 'No improving safe straight or bent cut found',
        segments,
        examined,
        simulatedTicks,
        milliseconds: performance.now() - started,
...
/** Replay a proposed directional route under this exact context, releasing at
 * its own captures. This is another actual simulation, never an assumed win. */
export function tryRoute(level, classes, turnPolicy, source) {
  const state = createRun(level, { seed: 1, turnPolicy, classId: 'scout', classRecipes: classes }),
    segments = [];
  let simulatedTicks = 0;
  for (const segment of source) {
    if (segment.input.direction === null) continue;
    for (
      let i = 0;
      i < segment.ticks && state.status === 'running' && simulatedTicks < 21600;
      i++
    ) {
      const input = { direction: segment.input.direction };
      stepRun(state, input, FIXED_DT);
      append(segments, input, 1);
      simulatedTicks++;
      if (state.status === 'running' && state.events.some((e) => e.type === 'capture.stopped')) {
        const release = { direction: null };
        stepRun(state, release, FIXED_DT);
        append(segments, release, 1);
        simulatedTicks++;
      }
    }
  ...
  return {
    won: state.status === 'won' && state.classic.livesLost === 0,
    reason:
      state.status === 'won' ? 'completed' : 'Candidate directions did not complete this context',
    segments,
    simulatedTicks,
    summary: getSummary(state),
  };
