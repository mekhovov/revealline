"""Serve only the wave 4/wave 5 comparison and eight selected original PNGs."""
import argparse
import json
import stat
...
def main():
    ...
    parser.add_argument('--previous', type=Path, default=cohort.parent / 'reserve-illustrations-wave-4')
    args = parser.parse_args()
    current = json.loads((cohort / 'manifest.json').read_text())
    previous = json.loads((args.previous / 'manifest.json').read_text())
    if current['wave'] != 5 or previous['wave'] != 4:
        raise ValueError('Expected exact wave 4 / wave 5 manifests')
