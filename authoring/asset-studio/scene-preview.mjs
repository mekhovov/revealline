...
export function componentPreview(surface, slot, options, backgroundImage = null) {
  const box = text('div', '', 'recipe-sample component-specimen');
  box.dataset.state = options.state;
  const state = options.state;
  if (backgroundImage && slot.group === 'screens') {
    box.classList.add('background-specimen');
    box.style.backgroundImage = `linear-gradient(#070b1255, #070b12cc), url("${backgroundImage}")`;
  }
  const heading = slot.id.includes('pause')
    ? 'Flight paused'
    : slot.group === 'screens'
      ? 'Field operations'
      : slot.label;
  box.append(text('h3', heading));
  if (/input|search/.test(slot.id)) {
    const input = document.createElement('input');
    input.placeholder = 'Pilot callsign · Позивний';
    input.disabled = state === 'disabled';
    input.setAttribute('aria-label', 'Component specimen input');
    box.append(input);
  } else if (/meter|progress|coverage/.test(slot.id)) {
    const meter = document.createElement('progress');
    meter.max = 100;
    meter.value = 62;
    box.append(meter, text('p', 'Captured 62% / Target 75%'));
  } else if (/toggle/.test(slot.id)) {
    const label = text('label', 'Reduced effects');
    const check = document.createElement('input');
    check.type = 'checkbox';
    check.checked = ['selected', 'pressed'].includes(state);
    check.disabled = state === 'disabled';
    label.append(check);
    box.append(label);
  } else {
    const button = text('button', state === 'loading' ? 'Loading…' : 'Deploy · Почати');
    button.type = 'button';
    button.disabled = state === 'disabled';
    button.setAttribute('aria-pressed', String(state === 'pressed' || state === 'selected'));
    box.append(button);
  }
  box.append(
    text(
      'p',
      state === 'error' ? 'Connection unavailable. Retry when ready.' : 'Mission 01 · First light',
    ),
    text(
      'small',
      `${state} · scoped ${slot.group === 'screens' ? 'title/dialog placement' : 'component state'} specimen`,
      'bounded-label',
    ),
  );
  if (backgroundImage && slot.group !== 'screens') {
    const target = slot.id.includes('panel')
      ? box
      : box.querySelector('button,input,progress') || box;
    const slice = options.assetGeometry?.nineSlice;
    if (slice) {
      target.style.borderImageSource = `url("${backgroundImage}")`;
      target.style.borderImageSlice = `${slice.top} ${slice.right} ${slice.bottom} ${slice.left} fill`;
      target.style.borderImageWidth = '8px';
      target.style.borderImageRepeat = 'stretch';
