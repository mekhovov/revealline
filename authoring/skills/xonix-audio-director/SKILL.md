...
Changing-name master actions (`Mute sound` / `Unmute sound`) are ordinary commands without `aria-pressed`; follow the [shared master contract](../../../docs/shared-master-audio.md) and [W3C button pattern](https://www.w3.org/WAI/ARIA/apg/patterns/button/). Render playback text from current transport notifications and current master state, including immediate mute changes; a late Play result must not overwrite a newer Pause, player or request. Rendering must not start media or persist preferences. Preserve explicit failure text until a transport update or retry changes it. The soundtrack host, master-keyboard host, panel and Couch audio-master tests cover these ownership and command contracts; [bounded native observations](../../../docs/verification/cross-mode/p02-a/published-v0574-composition/README.md) remain separate from audible output, PCM measurements and device qualification.
...
For shared mixer changes, use the [native signal check](../../../docs/verification/cross-mode/p02-a/output-probe/README.md) against the intended source. Retain its source pins and actual report, including cancellation. The analyser measures digital mixer output with quiet fixture inputs; keep this distinct from native MP3/video playback, subjective listening, physical speakers and platform certification. Do not replace a failed measurement with a modeled result.
