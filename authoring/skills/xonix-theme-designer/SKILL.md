...
## Team crawling and rescue presentation

When reviewing Team pilot bodies, follow [the current state contract](../../../docs/team-actor-presentation.md#crawling-and-active-rescue-states--source-candidate). Downed rotors stay stopped while actual safe-ground crawling can turn the body. Preserve the final rotated frame/pivot/rotor envelope, true contact head and player number plus circle/diamond. Active rescue uses a static partner-labelled marker; percentage progress stays in the existing HUD/Support control. Distinguish legal movement from blocked intent or recovery teleport, and qualify both seats with real command-earned states, paused/reduced effects and actual-size browser views. This correction uses existing body slots: explicit Team pilot/objective recipe registration, complete edition coverage and public acceptance remain separate requirements. Use the reproduction prompt in that guide when preparing or reviewing a variation.
