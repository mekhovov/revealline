...
Keep definitions in pinned sidecars outside normalized levels, campaign data and `xonix-pack.v1`. Use `masteryFor(campaignKey(installedCampaign), levelId)`; all three Homeward goals require `homeward-skies/1/0d01f5687b3c38ff`. The reviewed map identities are `homeward-01` → `level-v1-5983ec4eaf745012`, `homeward-02` → `level-v1-f1c1d86b070b5419`, and `homeward-03` → `level-v1-03b703a6160004fc`, with map revision `1` and roster `roster-v1-e159e435`. Edited same-ID maps/rosters do not inherit those registrations. The [pack mastery design](../../../docs/round-18-pack-mastery-plan.md) remains a later extension; no generic definition-upload, pack-v2 acceptance or mastery-enabled playground schema is implied here.
