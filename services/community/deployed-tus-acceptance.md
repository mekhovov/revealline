...
After the worker publishes the package, the command discovers and downloads that immutable edition,
...
run identities, byte identities, offsets, request counts, validation status, and cleanup status.
Credentials, account subjects, response bodies, package content, and configuration values are not
written to the receipt or terminal.
