...
**Play:** [current public game](https://mekhovov.github.io/revealline/game/) · [release explorer](https://mekhovov.github.io/revealline/releases/). The [cross-mode execution register](docs/cross-mode-execution.md) is the single authority for current status, qualified source, delivery evidence and remaining work. [Delivery priorities](docs/delivery-priorities.md) retains earlier requirements and dated checkpoints. The [detailed delivery history](docs/current-delivery-plan.md) and [production checkpoints](docs/production-plan.md) retain earlier observations; their dated pending/latest wording does not identify today's public edition.
