...
Play [the public game](https://mekhovov.github.io/revealline/), or use the [saved v0.18.0 build](http://127.0.0.1:8767/releases/v0.18.0/site/game/) while the workspace server runs. v0.18 adds Pack and Level selectors, campaign launch links and the gallery-clock repair. Its [archive audit](docs/verification/round-28/v018-preservation.md) reproduces all 144 output files and preserves every earlier release. [Earlier release evidence](docs/release-history.md) retains the original successes and failures.

The working source for v0.19 adds **Export saved attempt** at Ready and **Export current attempt** during an unfinished flight, with cancellable verification and a copyable file. Missing campaign content permits an explicitly marked rescue export; resuming still requires its exact campaign. See [saves and packs](docs/library-and-packs.md), [120 affected source checks](docs/verification/round-28/attempt-export.md), and the [source browser checks](docs/verification/round-28/attempt-export-browser.md). This next increment is not yet a frozen or published v0.19 release.
