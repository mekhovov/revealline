...
Play [the public game](https://mekhovov.github.io/revealline/), or use the [saved v0.19.0 build](http://127.0.0.1:8767/releases/v0.19.0/site/game/) while the workspace server runs. v0.19 adds **Export saved attempt** at Ready and **Export current attempt** during an unfinished flight, with cancellable verification and a copyable file. Missing campaign content permits an explicitly marked rescue export; resuming still requires its exact campaign. See [saves and packs](docs/library-and-packs.md).

All [six source checks](docs/verification/round-28/v019-source-gates.md) passed, including **1,667 tests**. The [offline browser journey](docs/verification/round-28/v019-browser.md) exported and resumed saved flights, completed a mission and retained its picture, score and seal after reload. The [independent rebuild](docs/verification/round-28/v019-integrity.md) matches all 146 packaged files, including the exact browser-tested candidate. [Public deployment](docs/verification/round-28/v019-public-deployment.md) verifies the published version separately. Pack/Level selectors, campaign launch links and the gallery-clock repair from v0.18 remain included; [earlier release evidence](docs/release-history.md) retains the original results and limitations.
