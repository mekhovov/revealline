...
Play [the public game](https://mekhovov.github.io/revealline/), or use the [saved v0.20.0 build](http://127.0.0.1:8767/releases/v0.20.0/site/game/) while the workspace server runs. v0.20 adds [shared couch controller menus](docs/couch-controller-navigation.md): either assigned player can deliberately join, choose the setup and control the round, with keyboard and touch still available. **Export saved attempt** at Ready and **Export current attempt** during an unfinished flight retain cancellable verification and a copyable file. Missing campaign content permits an explicitly marked rescue export; resuming still requires its exact campaign. See [saves and packs](docs/library-and-packs.md).

All [six source checks](docs/verification/round-29/v020-source-gates.md) passed, including **1,683 tests** on 354 unchanged inputs. The [packaged browser journey](docs/verification/round-29/v020-browser.md) verified offline readiness and ordinary couch wins; its separate responsive fixtures retain their own limits and console observations. The [independent rebuild](docs/verification/round-29/v020-integrity.md) matches all 146 packaged files, including the exact browser-tested candidate. [Public deployment](docs/verification/round-29/v020-public.md) separately checked pack installation, native couch controls, round transitions and Solo return. These are software/browser results, not physical-controller certification. Pack/Level selectors, campaign launch links and the gallery-clock repair from v0.18 remain included; [earlier release evidence](docs/release-history.md) retains the original results and limitations.
