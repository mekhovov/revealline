...
export function releaseAssetNames(formatVersion = 1) {
  if (![1, 2].includes(formatVersion)) throw new Error('unsupported source contract');
...
export function decideReleaseAssets({ expected, actual, published = false }) {
  ...
  const names = releaseAssetNames(expectedByName.has('source-manifest.json') ? 2 : 1);
  if (actualByName.size !== actual.length) throw new Error('duplicate release asset names');
  if (expected.length !== names.length || expectedByName.size !== names.length)
    throw new Error('expected release asset set must contain exactly nine names');
  for (const name of names)
    if (!expectedByName.has(name)) throw new Error(`expected release asset is missing ${name}`);
  for (const name of actualByName.keys())
    if (!expectedByName.has(name)) throw new Error(`unexpected release asset ${name}`);
  ...
  if (published && missing.length)
    throw new Error('published release assets are incomplete and must never be overwritten');
...
export async function inspectLocalAssets(directory) {
  ...
  if (manifestSource) {
    const record = JSON.parse(await fs.readFile(path.join(directory, 'release.json'), 'utf8'));
    if (
  ...
  return async function githubRequest(pathname, options = {}) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      try {
        ...
        const error = new Error(`GitHub transport outcome unknown: ${method} ${pathname}`);
      ...
      try {
        ...
        const error = new Error(`GitHub response body unavailable: ${method} ${pathname}`);
        error.ambiguousWrite = method !== 'GET';
        throw error;
      }
      if (!response.ok) {
        ...
        const codes = Array.isArray(body.errors)
          ? body.errors
              ...
              .filter((code) => typeof code === 'string' && /^[a-z_]{1,64}$/u.test(code))
          : [];
        ...
        if (rateLimited) {
          ...
          } else if (reset !== null && response.headers.get('x-ratelimit-remaining') === '0') {
        ...
        let message = typeof body.message === 'string' ? body.message : 'non-JSON error response';
      ...
      try {
        ...
        const error = new Error(`GitHub response JSON invalid: ${method} ${pathname}`);
        error.ambiguousWrite = method !== 'GET';
        throw error;
      }
    }
...
export async function verifyAdditiveEditionAssets({
  ...
  // Validate one exact core source contract before classifying any edition additions.
  decideReleaseAssets({ expected, actual: [] });
  const coreNames = new Set(expected.map((asset) => asset.name));
  const names = new Set(actual.map((asset) => asset.name));
  if (names.size !== actual.length) throw new Error('duplicate release asset name');
  const additions = actual.filter((asset) => !coreNames.has(asset.name));
...
export async function ensureDraftRelease({
  ...
  if (state.decision.state === 'published' || state.release?.draft) return state.release;
  ...
  if (state.decision.action === 'create') {
    ...
    try {
      ...
      if (!SHA.test(tag?.sha || '')) throw new Error('invalid annotated tag creation response');
    } catch (error) {
    ...
    if (!writeError) {
      try {
        ...
        if (!recoverableWrite(error) && ![409, 422].includes(error?.status)) throw error;
        writeError = error;
      }
    }
  ...
  if (state.decision.action === 'create-release') {
    ...
    try {
      ...
      if (!created.draft) throw new Error('draft creation returned a published release');
    ...
    state = await discover(
      (found) =>
        found.release !== null && (releaseId === undefined || found.release.id === releaseId),
...
export async function reconcileReleaseAssets({
  ...
  return admitted.length ? { ...result, admittedEditionAssets: admitted } : result;
}
...
export async function publishExactRelease({
  ...
  const published = await request(`/repos/${repository}/releases/${state.release.id}`, {
    method: 'PATCH',
...
async function main() {
  ...
  const directory = path.resolve(process.env.ASSET_DIRECTORY || 'release-assets');
