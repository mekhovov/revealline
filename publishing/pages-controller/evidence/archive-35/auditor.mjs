...
assert.equal(run.id,35495526770); assert.equal(run.head_sha,'b603b1d1aa75c5352155d97876033ca37a883888');
...
assert.equal(lock.expectedFiles,729);assert.equal(lock.expectedBytes,313820502);
...
// New archive: no previously accepted inventory.
const changed=[];
...
try {
 ...
 const report={status:failed.length?'FAIL':'PASS',base:BASE,source:run.head_sha,run:run.id,files:rows.length,failedFiles:failed.length,expectedBytes:lock.expectedBytes,verifiedBytes:rows.filter(r=>r.ok).reduce((n,r)=>n+r.bytes,0),expectedInventorySha256:lock.expectedInventorySha256,priorPaths:0,unchangedPriorPaths:0,mutablePriorExceptions:changed,startedAt,finishedAt:new Date().toISOString(),elapsedSeconds:(performance.now()-start)/1000,skipped:[],nativeAcceptance:false};
