"""Read-only, fail-closed intake of the actual Archive54 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
BASE = 'https://mekhovov.github.io/revealline-archive-54/'
REPO = 'mekhovov/revealline-archive-54'
TREE = '62d60cfb85f6c0e2824109da6b2cdc8734e8e7a2'
INVENTORY_SHA = 'c819c41dd41aa1b0b56d7596c4b71b21e604ac06a68934066b21f249b2c8a5d1'
LOCK_SHA = 'b16506333aa464579cb4cf5d2334ec537230a1a832e5c445da9edd664a031da0'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1085 and sum(r['bytes'] for r in rows) == 590851098, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive54-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive54-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.85.0.json']
