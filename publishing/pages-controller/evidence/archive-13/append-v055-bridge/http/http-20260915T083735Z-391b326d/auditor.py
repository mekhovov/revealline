"""Read every hash-pinned Archive13 HTTP body without persisting payloads.
...
INFRA = Path('/Users/oleksandr.mekhovov/work/my_projects/go_test/.cache/worktrees/archive13-v055-extension')
BASE = 'https://mekhovov.github.io/revealline-archive-13/'
COMMIT = '18f70dba10b6c7e24d3074e05931ecb32b4dcdae'
DEPLOYED_COMMIT = '58b21ebeff3c2d8e8ca37d5b7764f8203b8824ed'
DEPLOYMENT_ID = 6454910731
RUN_ID = 34947667008
INVENTORY_SHA = 'd825b3ff2c7ae8a6f4a1212d86b28a9f8d6dcee117e1726b976fa25655c74ffa'
EXPECTED_FILES = 1303
EXPECTED_BYTES = 624422286
SOCKET_TIMEOUT = 25
ATTEMPT_SECONDS = 90
AUDIT_SECONDS = 1800
MAX_ATTEMPTS = 3
...
def pinned_inventory():
    ...
    if digest(raw) != INVENTORY_SHA:
        raise ValueError('Archive13 inventory pin changed')
    head = subprocess.check_output(['git', '-C', str(INFRA), 'rev-parse', 'HEAD'], text=True).strip()
    committed = subprocess.check_output(['git', '-C', str(INFRA), 'show', COMMIT + ':expected-inventory.json'])
    if head != COMMIT or committed != raw:
        raise ValueError('Archive13 inventory is not from its pinned infrastructure commit')
    value = json.loads(raw)
    if set(value) != {'base', 'files'} or value['base'] != BASE:
        raise ValueError('Archive13 base or inventory format changed')
    ...
    if (len(rows) != EXPECTED_FILES or len({row['path'] for row in rows}) != EXPECTED_FILES or
        ...
        raise ValueError('Archive13 finite inventory changed')
    return raw, rows
...
def fetch_once(row, attempt, opener, *, deadline, clock=time.monotonic):
    ...
    try:
        ...
        request = urllib.request.Request(url, headers={
            ...
            'User-Agent': 'RevealLine-archive13-byte-audit/2.0',
        })
...
def main():
    ...
    parser.add_argument('--run', action='store_true', help='Audit the fixed public Archive13 inventory')
    ...
    if not args.run:
        ...
        return 0
    out = EVIDENCE / ('http-' + datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ') + '-' + uuid.uuid4().hex[:8])
    ...
    report = {
        'status': 'FAIL' if failed else 'PASS', 'base': BASE, 'archiveCommit': DEPLOYED_COMMIT,
        'infrastructureSourceCommit': COMMIT, 'deploymentId': DEPLOYMENT_ID, 'runId': RUN_ID,
