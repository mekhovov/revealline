"""Read-only, fail-closed intake of the actual Archive50 initial-deployment receipts."""
import hashlib
import io
import json
...
AUTHORITY_ROOT = ROOT
INFRA = ROOT.parent / 'worktrees/revealline-archive-50'
REQUEST = ROOT / 'execution-request.reviewed.json'
BASE = 'https://mekhovov.github.io/revealline-archive-50/'
REPO = 'mekhovov/revealline-archive-50'
TREE = 'ea35972c2f58c202f36e77030d0946451ea360de'
INVENTORY_SHA = '0afa89ef51a67bb5fbdb3b7656da488e76eff0f8cd1005c07bf7a982d83b6a9f'
LOCK_SHA = '809d27a2c4fcfcee9c621bccbb2292d6dc726c9fcb24a6838efd763d46f6a331'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1056 and sum(r['bytes'] for r in rows) == 590361506, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive50-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive50-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.82.0.json']
