...
def row(path='releases/v0.97.0/site/game/example.json', body=b'{"ok":true}'):
...
class AuditTests(unittest.TestCase):
    ...
    def test_prepared_initial_inventory_is_exact_without_network(self):
        ...
        self.assertEqual(sum(item['bytes'] for item in rows), 594897841)
...
class MetadataCapacityTests(unittest.TestCase):
    def test_worst_case_three_attempts_all_rows_fits_below_free_guard(self):
        self.assertEqual(audit.MAX_WORST_CASE_METADATA_BYTES, 26755072)
