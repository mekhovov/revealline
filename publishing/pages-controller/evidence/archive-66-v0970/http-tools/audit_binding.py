"""Read-only, fail-closed intake of the actual Archive66 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
REPO = 'mekhovov/revealline-archive-66'
TREE = '346f208297297bf48838e86ceb01e6e5d7ac99ab'
INVENTORY_SHA = '1d1eb8d4dff55f46c8a3f322f1479ea96f2f89ee93f5e42b5cc759e8f62d5bc5'
LOCK_SHA = 'e08e7333fc1aafe70610fef20c7eb94bb459d371a5f5e19f98d3e2bd42370cac'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1121 and sum(r['bytes'] for r in rows) == 594897841, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive66-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive66-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.97.0.json']
