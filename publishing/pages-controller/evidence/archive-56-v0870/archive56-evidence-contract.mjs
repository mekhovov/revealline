...
export const REPO = 'mekhovov/revealline-archive-56';
export const BASE = 'https://mekhovov.github.io/revealline-archive-56/';
export const COMMIT = '592d0afe42f66dfb9fe95962c80adfaf236cd8ef';
export const TREE = '4443f3a017da924836de658ebd5dbbe9cf96dc21';
export const SOURCE = '08548aeabe72f396fb75aede938d5bcadcfdc00e';
export const RUN = 35808261588;
export const FILES = 1088, BYTES = 590907537;
...
export function deployment(file) {
  ...
  assert.equal(binding.format, 'revealline-archive56-http-request.v1');
  ...
  assert.equal(artifact.name, 'archive56-verification-receipts');
...
export function httpEvidence(dir, binding) {
  ...
  assert.equal(sha(requestRaw), 'eca3ca5edc19bb904c33bc7f25d7ba9b34b3b09a8363208619f7453576415f71');
  const auditHead = 'e3587c554962ad1b0e33f1770951a935720b1329';
  const sourcePins = [
    ['auditor.py', 'audit/http-tools/http_audit.py', '1fda486f7abb2727a314e5db56527b880e070628e04cd7993342b042c03de1dc'],
    ['audit_binding.py', 'audit/http-tools/audit_binding.py', '8cef718128c8d27cc98448234453d90db5107e88c91c88874b0b8f904ac1a485'],
    ['execution-request.json', 'audit/execution-request.reviewed.json', sha(requestRaw)],
    [null, '.github/workflows/deploy.yml', '206f6d6ee054687f608df1a6c9c3226d23409d149544470854cd8ced5dfc5b9f'],
  ];
  ...
  assert.equal(hosted.id, 35808480891); assert.equal(hosted.head_sha, auditHead);
  assert.equal(hosted.head_branch, 'codex/audit-archive56-http');
  ...
  assert.equal(artifact.id, 10728763036); assert.equal(artifact.name, 'archive56-public-http-evidence');
  ...
  assert.equal(sha(zip), '0fef57f53247d9c7714605f3a30281233cdf4d91572397f31e23ad0c6bb5d39c');
...
export function nativeEvidence(file, expectedSha, binding) {
  ...
  assert.equal(receipt.version, 'v0.87.0'); assert.equal(receipt.sourceRevision, SOURCE);
  ...
  assert.equal(receipt.gameURL, BASE + 'releases/v0.87.0/site/game/');
  assert.equal(receipt.canonicalReleaseMarker, BASE + 'releases/v0.87.0/release.json');
