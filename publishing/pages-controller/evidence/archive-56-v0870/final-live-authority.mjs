...
import { REPO, BASE, COMMIT, TREE, RUN, at, read, json, sha, deployment, httpEvidence, writeJSON } from './archive56-evidence-contract.mjs';
...
const marker = execFileSync('curl', ['--fail', '--silent', '--show-error', '--max-time', '30', BASE + 'releases/v0.87.0/release.json'], { maxBuffer: 1024 * 1024, timeout: 35_000 });
fs.writeFileSync(at('after-public-release-original.json'), marker, { flag: 'wx' });
const pin = json(at('repository/expected-inventory.json')).files.find((row) => row.path === 'releases/v0.87.0/release.json');
