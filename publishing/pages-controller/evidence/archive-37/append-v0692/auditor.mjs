...
assert.equal(run.id,35520468794); assert.equal(run.head_sha,'3aa531978c58b2c7c22e49b88621f0b6b849e089');
...
assert.equal(lock.expectedFiles,1557);assert.equal(lock.expectedBytes,680029417);
...
// Compare every accepted prior path, with only the mutable landing index excepted.
const prior=read('../previous-expected-inventory.json');
assert.equal(prior.files.length,780);
const current=new Map(inventory.files.map(f=>[f.path,f]));
const changed=[];
for(const row of prior.files){const now=current.get(row.path);assert(now);if(now.bytes!==row.bytes||now.sha256!==row.sha256)changed.push(row.path);}
assert.deepEqual(changed,['index.html']);
assert.equal(prior.files.filter(f=>f.path.startsWith('releases/v0.69.1/')).length,777);
...
try {
 ...
 const report={status:failed.length?'FAIL':'PASS',base:BASE,source:run.head_sha,run:run.id,files:rows.length,failedFiles:failed.length,expectedBytes:lock.expectedBytes,verifiedBytes:rows.filter(r=>r.ok).reduce((n,r)=>n+r.bytes,0),expectedInventorySha256:lock.expectedInventorySha256,priorPaths:780,unchangedPriorPaths:779,mutablePriorExceptions:changed,startedAt,finishedAt:new Date().toISOString(),elapsedSeconds:(performance.now()-start)/1000,skipped:[],nativeAcceptance:false};
