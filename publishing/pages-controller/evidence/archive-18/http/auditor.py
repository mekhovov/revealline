"""Read every hash-pinned Archive18 HTTP body without persisting payloads.
...
INFRA = BASE_DIR.parent.parent / 'worktrees/archive18-v0580-infrastructure'
BASE = 'https://mekhovov.github.io/revealline-archive-18/'
COMMIT = 'b83acdd80f0aaa35a72cff8c6b861250e894b1d7'
SOURCE_COMMIT = '7e13712909dd0deb686dff18499710cb9b2c3458'
SOURCE_TREE = '69b9a690589500ea78b240ae242efb7309474730'
AUTHORITY_SHA = '9553ca2f14e06c0f1404d91e4fefbce9eeddb73f11edc73f29c58ed5d21a5777'
INVENTORY_SHA = '7dfba91f1093de6983ab6a783242307eab196f82d84a55b76d84737c45fd2d46'
EXPECTED_FILES = 665
EXPECTED_BYTES = 312702795
SOCKET_TIMEOUT = 25
ATTEMPT_SECONDS = 90
AUDIT_SECONDS = 1800
MAX_ATTEMPTS = 3
...
def pinned_inventory():
    ...
    if digest(raw) != INVENTORY_SHA:
        raise ValueError('Archive18 inventory pin changed')
    ...
    if head != SOURCE_COMMIT or committed != raw:
        raise ValueError('Archive18 inventory is not from its pinned infrastructure commit')
    ...
    if tree != SOURCE_TREE or digest(authority_raw) != AUTHORITY_SHA:
        raise ValueError('Archive18 source tree or hosted authority pin changed')
    ...
    if (authority['status'] != 'HOSTED_READY_FOR_HTTP' or
        ...
        raise ValueError('Archive18 merged deployment differs from the held source tree')
    for pin in authority['evidence']:
        ...
        if len(evidence) != pin['bytes'] or digest(evidence) != pin['sha256']:
            raise ValueError('Archive18 retained hosted evidence changed')
    value = json.loads(raw)
    if set(value) != {'base', 'files'} or value['base'] != BASE:
        raise ValueError('Archive18 base or inventory format changed')
    ...
    if (len(rows) != EXPECTED_FILES or len({row['path'] for row in rows}) != EXPECTED_FILES or
        ...
        raise ValueError('Archive18 finite inventory changed')
    return raw, rows
...
def fetch_once(row, attempt, opener, *, deadline, clock=time.monotonic):
    ...
    try:
        ...
        request = urllib.request.Request(url, headers={
            ...
            'User-Agent': 'RevealLine-archive18-byte-audit/2.0',
        })
...
def main():
    ...
    parser.add_argument('--run', action='store_true', help='Audit the fixed public Archive18 inventory')
