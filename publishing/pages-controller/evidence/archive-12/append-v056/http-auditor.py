"""Read every hash-pinned Archive12 HTTP body without persisting payloads.
...
P01 = Path(__file__).resolve().parents[2]
INFRA = P01 / 'archive12-v056'
BASE = 'https://mekhovov.github.io/revealline-archive-12/'
COMMIT = '48fadddd098af0e3e71e7913445144731d6da259'
INVENTORY_SHA = 'f0482926a23fbb56b3f6f808e64c3d44cdec104c09f0fc698510e815d2f5c338'
EXPECTED_FILES = 1288
EXPECTED_BYTES = 635435625
SOCKET_TIMEOUT = 25
ATTEMPT_SECONDS = 90
AUDIT_SECONDS = 1800
MAX_ATTEMPTS = 3
...
def pinned_inventory():
    ...
    if digest(raw) != INVENTORY_SHA:
        raise ValueError('Archive12 inventory pin changed')
    ...
    if head != COMMIT or committed != raw:
        raise ValueError('Archive12 inventory is not from its pinned infrastructure commit')
    value = json.loads(raw)
    if set(value) != {'base', 'files'} or value['base'] != BASE:
        raise ValueError('Archive12 base or inventory format changed')
    ...
    if (len(rows) != EXPECTED_FILES or len({row['path'] for row in rows}) != EXPECTED_FILES or
        ...
        raise ValueError('Archive12 finite inventory changed')
    return raw, rows
...
def fetch_once(row, attempt, opener, *, deadline, clock=time.monotonic):
    ...
    try:
        ...
        request = urllib.request.Request(url, headers={
            ...
            'User-Agent': 'RevealLine-archive12-byte-audit/2.0',
        })
...
def main():
    ...
    parser.add_argument('--run', action='store_true', help='Audit the fixed public Archive12 inventory')
    ...
    if not args.run:
        ...
        return 0
    out = P01 / 'archive12-v056-evidence' / ('http-' + datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ') + '-' + uuid.uuid4().hex[:8])
