"""Read-only, fail-closed intake of the actual Archive59 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
BASE = 'https://mekhovov.github.io/revealline-archive-59/'
REPO = 'mekhovov/revealline-archive-59'
TREE = '60e7ab0b6240de281df3bc668dd71ac5cb070fb2'
INVENTORY_SHA = '787197923251ee6049d5103bbd48e41f094f5cc34ff57dad3cff0c7849a7edf1'
LOCK_SHA = '5b2f10c1260ab5936d2d4717a565cb57a11416fbf6c482c2e95f74902d0917f6'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1091 and sum(r['bytes'] for r in rows) == 590954684, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive59-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive59-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.90.0.json']
