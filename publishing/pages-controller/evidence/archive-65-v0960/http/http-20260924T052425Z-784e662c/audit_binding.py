"""Read-only, fail-closed intake of the actual Archive65 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
REPO = 'mekhovov/revealline-archive-65'
TREE = 'a8b6df07f906fa701562866075e07d60dc3dfcd1'
INVENTORY_SHA = '16522f243b4eadbea8085ff5cecb6d796b2ca572fbe6247c467091c2162cdb92'
LOCK_SHA = '3cdda8b2ff4a315a909295d08314e4f7db43ea713bbe722c4f4741e494aaa12c'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1120 and sum(r['bytes'] for r in rows) == 594895002, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive65-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive65-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.96.0.json']
