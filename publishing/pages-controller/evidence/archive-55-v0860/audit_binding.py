"""Read-only, fail-closed intake of the actual Archive55 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
BASE = 'https://mekhovov.github.io/revealline-archive-55/'
REPO = 'mekhovov/revealline-archive-55'
TREE = '5b6be1b91f41647f6e3250857aee29e737d7bba8'
INVENTORY_SHA = 'ee2bad670cb4d92382c831981d47b8a7955cd3f76855d1e4c4ff32949f6413a2'
LOCK_SHA = 'f34b88eeb5d2fbc436ebee576292a1e6a34c38806d8757b7379cc9cc03820a6a'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1087 and sum(r['bytes'] for r in rows) == 590887025, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive55-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive55-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.86.0.json']
