...
def row(path='releases/v0.82.1/site/game/example.json', body=b'{"ok":true}'):
...
class AuditTests(unittest.TestCase):
    ...
    def test_prepared_initial_inventory_is_exact_without_network(self):
        ...
        self.assertEqual(len(rows), 1059)
        self.assertEqual(sum(item['bytes'] for item in rows), 590404002)
