"""Read-only, fail-closed intake of the actual Archive51 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
AUTHORITY_ROOT = ROOT
INFRA = ROOT.parent / 'worktrees/revealline-archive-51'
REQUEST = ROOT / 'execution-request.reviewed.json'
BASE = 'https://mekhovov.github.io/revealline-archive-51/'
REPO = 'mekhovov/revealline-archive-51'
TREE = 'c057f683ae6f7afcf3696120d9d5d65fb9fb38d7'
INVENTORY_SHA = '7687ea84796d955b63b6bc11b41465a7777be7e7f24ccd07b2becd2eeee246c0'
LOCK_SHA = '577463d0b1cfc7709581e3abd73945a53792fc2735e9efa1c8a5694c8ab01512'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1059 and sum(r['bytes'] for r in rows) == 590404002, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive51-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive51-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.82.1.json']
