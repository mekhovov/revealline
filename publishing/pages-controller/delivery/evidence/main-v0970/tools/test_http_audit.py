...
class AuditTests(unittest.TestCase):
    def test_low_capacity_refuses_before_output_or_transport(self):
        with patch.object(audit, 'pinned_inventory', return_value=(b'{}', [row()])), \
            ...
            with self.assertRaisesRegex(ValueError, '60 MiB'):
                audit.main()
    ...
    def test_overall_deadline_expiring_during_final_authority_check_refuses(self):
        ...
        from types import SimpleNamespace
        expected = row()
        request = {key: 'fixture' for key in ['controllerTree', 'runId', 'deploymentId', 'deploymentStatusId', 'receiptArtifactId']}
        ...
        with tempfile.TemporaryDirectory() as directory, \
                ...
                patch.object(binding, 'validate_execution_binding', side_effect=late_authority), \
...
class MetadataCapacityTests(unittest.TestCase):
    def test_worst_case_three_attempts_all_rows_fits_below_free_guard(self):
        self.assertEqual(audit.MAX_WORST_CASE_METADATA_BYTES, 56623104)
    ...
    def test_complete_record_boundary_counts_headers_errors_urls_and_paths(self):
        import json
        output = io.StringIO()
        record = {
            'path': 'releases/v0.97.0/site/' + 'p' * 170 + '.json',
            'url': audit.BASE + 'releases/v0.97.0/site/' + 'p' * 170 + '.json',
            'status': 'FAIL', 'attempt': 3, 'expectedBytes': 100000000,
            'expectedSha256': 'a' * 64, 'sha256': 'b' * 64,
            'contentType': 'application/json; charset=utf-8',
            'contentEncoding': 'identity', 'contentLength': '100000000',
            'errorType': 'ValueError', 'error': '',
        }
        size = len((json.dumps(record) + '\n').encode())
        record['error'] = 'x' * (audit.MAX_LOG_LINE_BYTES - size)
        budget = audit.MetadataBudget()
        budget.line(output, record)
        self.assertEqual(budget.used, audit.MAX_LOG_LINE_BYTES)
        previous = output.getvalue()
        record['error'] += 'x'
        with self.assertRaisesRegex(ValueError, 'item exceeds'):
            budget.line(output, record)
    ...
    def test_inventory_above_capacity_never_creates_output_or_transport(self):
        import tempfile
        with tempfile.TemporaryDirectory() as directory, \
                patch.object(audit, 'BASE_DIR', Path(directory)), \
                patch.object(audit, 'pinned_inventory', return_value=(b'{}', [row()] * (audit.MAX_AUDIT_FILES + 1))), \
                patch.object(audit.urllib.request, 'build_opener', side_effect=AssertionError('no transport')), \
                patch('sys.argv', ['http_audit.py', '--run']):
            with self.assertRaisesRegex(ValueError, 'reviewed metadata capacity'):
                audit.main()
            self.assertEqual(list(Path(directory).iterdir()), [])

    def test_report_and_setup_file_limits_refuse_before_open(self):
        from unittest.mock import Mock
        target = Mock()
        for maximum in [audit.MAX_SETUP_BYTES, audit.MAX_REPORT_BYTES]:
            with self.assertRaisesRegex(ValueError, 'item exceeds'):
                audit.MetadataBudget().file(target, b'x' * (maximum + 1), maximum)
