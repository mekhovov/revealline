...
CHUNK_BYTES = 65536
WORKERS = 4
# This main audit persists metadata only. The finite receipt must fit this
# reviewed bound; a larger deployment needs a new capacity review before requests.
MIN_FREE_METADATA_BYTES = 60 * 1024**2
MAX_PERSISTED_METADATA_BYTES = 56 * 1024**2
MAX_LOG_LINE_BYTES = 2048
MAX_REPORT_BYTES = 14 * 1024**2
MAX_SETUP_BYTES = 4 * 1024**2
MAX_AUDIT_FILES = 4608
MAX_WORST_CASE_METADATA_BYTES = MAX_AUDIT_FILES * (MAX_ATTEMPTS + 1) * MAX_LOG_LINE_BYTES + MAX_REPORT_BYTES + MAX_SETUP_BYTES
...
def fetch_once(row, attempt, opener, *, deadline, clock=time.monotonic):
    ...
    try:
        ...
        request = urllib.request.Request(url, headers={
            ...
            'User-Agent': 'RevealLine-v097-main-byte-audit/1.0',
        })
...
def main():
    ...
    if not args.run:
        ...
        return 0
    if len(rows) > MAX_AUDIT_FILES:
        raise ValueError('Receipt inventory exceeds the reviewed metadata capacity')
    if shutil.disk_usage(BASE_DIR).free < MIN_FREE_METADATA_BYTES:
        raise ValueError('At least 60 MiB free metadata space required; no payloads are persisted')
    ...
    setup = {'expected-inventory.json': raw, 'execution-request.json': BINDING['requestRaw']}
    for name in original_helpers:
        setup[name] = binding.bounded(Path(__file__).with_name(name))
    for role, body in BINDING['evidenceBodies'].items():
        setup['authority-originals/' + role + ('.zip' if role == 'receiptZIP' else '.json')] = body
    for name, body in binding.pinned_metadata()[0].items():
        setup['source-metadata/' + name] = body
    ...
    report = {
        ...
        'status': 'FAIL' if failed or authority_error or audit_deadline_exceeded else 'PASS', 'base': BASE,
        ...
        'deadlineSemantics': 'Checked before/after body reads, after response close and after final authority validation. Expired completions fail. Blocking transport operations may overrun; socket timeout is an inactivity bound, not a strict process wall limit.',
    ...
    budget.file(out / 'http-report.json', binding.encoded(report), MAX_REPORT_BYTES)
