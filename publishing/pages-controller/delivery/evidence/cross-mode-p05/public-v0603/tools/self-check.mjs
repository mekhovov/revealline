...
assert.equal(SOURCE, 'a13ab970222498d7c5fa7f62f9fc04fe436979d5');
assert.equal(TREE, 'cf744b1ced9680b2fb4e0c304ddfc4b9f44b3e86');
assert.equal(VERSION, 'v0.60.3');
const retainedRaw = await readFile(path.join(home, 'retained-catalog.json'));
assert.equal(sha(retainedRaw), RETAINED_CATALOG_SHA);
assert.equal(JSON.parse(retainedRaw).releases.length, 73);
