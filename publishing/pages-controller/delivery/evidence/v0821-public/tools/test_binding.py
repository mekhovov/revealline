...
class BindingTests(unittest.TestCase):
    ...
    def write(self, *, missing_member=False, extra_member=False, special_member=False):
        ...
        self.request = {'format': 'revealline-main-v0821-http-request.v1', 'reviewed': True, 'controllerCommit': self.commit, 'controllerTree': self.tree, **self.ids, 'inventory': {'files': len(rows), 'bytes': sum(r['bytes'] for r in rows), 'sha256': b.sha(b.encoded({'base': b.BASE, 'files': rows}))}, 'pins': pins}
        self.save_request()
