...
import { SOURCE, TREE, VERSION, RETAINED_CATALOG_SHA, sha, safePath, validateRows, auditRows, isTransient, loadAuthority } from './audit-main.mjs';
import { inspect, expectedTypes } from './http-engine.mjs';
const home = path.dirname(fileURLToPath(import.meta.url));
assert.equal(SOURCE, '4c85277ac7393eeeabab035387d4d4ae8734aba1');
assert.equal(TREE, 'd3549d0efd15529f71f4cff9a940bdda5e84f3a6');
assert.equal(VERSION, 'v0.57.1');
const retainedRaw = await readFile(path.join(home, 'retained-catalog.json'));
assert.equal(sha(retainedRaw), RETAINED_CATALOG_SHA);
assert.equal(JSON.parse(retainedRaw).releases.length, 62);
