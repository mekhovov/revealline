...
expected=1136547026;limit=40*1024**2;reserve=512*1024**2
def disk(stage):
 ...
 with (P/'capacity.jsonl').open('a') as out:out.write(json.dumps(r)+'\n')
 assert profile_bytes<=limit,'Owned profile exceeds40MiB'
 assert r['projectedFreeAfterArtifact']-max(0,limit-profile_bytes)-8*1024**2>=reserve,'Insufficient projected reserve'
 return r
disk('before-launch')
args=['/Applications/Google Chrome.app/Contents/MacOS/Google Chrome','--headless=new','--remote-debugging-port=0','--enable-automation','--no-first-run','--no-default-browser-check','--disable-background-networking','--disable-component-update','--disable-default-apps','--disable-sync','--enable-unsafe-swiftshader','--password-store=basic','--use-mock-keychain','--disable-quic','--disable-extensions','--disk-cache-size=4194304','--media-cache-size=1048576','--window-size=390,844','--user-data-dir='+str(profile),'--no-proxy-server','about:blank']
...
try:
 ...
 (P/'exit.json').write_text(json.dumps({'pid':proc.pid,'exitCode':proc.returncode,'capacity':disk('after-exit')},indent=2)+'\n')
except BaseException as e:
 (P/'launch-failure.json').write_text(json.dumps({'type':type(e).__name__,'message':str(e)},indent=2)+'\n')
