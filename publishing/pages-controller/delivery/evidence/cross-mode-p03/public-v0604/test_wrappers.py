...
class WrapperTests(unittest.TestCase):
    def setUp(self):
        ...
        self.tree = 'd' * 40
        self.authorities = {
            'metadata/v0.60.4/manifest.json': contract.encode({'mock': 'manifest'}),
            'metadata/v0.60.4/release.json': contract.encode({'mock': 'record'}),
        ...
        for index, name in enumerate(sorted(contract.ASSETS)):
            path = {'manifest.json': 'metadata/v0.60.4/manifest.json',
                    'release.json': 'metadata/v0.60.4/release.json',
            ...
            assets.append({'id': 200 + index, 'name': name, 'size': len(body),
                           ...
                           'browser_download_url': f'https://github.com/mekhovov/revealline/releases/download/v0.60.4/{name}'})
    ...
    def refresh_runner(self, command, **kwargs):
        ...
        values = {'releases/latest': self.release, 'releases/tags/v0.60.4': self.release,
                  'git/ref/tags/v0.60.4': {'object': {'type': 'tag', 'sha': 'e' * 40}},
