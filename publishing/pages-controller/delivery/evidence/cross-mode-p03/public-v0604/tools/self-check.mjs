...
assert.equal(SOURCE, 'd77ea4f5c2aeccf2ba3bdf483a3f25531f8095b6');
assert.equal(TREE, 'f3a02f9d9e38650f2f9f61ddacd358dfc1f64333');
assert.equal(VERSION, 'v0.60.4');
const retainedRaw = await readFile(path.join(home, 'retained-catalog.json'));
assert.equal(sha(retainedRaw), RETAINED_CATALOG_SHA);
assert.equal(JSON.parse(retainedRaw).releases.length, 74);
