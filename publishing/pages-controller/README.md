...
## Archive10 combined admission preparation

The combined Archive10 deployment `6450531920` at infrastructure commit `55019541f8210ccef70f09cc04885f0debf1c4a7` contains v0.42 and v0.51: 976 files / 633,532,427 bytes. `evidence/archive-10/combined-051/` retains its hosted reconstruction and complete HTTP audit, including every attempt and result. The earlier v0.42-only evidence stays byte-exact at its original paths; the extracted historical admission identifies its original controller. Hosted receipts report `selectedFrozenTreesUnchanged: false` because comparison with an existing local frozen tree was not requested; the separate archived-CLI/source/manifest/ZIP and exact canonical-inventory checks passed. This does not establish browser acceptance.

The combined browser admission now binds the root-observed v0.42 win/picture/Results journey and v0.51 capture-stop/Resume/Pause journey. The v0.42 missing-pack refusal and v0.51 session-only limitation remain explicit; this is routing acceptance, with no new offline, save/load or physical-device claim. The earlier pending note remains a historical preparation record and is not an active browser pin. Current v0.51, all Archive01–09 admissions, source qualification, catalog, retention and testing-route policy are unchanged.
