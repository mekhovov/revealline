...
The reviewed selector candidate selects published **v0.64.1** from source `8ae70850db00777eb9afb233ff8a881fc26714f0`: readable Solo Replay export/import JSON at the shared Standard/Large text size. Both complete hosted source families passed 6,275 tests across 486 files on Node 20.19.6; the stable release's nine original assets are verified. The four downloaded metadata/qualification bodies match their published hashes. All 99 earlier catalog entries and the other 30 allocation/admission records are unchanged. Archive31 now retains v0.63.0 and adds v0.64.0 through its accepted successor: all 1,420 files / 627,122,610 bytes passed, and the scoped native entry/retry/pause journeys are bound to that deployment. Every earlier Archive31 evidence pin is retained as a historical reference. This candidate still requires publisher review and deployment. The accepted public baseline is v0.64.0: its full 3,400-file / 644,070,167-byte audit and scoped Team keyboard/layout journeys passed. v0.64.1 Pages deployment, complete public bytes and affected Replay journeys remain pending. Parent phases, Team win → Next, held rescue, audio/offline and physical-device gates remain open. See the [execution register](../../docs/cross-mode-execution.md).
