...
This directory publishes one explicitly selected immutable game and the semantic release explorer. It does not rebuild historical games or change their release records, tags, assets, original workers, saved-data keys or ZIP files. The `retainedReleasesPerMajor: "all"` policy retains every pinned semantic release, including after the 100th entry in a major line. Numeric values from 1 through 100 remain supported for explicitly bounded selections. The 1,024-entry catalog, archive admissions and existing main/archive byte budgets still apply. The root and `/game/` select only the newest published stable release; historical and comparison entries link to their own canonical archives. A canonical archive URL satisfies each earlier phase's Pages testing; main does not need to cycle through those phases.
...
The HUD publisher candidate selects **v0.64.2** from source `adbfcb94037b8a38d49b9650e7f8a83c7face1db`: readable two-row Solo flight counters in short landscape. Both complete hosted source families passed 6,275 tests across 486 files on Node 20.19.6. Stable latest release `392067660` is published; all nine release assets remain identical to their qualified descriptors, and the four retained metadata/qualification bodies match their exact hashes. All 100 earlier catalog entries, 31 prior archive allocations/admissions and historical evidence remain unchanged.

**Archive32 is accepted; HUD Pages acceptance remains pending.** Accepted main baseline v0.64.1 remains at publisher `95575953e9601c2ee018e9803ea6f90db9d93e20`, Pages run `35445356978`, deployment `6541328289`; its complete audit had one resolved transient503 retry and no final failures. Archive32 repository `1377207652`, infrastructure `50909e7cf38916fe7d3e2e7eb79339bb49ae80b9`, Pages run `35446915477` and deployment `6541593688` now retain v0.64.1. The independent public HTTP audit verified all712 files /313,567,687 bytes without failures or retries; fresh authorities and root acceptance combine those bytes with scoped native keyboard win/picture, Pause/details/Resume, Replay/focus and explorer routing. All31 earlier admissions and allocations remain unchanged; Archive32 is the32nd. This candidate enables the HUD selector for production validation and publisher review. Actual HUD deployment, complete public-byte and affected player-journey checks still follow. Physical controls, audio/offline, Team two-core encounter, held rescue and parent P03/P05/P08/P18 remain open. See the [execution register](../../docs/cross-mode-execution.md).
