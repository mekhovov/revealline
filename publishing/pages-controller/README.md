...
This directory publishes one explicitly selected immutable game and the semantic release explorer. It does not rebuild historical games or change their release records, tags, assets, original workers, saved-data keys or ZIP files. The `retainedReleaseCount: 5` policy selects five stable releases globally in numeric semantic-version order, including the current release during an explicit rollback. Explicit testing and comparison routes remain addressable but are excluded from the public selector. Existing `retainedReleasesPerMajor` configurations retain their legacy behavior, including `"all"`; exactly one retention field must be present. Global counts support integers 1 through 100. Historical tags, releases, assets, archives and evidence remain preserved. Every local admission and evidence pin is validated; only selected or explicitly requested testing archives receive synchronous remote checks. The 1,024-entry catalog and shared maximum of 128 archive shards/admissions remain bounded. Each new archive still requires exact allocation, original-byte inventory, HTTP verification and browser approval. The 800,000,000-byte archive and 950,000,000-byte main budgets are unchanged. Root and `/game/` select only the newest published stable release; retained historical and comparison entries link to canonical archives.
