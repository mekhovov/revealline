"""Read every hash-pinned Archive32 HTTP body without persisting payloads.
...
from audit_binding import validate_execution_binding, candidate_inventory, preservation_metadata
...
def fetch_once(row, attempt, opener, *, deadline, clock=time.monotonic):
    ...
    try:
        ...
        request = urllib.request.Request(url, headers={
            ...
            'User-Agent': 'RevealLine-archive32-byte-audit/2.0',
        })
...
def main():
    ...
    parser.add_argument('--run', action='store_true', help='Audit the fixed public Archive32 inventory')
    ...
    if args.prepare:
        ...
        for row in rows:
            validate_row(row)
        print(json.dumps({'status': 'PREPARED_ACTUAL_BINDING_REQUIRED', 'files': len(rows), 'bytes': EXPECTED_BYTES, **preservation_metadata(rows), 'networkRequests': 0}))
        return 0
    ...
    (out / 'prior-expected-inventory.json').write_bytes((BASE_DIR / 'inputs/prior-expected-inventory.json').read_bytes())
    ...
    report = {
        'status': 'FAIL' if failed or not unchanged_authority else 'PASS', 'base': BASE, 'archiveCommit': COMMIT,
    ...
    return 1 if failed or not unchanged_authority else 0
