...
assert.equal(SOURCE, '7a9985ec00168b99a86c1d5db0457343ca18ffae');
assert.equal(TREE, '4a5960488f4c19fc8fbd6e784c612b8bdb81596e');
assert.equal(VERSION, 'v0.64.3');
const retainedRaw = await readFile(path.join(home, 'retained-catalog.json'));
assert.equal(sha(retainedRaw), RETAINED_CATALOG_SHA);
assert.equal(JSON.parse(retainedRaw).releases.length, 101);
