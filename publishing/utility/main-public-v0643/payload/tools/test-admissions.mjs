...
configured.admissions[configured.admissions.findIndex(a => a.id === 'archive-32')] = JSON.parse(await readFile(new URL('./test-fixtures/new-admission.json', import.meta.url)));
...
assert.throws(() => validateAdmissions(prior, observed(prior)), /accepted v64.2 append/);
...
assert.throws(() => validateAdmissions(wrongNew, observed(wrongNew)), /accepted v64.2 append/);
