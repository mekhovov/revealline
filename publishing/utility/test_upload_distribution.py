...
import upload_distribution as u
            ...
            def do_GET(self):
                ...
                if self.path == PREFIX:
                    ...
                    if owner.mode == 'duplicate-page2':
                        rows = [{'name': f'other-{i}'} for i in range(100)] if page == 1 else [{'name': 'distribution.zip'}]
                    else:
                        rows = owner.assets
...
class UploaderTests(unittest.TestCase):
    def setUp(self):
        ...
        with zipfile.ZipFile(self.outer, 'w', compression=zipfile.ZIP_DEFLATED) as z:
            z.writestr(TAG + '/site/distribution.zip', self.tar)
        self.args = types.SimpleNamespace(outer_zip=str(self.outer), outer_bytes=self.outer.stat().st_size,
                    outer_sha256=hashlib.sha256(self.outer.read_bytes()).hexdigest(), member_bytes=len(self.tar),
                    member_sha256=hashlib.sha256(self.tar).hexdigest(), source_commit='a' * 40, tag=TAG,
        ...
        self.inspection = {'format': 'revealline-qualified-artifact-offline-inspection.v1', 'status': 'PASS',
            ...
            'distribution': {'outerMember': TAG + '/site/distribution.zip', 'bytes': len(self.tar),
                          'sha256': self.args.member_sha256, 'copiedToDisk': False}}
        self.receipt()
    ...
    def tearDown(self):
        self.assertFalse(any(p.name == 'distribution.zip' for p in self.dir.rglob('*')))
        self.temp.cleanup()
    ...
    def test_stream_upload_exact_length_hash_and_readback(self):
        with self.server() as server, contextlib.closing(u.PinnedDistribution(self.args)) as source:
            ...
            self.assertEqual(server.posts[0], {'path': PREFIX + '/assets?name=distribution.zip',
                'declared': len(self.tar), 'bytes': len(self.tar), 'sha256': self.args.member_sha256,
                'type': 'application/zip', 'chunked': None})
            self.assertEqual(len(server.gets), 6)

    def test_annotated_tag_resolves_exact_commit(self):
        with self.server('annotated-tag') as server, contextlib.closing(u.PinnedDistribution(self.args)) as source:
    ...
    def test_wrong_or_cyclic_tag_refused_before_post(self):
        for mode in ['wrong-tag-commit', 'tag-cycle']:
            with self.subTest(mode=mode), self.server(mode) as server, contextlib.closing(u.PinnedDistribution(self.args)) as source:
    ...
    def test_tag_change_after_send_is_ambiguous(self):
        with self.server('tag-changed-after-send') as server, contextlib.closing(u.PinnedDistribution(self.args)) as source:
    ...
    def test_inspected_root_member_layout_supported(self):
        with zipfile.ZipFile(self.outer, 'w') as z:
            z.writestr('site/distribution.zip', self.tar)
        ...
        self.inspection['distribution']['outerMember'] = 'site/distribution.zip'
        self.receipt()
        with contextlib.closing(u.PinnedDistribution(self.args)) as source:
            self.assertEqual(source.member, 'site/distribution.zip')
    ...
    def test_bad_source_hash_refused_before_http(self):
        self.args.member_sha256 = '0' * 64
        self.inspection['distribution']['sha256'] = self.args.member_sha256
        self.receipt()
