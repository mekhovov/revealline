...
assert.deepEqual(output([row('v0.62.1')], []), {
  retainedCatalogAuthorities: 0, newCatalogAuthorities: 1, totalCatalogAuthorities: 1,
  retainedCatalogVersions: [], newCatalogVersions: ['v0.62.1'], historicalHTTPBodyPreservationClaimed: false,
});
assert.deepEqual(output([row('v0.62.1'), row('v0.61.4'), row('v0.61.5')], [row('v0.61.5'), row('v0.61.4')]), {
  retainedCatalogAuthorities: 2, newCatalogAuthorities: 1, totalCatalogAuthorities: 3,
  retainedCatalogVersions: ['v0.61.4', 'v0.61.5'], newCatalogVersions: ['v0.62.1'], historicalHTTPBodyPreservationClaimed: false,
});
const retained = JSON.parse(await readFile(new URL('./retained-catalog.json', import.meta.url))).releases;
const exact = output([...retained, row('v0.62.1')], retained);
...
assert.deepEqual(exact.newCatalogVersions, ['v0.62.1']);
assert.equal(exact.historicalHTTPBodyPreservationClaimed, false);
assert.throws(() => output([row('v0.62.1')], [row('v0.61.5')]), /missing/);
assert.throws(() => output([row('v0.62.1'), row('v0.62.1')], []), /unique/);
assert.throws(() => output([row('v0.62.1')], [row('v0.62.1'), row('v0.62.1')]), /unique/);
