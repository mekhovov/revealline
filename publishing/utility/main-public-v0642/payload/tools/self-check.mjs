...
assert.equal(SOURCE, 'adbfcb94037b8a38d49b9650e7f8a83c7face1db');
assert.equal(TREE, '32369df7260360cc273857b195b2c2c7ca855994');
assert.equal(VERSION, 'v0.64.2');
const retainedRaw = await readFile(path.join(home, 'retained-catalog.json'));
assert.equal(sha(retainedRaw), RETAINED_CATALOG_SHA);
assert.equal(JSON.parse(retainedRaw).releases.length, 100);
