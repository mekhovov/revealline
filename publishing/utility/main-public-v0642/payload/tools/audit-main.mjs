...
export const SOURCE = 'adbfcb94037b8a38d49b9650e7f8a83c7face1db';
export const TREE = '32369df7260360cc273857b195b2c2c7ca855994';
export const VERSION = 'v0.64.2';
export const RETAINED_CATALOG_SHA = '05ba9feb3776b1091610b7de175463d5a41b247d15142a6f3b5f5c5ad38a369d';
...
export function validateAdmissions(configuration, receipt) {
  require(Array.isArray(configuration.admissions) && configuration.admissions.length === 32 &&
    receipt.admittedArchives === 32 && Array.isArray(receipt.observations) && receipt.observations.length === 32,
    'Exactly32 archive admissions and observations required.');
  const expectedIds = Array.from({ length: 32 }, (_, i) => 'archive-' + String(i + 1).padStart(2, '0'));
  const ids = configuration.admissions.map(a => a.id), observedIds = receipt.observations.map(a => a.archiveId);
  require(new Set(ids).size === 32 && new Set(observedIds).size === 32 &&
  ...
  for (const a of configuration.admissions) {
    ...
    for (const e of a.evidence) {
      safePath(e.path);
      require((a.id !== 'archive-32' || e.path.startsWith('evidence/archive-32/')) && !paths.has(e.path) && sha256(e.sha256) &&
    ...
    if (a.id === 'archive-32') {
      require(a.infrastructureCommit === '50909e7cf38916fe7d3e2e7eb79339bb49ae80b9' && a.deploymentId === 6541593688,
        'Archive32 must use the accepted initial v64.1 archive.');
  ...
  return { total: 32, retainedUnchanged: 31, updated: ['archive-32'], externalBodiesReaudited: false };
}
...
export function validateModels(binding, models, raws, retainedCatalog) {
  ...
  require(sha(raws.catalog) === catalogSha && catalog.format === 'revealline-frozen-catalog.v1' &&
    ...
    catalog.releases.length === 101 && retainedCatalog.releases.length === 100 &&
