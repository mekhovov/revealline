EXECUTION_REQUEST_SHA = '67dcd2439c33e2e348ab915c0311e4a1ad2edc4c1add74b6fe7bdedbee3d15b8'
...
assert report['status']=='PASS';assert len(inv)==len(results)==711;expected={r['path']:r for r in inv};assert len(expected)==711
...
assert len(unchanged)==len(release)==0 and not support and not changed and len(added)==711
...
assert report['preservedRootSupportPaths']==sorted(support) and report['changedPriorPaths']==sorted(changed) and report['newRows']==len(added) and report['newBytes']==sum(expected[p]['bytes'] for p in added)==313557913
assert (H/'prior-expected-inventory.json').read_bytes()==priorraw
assert sum(r['bytes'] for r in results)==report['verifiedBytes']==report['expectedBytes']==313557913
