EXECUTION_REQUEST_SHA = '2fb2a5fcb33475ffc3e3a64ea78388a83d074cc11b2946cf9745189b347c9732'
...
assert report['status']=='PASS';assert len(inv)==len(results)==1420;expected={r['path']:r for r in inv};assert len(expected)==1420
...
prior=read(D/'inputs/prior-expected-inventory.json')['files'];previous={r['path']:r for r in prior};assert len(previous)==len(prior)==711
unchanged={p for p,r in previous.items() if expected.get(p)==r};changed=set(previous)-unchanged;added=set(expected)-set(previous)
release={p for p in unchanged if p.startswith('releases/v0.63.0/')};support=unchanged-release
assert len(unchanged)==710 and len(release)==708 and support=={'.nojekyll','releases/index.html'} and changed=={'index.html'} and len(added)==709
...
assert report['preservedRootSupportPaths']==sorted(support) and report['changedPriorPaths']==sorted(changed) and report['newRows']==len(added) and report['newBytes']==sum(expected[p]['bytes'] for p in added)==313564618
assert expected['index.html']['bytes']-previous['index.html']['bytes']==79
assert (H/'prior-expected-inventory.json').read_bytes()==priorraw
assert sum(r['bytes'] for r in results)==report['verifiedBytes']==report['expectedBytes']==627122610
