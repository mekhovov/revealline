#!/usr/bin/env python3
"""Pinned source.tar ZIP-member upload. Default is local verification, not upload."""
import argparse
import hashlib
import http.client
import json
...
class PinnedSource:
    ...
    def __init__(self, args):
        ...
        try:
            ...
            require(not self.info.is_dir() and self.info.file_size == args.source_bytes,
                    'ZIP source member byte length differs')
            self.size, self.sha256, self.commit = args.source_bytes, args.source_sha256, args.source_commit
...
def main():
    ...
    for name in ('outer-zip', 'inspection', 'outer-sha256', 'source-sha256', 'inspection-sha256',
                 'source-commit', 'tag', 'repository', 'receipt'):
        parser.add_argument('--' + name, required=True)
    for name in ('outer-bytes', 'source-bytes', 'release-id'):
    ...
    result = {'status': 'REFUSED', 'uploadRequested': args.upload, 'repository': args.repository,
              ...
              'sourceBytes': args.source_bytes, 'sourceSha256': args.source_sha256,
