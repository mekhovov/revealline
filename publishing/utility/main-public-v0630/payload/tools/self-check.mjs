...
assert.equal(SOURCE, '3e47f60b9f9b4c5088410536d649ff595b2667eb');
assert.equal(TREE, 'fd0138c0e5f71b8b25368878c1948f5bef14255d');
assert.equal(VERSION, 'v0.63.0');
const retainedRaw = await readFile(path.join(home, 'retained-catalog.json'));
assert.equal(sha(retainedRaw), RETAINED_CATALOG_SHA);
assert.equal(JSON.parse(retainedRaw).releases.length, 97);
