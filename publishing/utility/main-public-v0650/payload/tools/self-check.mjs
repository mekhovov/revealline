...
assert.equal(SOURCE, '57fdc39aa78683f6de479eb77f810f60e7c6756a');
assert.equal(TREE, 'b71a23d6c2474da72ef379793c811e7097b0a156');
assert.equal(VERSION, 'v0.65.0');
const retainedRaw = await readFile(path.join(home, 'retained-catalog.json'));
assert.equal(sha(retainedRaw), RETAINED_CATALOG_SHA);
assert.equal(JSON.parse(retainedRaw).releases.length, 102);
