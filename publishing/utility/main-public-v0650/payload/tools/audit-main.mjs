...
export const SOURCE = '57fdc39aa78683f6de479eb77f810f60e7c6756a';
export const TREE = 'b71a23d6c2474da72ef379793c811e7097b0a156';
export const VERSION = 'v0.65.0';
export const RETAINED_CATALOG_SHA = '99c628f8cb63983d1fd1815ac3667323031dc678518186ca2a4a2e34d8869ca0';
...
export function validateAdmissions(configuration, receipt) {
  require(Array.isArray(configuration.admissions) && configuration.admissions.length === 33 &&
    receipt.admittedArchives === 33 && Array.isArray(receipt.observations) && receipt.observations.length === 33,
    'Exactly33 archive admissions and observations required.');
  const expectedIds = Array.from({ length: 33 }, (_, i) => 'archive-' + String(i + 1).padStart(2, '0'));
  const ids = configuration.admissions.map(a => a.id), observedIds = receipt.observations.map(a => a.archiveId);
  require(new Set(ids).size === 33 && new Set(observedIds).size === 33 &&
  ...
  for (const a of configuration.admissions) {
    ...
    for (const e of a.evidence) {
      safePath(e.path);
      require((a.id !== 'archive-33' || e.path.startsWith('evidence/archive-33/')) && !paths.has(e.path) && sha256(e.sha256) &&
    ...
    if (a.id === 'archive-33') {
      require(a.infrastructureCommit === 'b184d8d8872ec305d8606ac99bd3d6235e291db8' && a.deploymentId === 6545564229,
        'Archive33 must use the accepted v64.3 preservation.');
  ...
  return { total: 33, retainedUnchanged: 32, updated: ['archive-33'], externalBodiesReaudited: false };
}
...
export function validateModels(binding, models, raws, retainedCatalog) {
  ...
  require(sha(raws.catalog) === catalogSha && catalog.format === 'revealline-frozen-catalog.v1' &&
    ...
    catalog.releases.length === 103 && retainedCatalog.releases.length === 102 &&
