...
def row(path='releases/v0.64.3/site/game/example.json', body=b'{"ok":true}'):
...
class AuditTests(unittest.TestCase):
    ...
    def test_prepared_initial_inventory_is_exact_without_network(self):
        ...
        self.assertEqual(sum(item['bytes'] for item in rows), 313572608)
    ...
    def test_locked_immutable_tag_refspec_pass(self):
        binding.validate_tag_fetch_cohorts('git fetch origin refs/tags/v0.64.3:refs/tags/v0.64.3 refs/tags/v0.61.24:refs/tags/v0.61.24', {'releases': [{'version': 'v0.64.3'}, {'version': 'v0.61.24'}]})

    def test_missing_locked_tag_refspec_refuses(self):
        with self.assertRaises(ValueError):
            binding.validate_tag_fetch_cohorts('git fetch origin refs/tags/v0.64.3:refs/tags/v0.64.3', {'releases': [{'version': 'v0.64.3'}, {'version': 'v0.61.24'}]})

    def test_crosswired_tag_refspec_refuses(self):
        with self.assertRaises(ValueError):
            binding.validate_tag_fetch_cohorts('git fetch origin refs/tags/v0.64.3:refs/tags/v0.61.4', {'releases': [{'version': 'v0.64.3'}]})
