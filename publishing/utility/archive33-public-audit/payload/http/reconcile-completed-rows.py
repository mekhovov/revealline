EXECUTION_REQUEST_SHA = '81855f7452d8d058b73c5fb5d6fa202f54b43430014ea780e3cf3a6022367831'
...
assert report['status']=='PASS';assert len(inv)==len(results)==712;expected={r['path']:r for r in inv};assert len(expected)==712
...
prior=read(D/'inputs/prior-expected-inventory.json')['files'];previous={r['path']:r for r in prior};assert len(previous)==len(prior)==0
unchanged={p for p,r in previous.items() if expected.get(p)==r};changed=set(previous)-unchanged;added=set(expected)-set(previous)
release={p for p in unchanged if p.startswith('releases/v0.64.3/')};support=unchanged-release
assert len(unchanged)==len(release)==0 and not support and not changed and len(added)==712
...
assert report['preservedRootSupportPaths']==sorted(support) and report['changedPriorPaths']==sorted(changed) and report['newRows']==len(added) and report['newBytes']==sum(expected[p]['bytes'] for p in added)==313572608
assert (H/'prior-expected-inventory.json').read_bytes()==priorraw
assert sum(r['bytes'] for r in results)==report['verifiedBytes']==report['expectedBytes']==313572608
