EXECUTION_REQUEST_SHA = '9db1ef2e06e573f827228b137955d854e37895e439c4e925f7949d84036be80f'
...
assert report['preservedRootSupportPaths']==sorted(support) and report['changedPriorPaths']==sorted(changed) and report['newRows']==len(added) and report['newBytes']==sum(expected[p]['bytes'] for p in added)==313571215
assert expected['index.html']['bytes']-previous['index.html']['bytes']==79
assert (H/'prior-expected-inventory.json').read_bytes()==priorraw
assert sum(r['bytes'] for r in results)==report['verifiedBytes']==report['expectedBytes']==627143902
