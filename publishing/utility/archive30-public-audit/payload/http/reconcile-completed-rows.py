EXECUTION_REQUEST_SHA = 'ac0b16f2172689be950595641ea9156963ba003c017b96711ff67dd833130b36'
...
assert report['status']=='PASS';assert len(inv)==len(results)==705;expected={r['path']:r for r in inv};assert len(expected)==705
...
release={p for p in unchanged if p.startswith('releases/v0.62.0/')};support=unchanged-release
assert len(unchanged)==len(release)==0 and not support and not changed and len(added)==705
...
assert report['preservedRootSupportPaths']==sorted(support) and report['changedPriorPaths']==sorted(changed) and report['newRows']==len(added) and report['newBytes']==sum(expected[p]['bytes'] for p in added)==313563420
assert (H/'prior-expected-inventory.json').read_bytes()==priorraw
assert sum(r['bytes'] for r in results)==report['verifiedBytes']==report['expectedBytes']==313563420
