...
assert report['status']=='PASS';assert len(inv)==len(results)==1408;expected={r['path']:r for r in inv};assert len(expected)==1408
...
prior=read(D/'inputs/prior-expected-inventory.json')['files'];previous={r['path']:r for r in prior};assert len(previous)==len(prior)==705
...
assert len(unchanged)==704 and len(release)==702 and support=={'.nojekyll','releases/index.html'} and changed=={'index.html'} and len(added)==703
...
assert report['preservedRootSupportPaths']==sorted(support) and report['changedPriorPaths']==sorted(changed) and report['newRows']==len(added) and report['newBytes']==sum(expected[p]['bytes'] for p in added)==313522906
assert (H/'prior-expected-inventory.json').read_bytes()==priorraw
assert sum(r['bytes'] for r in results)==report['verifiedBytes']==report['expectedBytes']==627086405
