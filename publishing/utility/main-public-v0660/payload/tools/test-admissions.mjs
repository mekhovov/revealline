...
configured.admissions[configured.admissions.findIndex(a => a.id === 'archive-33')] = JSON.parse(await readFile(new URL('./test-fixtures/new-admission.json', import.meta.url)));
const observed = c => ({ admittedArchives: 33, observations: c.admissions.map(a => ({ archiveId: a.id,
...
assert.deepEqual(validateAdmissions(configured, observed(configured)),
  { total: 33, retainedUnchanged: 32, updated: ['archive-33'], externalBodiesReaudited: false });
...
assert.throws(() => validateAdmissions(missing, observed(missing)), /Exactly33/);
...
assert.throws(() => validateAdmissions(prior, observed(prior)), /accepted v65.0 append/);
...
assert.throws(() => validateAdmissions(wrongNew, observed(wrongNew)), /accepted v65.0 append/);
console.log(JSON.stringify({ status: 'ARCHIVE_ADMISSION_BOUNDARY_CHECK_PASS', authorities: 33,
 retainedUnchanged: 32, cases: 9, fixtureOnly: true, realNetworkRequests: 0, publicAcceptance: false }));
