...
The PR and manual runs must be distinct completed successful runs, with full unpaginated jobs inventories. PR `head_sha` must equal the supplied PR source, and manual `head_sha` must equal the frozen source. The manual run must have successful qualify/freeze jobs and one skipped `test` job or four skipped `test (N)` jobs. Mandatory actual gate, production-ledger, source-identity, freeze, retained-assets, and exact PR validation steps must be successful. The PR evidence must prove exactly one supported build mode: either the current explicit artifact deferral plus post-gate source check, or a historical successful full PR artifact build plus post-build source check. An inactive alternate mode may be absent or explicitly skipped; partial, mixed-success, failed, or relabeled evidence refuses. In fast mode the full artifact is built once from frozen merged source; in both modes that frozen artifact is independently inspected. Source verdicts are not borrowed from predecessors. If a PR workflow reports a synthetic merge SHA instead of the supplied source, the adapter refuses; collect appropriately bound source evidence rather than relabeling the run.
