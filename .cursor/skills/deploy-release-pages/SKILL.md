...
For a release touching More worlds, verify same-chapter preservation and different-chapter Stay/Replace through all three chapter adapters. Keep failed-save/readback and delayed/background callback evidence separate from actual browser input observations. A passing direct `selectEntry` or installation test does not establish safe player selection.
